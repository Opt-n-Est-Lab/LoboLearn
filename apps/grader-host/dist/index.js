...
async.series([
    ...
    async () => {
        ...
        async function worker() {
            ...
            while (true) {
                if (!healthCheck.isHealthy() || processTerminating)
                    return;
                ...
                await receiveFromQueue(sqs, config.jobsQueueUrl, async (job) => {
                    ...
                    await handleJob(job).then(() => globalLogger.info(`handleJob(${job.jobId}) succeeded`), (err) => {
...
async function initDocker(context) {
    const { logger, docker, job: { image }, } = context;
    let dockerAuth = {};
...
async function initFiles(context) {
    const { logger, s3, job: { jobId, s3Bucket, s3RootKey, entrypoint }, } = context;
    ...
    try {
        ...
        if (entrypoint != null) {
            ...
            if (path.isAbsolute(entrypointFirstToken) &&
...
async function runJob(context, receivedTime, tempDir) {
    const { docker, logger, job: { jobId, image, entrypoint, timeout, enableNetworking, environment }, } = context;
    ...
    const task = (async () => {
        ...
        if (results.succeeded) {
            await fs.readFile(path.join(tempDir, 'results', 'results.json')).then((data) => {
...
async function uploadResults(context, results) {
    const { logger, s3, job: { jobId, s3Bucket, s3RootKey }, } = context;
...
async function uploadArchive(context, tempDir) {
    const { logger, s3, job: { s3Bucket, s3RootKey }, } = context;
...
async function uploadLogs(context) {
    const { logger, s3, job: { s3Bucket, s3RootKey }, } = context;
