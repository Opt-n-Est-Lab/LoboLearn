...
import { AssessmentSyncErrorsAndWarnings, CourseInstanceSyncErrorsAndWarnings, CourseSyncErrorsAndWarnings, QuestionSyncErrorsAndWarnings, } from './SyncErrorsAndWarnings.html.js';
import { SyncProblemButton } from './SyncProblemButton.html.js';
export async function browseDirectory({ paths, }) {
    const filenames = await fs.readdir(paths.workingPath);
    const all_files = await async.mapLimit(filenames.sort().map((name, index) => ({ name, index })), 3, async (file) => {
        ...
        if (stats.isFile()) {
            ...
            const sync_data = await editorUtil.getErrorsAndWarningsForFilePath(paths.courseId, relative_path);
            return {
                id: file.index,
                name: file.name,
                isFile: true,
...
export async function browseFile({ paths }) {
...
export async function createFileBrowser({ resLocals, paths, isReadOnly, }) {
    ...
    if (stats.isDirectory()) {
        ...
        throw new Error(`Invalid working path - ${paths.workingPath} is neither a directory nor a file`);
    }
}
export function FileBrowser({ resLocals, paths, isFile, fileInfo, directoryListings, isReadOnly, }) {
    const { navPage, __csrf_token: csrfToken, authz_data, course, urlPrefix } = resLocals;
    const syncErrorsAndWarnings = navPage === 'course_admin'
        ...
        : navPage === 'instance_admin'
            ...
            : navPage === 'assessment'
                ...
                : navPage === 'question' || navPage === 'public_question'
                    ...
                    : '';
    const pageTitle = navPage === 'course_admin'
        ? 'Course Files'
        : navPage === 'instance_admin'
        ...
        <div class="card-header bg-primary text-white">
          <div class="row align-items-center justify-content-between">
            <div class="col-auto font-monospace d-flex">
              ${joinHtml(breadcrumbPaths.map((dir) => html `
                    ${dir.canView
            ? html `
                          <a
                            class="text-white"
...
function FileBrowserActions({ paths, fileInfo, isReadOnly, csrfToken, }) {
    ...
    <a
      ...
      href="${paths.urlPrefix}/file_download/${encodedPath}?attachment=${encodeURIComponent(fileInfo.name)}"
    >
    ...
    ${isReadOnly
        ...
        : html `
          <button
            ...
            data-bs-content="${escapeHtml(FileRenameForm({ file: fileInfo, csrfToken, isViewingFile: true }))}"
...
function DirectoryBrowserActions({ paths, csrfToken, }) {
    return html `
    ${paths.specialDirs?.map((d) => html `
        <button
          type="button"
          ...
          data-bs-placement="auto"
          data-bs-title="Upload file"
          data-bs-content="${escapeHtml(FileUploadForm({
...
function FileContentPreview({ paths, fileInfo, }) {
    if (fileInfo.isImage) {
        return html `
      <img
...
function DirectoryBrowserBody({ paths, directoryListings, isReadOnly, csrfToken, }) {
    return html `
    <table class="table table-sm table-hover" aria-label="Directories and files">
      ...
      <tbody>
        ${directoryListings.files?.map((f) => html `
            <tr>
              <td>
                ${f.sync_errors
        ? SyncProblemButton({
        ...
        : html `
                ...
                <a
                  ...
                  href="${paths.urlPrefix}/file_download/${encodePath(f.path)}?attachment=${encodeURIComponent(f.name)}"
                >
                  <i class="fa fa-arrow-down"></i>
                  <span>Download</span>
                </a>
        ...
        : html `
                      <button
                        ...
                        data-bs-placement="auto"
                        data-bs-title="Rename file"
                        data-bs-content="${escapeHtml(FileRenameForm({ file: f, csrfToken, isViewingFile: false }))}"
          ...
          `)}
        ${directoryListings.dirs.map((d) => html `
            <tr>
              <td colspan="2">
...
function FileUploadForm({ file, csrfToken }) {
    return html `
    <form
...
function FileDeleteForm({ file, csrfToken }) {
    return html `
...
function FileRenameForm({ file, csrfToken, isViewingFile, }) {
    return html `
    <form
...
//# sourceMappingURL=FileBrowser.html.js.map