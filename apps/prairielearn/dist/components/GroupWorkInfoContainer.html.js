...
export function GroupWorkInfoContainer({ groupConfig, groupInfo, userCanAssignRoles, csrfToken, }) {
    return html `
    <div class="container-fluid">
      <div class="row">
        ...
        : ''}
          ...
          ${groupInfo.groupMembers.map((user) => groupConfig.has_roles
        ? html `
                  <li>
                    ${user.uid} -
...
function LeaveGroupModal({ csrfToken }) {
    return Modal({
        id: 'leaveGroupModal',
...
function GroupRoleTable({ groupInfo, userCanAssignRoles, csrfToken, }) {
    const { rolesInfo, groupMembers, groupSize } = groupInfo;
    if (!rolesInfo)
        return '';
    const roleConfigProblems = rolesInfo.validationErrors.length +
    ...
    <details class="card mb-2">
      ...
      <div class="card-body">
        ...
        <form id="role-select-form" name="role-select-form" method="POST">
          ...
          >
            ...
            <tbody>
              ${groupMembers.map((user) => html `
                  <tr>
                    <td>${user.uid}</td>
                    <td>
                      ${rolesInfo.groupRoles.map((role) => html `
                          <label
        ...
        ? 'disabled'
        : ''}
                              ${rolesInfo.roleAssignments[user.uid]?.some((a) => idsEqual(a.group_role_id, role.id))
        ? 'checked'
        : ''}
                            />
                            ${role.role_name}
      ...
      <div class="card-footer small">
        ...
        >
          ...
          <tbody>
            ${rolesInfo.groupRoles.map((groupRole) => html `
                <tr>
...
function GroupRoleErrors({ rolesInfo, groupSize, }) {
    return html `
    ${!rolesInfo.rolesAreBalanced
        ...
        `
        : ''}
    ${rolesInfo.validationErrors?.map(({ role_name, count, minimum, maximum }) => html `
        <div class="alert alert-danger" role="alert">
...
//# sourceMappingURL=GroupWorkInfoContainer.html.js.map