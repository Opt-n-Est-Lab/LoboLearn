...
import { DateFromISOString, IntervalSchema, } from '../lib/db-types.js';
export function InstructorInfoPanel({ course, course_instance, assessment, assessment_instance, instance_question, assignedGrader, lastGrader, question, variant, instance_group, instance_group_uid_list, instance_user, authz_data, question_is_shared, questionContext, csrfToken, }) {
...
function InstanceUserInfo({ instance_user, instance_group, instance_group_uid_list, }) {
    if (instance_user == null && instance_group == null)
        return '';
    return html `
    <div>
      <details>
...
function QuestionInfo({ course, course_instance, question, variant, question_is_shared, questionContext, }) {
    if (question == null || variant == null)
        return '';
    const questionPreviewUrl = `${config.urlPrefix}/${course_instance != null
        ? `course_instance/${course_instance.id}/instructor`
        : `course/${course.id}`}/question/${question.id}?variant_seed=${variant.variant_seed}`;
    ...
    const sharingQid = course.example_course || !question.share_publicly
        ? question.qid
...
function VariantInfo({ variant, timeZone, questionContext, }) {
    if (variant == null)
        return '';
    // Some legacy queries still return the duration and date as a string, so parse them before formatting
    const duration = typeof variant.duration === 'string'
        ? IntervalSchema.parse(variant.duration)
        : (variant.duration ?? 0);
    const date = typeof variant.date === 'string' ? DateFromISOString.parse(variant.date) : variant.date;
    return html `
...
function AssessmentInstanceInfo({ assessment, assessment_instance, timeZone, }) {
    if (assessment == null || assessment_instance == null)
        return '';
    const instructorUrlPrefix = `${config.urlPrefix}/course_instance/${assessment.course_instance_id}/instructor`;
    // Some legacy queries still return the duration and date as a string, so parse them before formatting
    const duration = typeof assessment_instance.duration === 'string'
        ? IntervalSchema.parse(assessment_instance.duration)
        : (assessment_instance.duration ?? 0);
    const date = typeof assessment_instance.date === 'string'
...
function ManualGradingInfo({ instance_question, assignedGrader, lastGrader, assessment, questionContext, }) {
...
function IssueReportButton({ variant, csrfToken, questionContext, }) {
    if (variant == null ||
...
//# sourceMappingURL=InstructorInfoPanel.html.js.map