...
export function Navbar({ resLocals, navPage, navSubPage, navbarType, marginBottom = true, isInPageLayout = false, }) {
...
function NavbarByType({ resLocals, navPage, navSubPage, navbarType, isInPageLayout, }) {
...
function UserDropdownMenu({ resLocals, navPage, navbarType, }) {
    const { authz_data, authn_user, viewType, course_instance, urlPrefix, access_as_administrator, news_item_notification_count: newsCount, authn_is_administrator, } = resLocals;
    let displayedName;
    if (authz_data) {
...
function FlashMessages() {
    ...
    const flashMessages = run(() => {
        try {
            return flash(Object.keys(globalFlashColors));
        }
        catch {
            return [];
        }
    });
    return flashMessages.map(({ type, message }) => html `
      <div
        class="alert alert-${globalFlashColors[type]} border-start-0 border-end-0 rounded-0 mt-0 mb-0 alert-dismissible fade show"
        role="alert"
      >
        ${unsafeHtml(message)}
...
function ViewTypeMenu({ resLocals }) {
    const { viewType, course, course_instance, authz_data, assessment, question, assessment_instance, urlPrefix, } = resLocals;
    ...
    if (authz_data.authn_user.uid === authz_data.user.uid) {
        if (viewType === 'instructor') {
            if (authz_data.has_course_permission_preview ||
...
function AuthnOverrides({ resLocals, navbarType, }) {
    ...
    if (!authz_data?.authn_has_course_permission_preview &&
        !authz_data?.authn_has_course_instance_permission_view) {
        return '';
    }
        ...
        ? html `
          <div class="dropdown-item-text">
            <div class="list-group small text-nowrap">
              ${authz_data.overrides.map((override) => html `
...
function NavbarPlain({ resLocals, navPage }) {
...
function NavbarButton({ text, href, active = false, showArrow = true, }) {
    return html `
    <li class="nav-item">
...
function NavbarButtons({ resLocals, navPage, navbarType, }) {
    ...
    return html `
    ${allNavbarButtons.map((navbarButton, index) => NavbarButton({
...
function NavbarStudent({ resLocals, navPage, }) {
    const { course, course_instance, assessment_instance, assessment_instance_label, urlPrefix } = resLocals;
    return html `
...
function NavbarInstructor({ resLocals, navPage, navSubPage, }) {
    const { course, course_instance, assessment, assessment_label, assessments, navbarOpenIssueCount, navbarCompleteGettingStartedTasksCount, navbarTotalGettingStartedTasksCount, authz_data, urlPrefix, } = resLocals;
    return html `
...
function NavbarPublic({ resLocals }) {
...
function NavbarInstitution({ resLocals }) {
...
function NavbarAdministratorInstitution({ resLocals }) {
    const { institution } = resLocals;
    return html `
    <li class="nav-item">
...
//# sourceMappingURL=Navbar.html.js.map