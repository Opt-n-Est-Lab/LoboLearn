...
export function QuestionContainer({ resLocals, questionContext, questionRenderContext, showFooter = true, manualGradingPreviewUrl, aiGradingPreviewUrl, renderSubmissionSearchParams, }) {
    const { question, issues, variant, variantToken, questionJsonBase64, course_instance, authz_data, is_administrator, showTrueAnswer, submissions, submissionHtmls, answerHtml, } = resLocals;
    return html `
    <div
...
export function IssuePanel({ issue, course_instance, authz_data, is_administrator, }) {
    ...
    const showUserName = !issue.course_instance_id ||
        (course_instance &&
    ...
    const mailtoLink = `mailto:${issue.user_email || issue.user_uid || '-'}?subject=Reported%20PrairieLearn%20Issue&body=${encodeURIComponent(msgBody)}`;
    return html `
    <div class="card mb-3">
...
export function QuestionTitle({ questionContext, question, questionNumber, }) {
...
function QuestionFooter({ resLocals, questionContext, }) {
...
export function QuestionFooterContent({ resLocals, questionContext, }) {
    const { showSaveButton, showGradeButton, disableSaveButton, disableGradeButton, showNewVariantButton, showTryAgainButton, hasAttemptsOtherVariants, variantAttemptsLeft, variantAttemptsTotal, newVariantUrl, tryAgainUrl, question, variant, instance_question, assessment_question, instance_question_info, authz_result, group_config, group_info, group_role_permissions, user, } = resLocals;
    const contents = run(() => {
          ...
          <span class="d-flex align-items-center">
            ...
            ? html `
                  <button
                    ...
                    data-bs-content="Your group role (${getRoleNamesForUser(group_info, user).join(', ')}) is not allowed to submit this question."
                ...
                : hasAttemptsOtherVariants
                    ? html `
                      ...
                      <button
                        ...
                        data-bs-content="${escapeHtml(NewVariantInfo({ variantAttemptsLeft, variantAttemptsTotal }))}"
...
function SubmitRateFooter({ questionContext, showGradeButton, disableGradeButton, assessment_question, allowGradeLeftMs, }) {
    if (!showGradeButton || !assessment_question?.grade_rate_minutes)
        return '';
...
function NewVariantInfo({ variantAttemptsLeft, variantAttemptsTotal, }) {
    return html `
    <p>
...
function AvailablePointsNotes({ questionContext, instance_question, assessment_question, }) {
    if (questionContext !== 'student_exam' || !instance_question?.points_list)
        return '';
    const roundedPoints = instance_question.points_list.map((p) => Math.round(p * 100) / 100);
...
function QuestionPanel({ resLocals, questionContext, questionRenderContext, showFooter, manualGradingPreviewUrl, aiGradingPreviewUrl, }) {
    const { question, questionHtml, question_copy_targets, course, instance_question_info } = resLocals;
    ...
    const showCopyQuestionButton = question.type === 'Freeform' &&
...
function SubmissionList({ resLocals, questionContext, questionRenderContext, submissions, submissionHtmls, submissionCount, renderSubmissionSearchParams, }) {
    return submissions.map((submission, idx) => SubmissionPanel({
        questionContext,
        questionRenderContext,
...
function CopyQuestionModal({ resLocals }) {
    const { question_copy_targets, question, course } = resLocals;
    if (question_copy_targets == null)
        return '';
    return Modal({
        ...
        body: question_copy_targets.length === 0
            ? html `
            <p>
          ...
          `
            ...
            <select class="form-select" name="to_course_id" required>
              ${question_copy_targets.map((course, index) => html `
                  <option
                    value="${course.id}"
...
//# sourceMappingURL=QuestionContainer.html.js.map