import { escapeHtml, html, joinHtml } from '@prairielearn/html';
import { run } from '@prairielearn/run';
import { formatPoints, formatPointsOrList } from '../lib/format.js';
import { idsEqual } from '../lib/id.js';
export function QuestionScorePanel(props) {
...
export function QuestionScorePanelContent({ instance_question, assessment, assessment_question, question, assessment_instance, instance_question_info, variant, urlPrefix, }) {
    const hasAutoAndManualPoints = assessment_question.max_auto_points &&
      ...
      <tbody>
        ...
        : assessment_question.max_auto_points
            ? html `
                ...
                currentWeight: (instance_question.points_list_original?.[instance_question.number_attempts] ?? 0) - (assessment_question.max_manual_points ?? 0),
                pointsList: instance_question.points_list?.map((p) => p - (assessment_question.max_manual_points ?? 0)),
...
function IssueReportingPanel({ variant, csrfToken }) {
    return html `
    <button
...
export function ExamQuestionStatus({ instance_question, assessment_question, }) {
    ...
    if (instance_question.status === 'saved' &&
        !assessment_question.max_auto_points &&
        assessment_question.max_manual_points) {
        return html `
...
export function QuestionVariantHistory({ instanceQuestionId, previousVariants, currentVariantId, urlPrefix, }) {
    if (!previousVariants)
        return '';
    ...
    ${previousVariants.length > MAX_DISPLAYED_VARIANTS
        ...
        `
        : ''}
    ${previousVariants.map((variant, index) => html `
        <a
...
export function InstanceQuestionPoints({ instance_question, assessment_question, component, }) {
    const points = component === 'auto'
        ? instance_question.auto_points
        : component === 'manual'
            ? instance_question.manual_points
            : instance_question.points;
    const maxPoints = component === 'auto'
        ? assessment_question.max_auto_points
        : component === 'manual'
            ? assessment_question.max_manual_points
            : assessment_question.max_points;
    const pointsPending = (['saved', 'grading'].includes(instance_question.status ?? '') && component !== 'manual') ||
...
export function ExamQuestionAvailablePoints({ open, pointsList, highestSubmissionScore, currentWeight, }) {
...
function QuestionValue({ instance_question, assessment_question, }) {
    const initAutoPoints = (assessment_question.init_points ?? 0) - (assessment_question.max_manual_points ?? 0);
    const currentAutoValue = (instance_question.current_value ?? 0) - (assessment_question.max_manual_points ?? 0);
    const bestCurrentScore = run(() => {
        const variantPoints = instance_question.variants_points_list.at(-1);
        if (variantPoints == null)
            return 0;
        if (variantPoints < initAutoPoints)
            return (variantPoints / initAutoPoints) * 100;
        return 0;
    });
    const popoverContent = run(() => {
      ...
      `);
        ...
        `);
                ...
                parts.push(html `<p>Your highest submission score so far is ${formatPoints(bestCurrentScore)}%.</p>`);
            }
...
//# sourceMappingURL=QuestionScore.html.js.map