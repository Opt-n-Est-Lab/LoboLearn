...
import {} from '../models/questions.js';
...
export function QuestionsTableHead() {
    ...
    <script src="${nodeModulesAssetPath('bootstrap-table/dist/extensions/filter-control/bootstrap-table-filter-control.min.js')}"></script>
...
export function QuestionsTable({ questions, templateQuestions = [], showAddQuestionButton = false, showAiGenerateQuestionButton = false, showSharingSets = false, current_course_instance, course_instances = [], qidPrefix, urlPrefix, plainUrlPrefix, __csrf_token, }) {
      ...
      ${questions.length > 0
        ? html `
            ...
            : ''}
                  ...
                  ${(course_instances || []).map((course_instance) => html ` <th
...
function CreateQuestionModal({ csrfToken, templateQuestions, }) {
    return Modal({
      ...
      <div id="templateContainer" class="mb-3" hidden>
        ...
        <select
          ...
          disabled
        >
          ${templateQuestions.map((question) => html `<option value="${question.qid}">${question.title}</option>`)}
        </select>
...
//# sourceMappingURL=QuestionsTable.html.js.map