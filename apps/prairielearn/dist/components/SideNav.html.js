    ...
    course_admin: [
        {
            ...
            htmlSuffix: ({ navbarCompleteGettingStartedTasksCount, navbarTotalGettingStartedTasksCount, }) => ProgressCircle({
                value: navbarCompleteGettingStartedTasksCount,
                maxValue: navbarTotalGettingStartedTasksCount,
                className: 'ms-auto',
            }),
            renderCondition: ({ authz_data, course }) => authz_data.has_course_permission_edit && course.show_getting_started,
        },
        {
            activePages: ['course_admin'],
        ...
        },
        {
            ...
            tabLabel: 'Issues',
            htmlSuffix: ({ navbarOpenIssueCount }) => IssueBadge({ count: navbarOpenIssueCount, suppressLink: true, className: 'ms-auto' }),
        },
        {
            activePages: ['course_admin'],
...
export function SideNav({ resLocals, page, subPage, }) {
    return html `
    ${CourseNav({
        resLocals,
        page,
...
function CourseNav({ resLocals, page, subPage, }) {
    ...
    <div class="side-nav-header">
      ...
      >
        <i
          ...
          class="${clsx('bi', resLocals.side_nav_expanded ? 'bi-arrow-bar-left' : 'bi-arrow-bar-right')}"
        ></i>
      </button>
    </div>
    <div class="side-nav-group">
      ...
      ${courseSideNavPageTabs.map((tabInfo) => SideNavLink({
        resLocals,
        navPage: page,
        navSubPage: subPage,
        tabInfo,
...
function CourseInstanceNav({ resLocals, page, subPage, }) {
    ...
    <div class="side-nav-group mb-3">
      <div>
        ...
        ${resLocals.course_instance
        ? courseInstanceSideNavPageTabs.map((tabInfo) => SideNavLink({
            resLocals,
            navPage: page,
            navSubPage: subPage,
            tabInfo,
...
function SideNavLink({ resLocals, navPage, navSubPage, tabInfo, }) {
    const { urlPrefix } = resLocals;
    const { activePages, checkActiveSubPageForPages, activeSubPages, iconClasses, tabLabel, htmlSuffix, renderCondition, } = tabInfo;
...
//# sourceMappingURL=SideNav.html.js.map