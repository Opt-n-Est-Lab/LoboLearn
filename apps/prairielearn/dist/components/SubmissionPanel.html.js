...
import { GradingJobSchema, SubmissionSchema, } from '../lib/db-types.js';
...
export function SubmissionPanel({ questionContext, questionRenderContext, question, assessment_question, instance_question, variant_id, course_instance_id, submission, submissionHtml, submissionCount, rubric_data, urlPrefix, expanded, renderSubmissionSearchParams, }) {
    const isLatestSubmission = submission.submission_number === submissionCount;
    expanded = expanded || isLatestSubmission;
    const renderUrlPrefix = questionContext === 'instructor' || questionContext === 'public'
      ...
      ${submission.feedback?.manual || submission.rubric_grading
        ? html `
            ...
            ? html `
                ...
                .filter((item) => item.always_show_to_students ||
                submission.rubric_grading?.rubric_items?.[item.id]?.score)
                .map((item) => RubricItem({
                item,
                item_grading: submission.rubric_grading?.rubric_items?.[item.id] ?? null,
            }))}
...
function SubmissionStatusBadge({ submission, question, isLatestSubmission, assessment_question, instance_question, }) {
    let manualGradingBadge = null;
    let autoGradingBadge = null;
    if (assessment_question && instance_question
...
function SubmissionInfoModal({ urlPrefix, submission, question, course_instance_id, }) {
...
function RubricItem({ item, item_grading, }) {
    return html `
    <div>
...
//# sourceMappingURL=SubmissionPanel.html.js.map