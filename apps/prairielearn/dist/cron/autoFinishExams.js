...
export async function run() {
    const assessment_instances = await sqldb.queryRows(sql.select_assessments_to_auto_close, { age_minutes: config.autoFinishAgeMins }, AssessmentInstanceSchema.pick({ id: true, open: true, assessment_id: true }));
    for (const assessment_instance of assessment_instances) {
        ...
        try {
            await gradeAssessmentInstance(assessment_instance.id, user_id, authn_user_id, requireOpen, close, overrideGradeRate, clientFingerprintId);
        }
        catch (err) {
...
//# sourceMappingURL=autoFinishExams.js.map