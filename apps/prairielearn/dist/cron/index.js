...
export async function init() {
    ...
    jobs = [
        ...
        },
        {
            ...
            intervalSec: config.cronOverrideAllIntervalsSec ||
    ...
    if (isEnterprise()) {
        ...
        jobs.push({
            ...
            intervalSec: config.cronOverrideAllIntervalsSec || config.cronIntervalChunksHostAutoScalingSec,
        });
    }
    ...
    logger.verbose('initializing cron', jobs.map(({ name, intervalSec }) => ({ name, intervalSec })));
...
async function tryJobWithLock(job, cronUuid) {
    ...
    const didLock = await namedLocks.doWithLock(lockName, { onNotAcquired: () => false }, async () => {
