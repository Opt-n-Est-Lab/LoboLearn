...
async function checkDBConsistency() {
    ...
    const nonTerminatedHosts = new Set(await queryRows(sql.select_nonterminated_workspace_hosts, z.string()));
    ...
    if (hostsNotInEc2.size > 0) {
        ...
        const stoppedWorkspaces = await workspaceHostUtils.terminateWorkspaceHostsIfNotLaunching(Array.from(hostsNotInEc2));
...
async function terminateHosts() {
    const ec2 = new EC2(makeAwsClientConfig());
    const hosts = await workspaceHostUtils.findTerminableWorkspaceHosts(config.workspaceHostUnhealthyTimeoutSec, config.workspaceHostLaunchTimeoutSec);
    if (hosts.length > 0) {
...
async function checkHealth() {
    const hosts = await queryRows(sql.select_healthy_hosts, z.object({
        id: z.string(),
...
//# sourceMappingURL=workspaceHostTransitions.js.map