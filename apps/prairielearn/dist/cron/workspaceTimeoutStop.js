...
async function stopLaunchedTimeoutWorkspaces() {
    ...
    for (const workspace of result.rows) {
        ...
        await workspaceUtils.updateWorkspaceState(workspace.id, 'stopped', `Maximum run time of ${Math.round(config.workspaceLaunchedTimeoutSec / 3600)} hours exceeded. Click "Reboot" to keep working.`);
...
async function stopHeartbeatTimeoutWorkspaces() {
    ...
    for (const workspace of result.rows) {
        ...
        await workspaceUtils.updateWorkspaceState(workspace.id, 'stopped', `Connection was lost for more than ${Math.round(config.workspaceHeartbeatTimeoutSec / 60)} min. Click "Reboot" to keep working.`);
...
async function stopInLaunchingTimeoutWorkspaces() {
    ...
    for (const workspace of workspaces) {
        ...
        await workspaceUtils.updateWorkspaceState(workspace.id, 'stopped', `Maximum launching time of ${Math.round(config.workspaceInLaunchingTimeoutSec / 60)} min exceeded. Click "Reboot" to keep working.`);
...
//# sourceMappingURL=workspaceTimeoutStop.js.map