...
router.post('/callback', asyncHandler(async (req, res) => {
    ...
    if (!lti13_instance.uid_attribute) {
        throw new HttpStatusError(500, 'LTI 1.3 instance configuration missing required UID attribute');
    }
    else {
        ...
        if (!uid && !inStateTest) {
            ...
            if (ltiClaim.isRoleTestUser()) {
                throw new HttpStatusError(403, 'Student View / Test user not supported. Use access modes within PrairieLearn to view as a student.');
            }
            else {
                // Error about missing UID
                throw new HttpStatusError(500, `Missing UID data from LTI 1.3 login (claim ${lti13_instance.uid_attribute} missing or empty)`);
            }
        }
    }
    ...
    if (lti13_instance.uin_attribute) {
        ...
        if (!uin && !inStateTest) {
            throw new HttpStatusError(500, `Missing UIN data from LTI 1.3 login (claim ${lti13_instance.uin_attribute} missing or empty)`);
        }
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        myPassport.authenticate('lti13', ((err, user, info) => {
            if (err) {
                ...
                reject(new AugmentedError('Authentication failed, before user validation.', {
                    status: 400,
                    data: {
                        info_raw: info,
                        info: info?.toString(),
...
async function launchFlow(req, res, next) {
    // https://www.imsglobal.org/spec/security/v1p0/#step-1-third-party-initiated-login
...
async function setupPassport(lti13_instance_id) {
    ...
    localPassport.use('lti13', new Strategy({
        client,
        passReqToCallback: true,
    }, callbackify(verify)));
    return localPassport;
}
async function verify(req, tokenSet) {
    ...
    const params = {
        ...
        tool_platform_name: lti13_claims['https://purl.imsglobal.org/spec/lti/claim/tool_platform']?.name ?? null,
    };
    await queryAsync(sql.verify_upsert, params);
    return lti13_claims;
}
//# sourceMappingURL=lti13Auth.js.map