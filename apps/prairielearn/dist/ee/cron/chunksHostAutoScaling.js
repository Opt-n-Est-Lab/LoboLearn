...
export async function run() {
    ...
    const pageViewsPerSecondMetric = metrics.MetricDataResults?.find((m) => m.Id === PAGE_VIEWS_PER_SECOND);
    const activeWorkersPerSecondMetric = metrics.MetricDataResults?.find((m) => m.Id === ACTIVE_WORKERS_PER_SECOND);
    const loadBalancerRequestsPerMinuteMetric = metrics.MetricDataResults?.find((m) => m.Id === LOAD_BALANCER_REQUESTS_PER_MINUTE);
    ...
    const maxLoadBalancerRequestsPerMinute = Math.max(...(loadBalancerRequestsPerMinuteMetric?.Values ?? []), 0);
    const desiredInstancesByPageViews = maxPageViewsPerSecond / config.chunksPageViewsCapacityFactor;
    const desiredInstancesByActiveWorkers = maxActiveWorkersPerSecond / config.chunksActiveWorkersCapacityFactor;
    const desiredInstancesByLoadBalancerRequests = maxLoadBalancerRequestsPerMinute / config.chunksLoadBalancerRequestsCapacityFactor;
    const desiredInstances = Math.ceil(Math.max(desiredInstancesByPageViews, desiredInstancesByActiveWorkers, desiredInstancesByLoadBalancerRequests, 1));
...
//# sourceMappingURL=chunksHostAutoScaling.js.map