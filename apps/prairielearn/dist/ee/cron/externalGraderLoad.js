...
async function setAutoScalingGroupCapacity(stats) {
    if (!config.externalGradingAutoScalingGroupName)
        return;
    if (!Number.isInteger(stats.desired_instances))
        return;
    if (stats.desired_instances < 1 || stats.desired_instances > 1e6)
        return;
    if (stats.desired_instances === stats.instance_count)
        return;
...
//# sourceMappingURL=externalGraderLoad.js.map