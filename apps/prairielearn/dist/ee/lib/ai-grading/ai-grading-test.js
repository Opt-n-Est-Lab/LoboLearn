...
import { IdSchema, InstanceQuestionSchema, RubricItemSchema, SubmissionGradingContextEmbeddingSchema, } from '../../../lib/db-types.js';
...
export async function aiGradeTest({ course, course_instance_id, question, assessment_question, urlPrefix, authn_user_id, user_id, }) {
    ...
    serverJob.executeInBackground(async (job) => {
        ...
        for (const instance_question of instance_questions) {
            ...
            const { variant, submission } = await queryRow(sql.select_last_variant_and_submission, { instance_question_id: instance_question.id }, aiGradingUtil.SubmissionVariantSchema);
            const grading_rubric_id = await queryOptionalRow(sql.select_rubric_id_from_grading, { manual_rubric_grading_id: submission.manual_rubric_grading_id }, IdSchema);
            if (rubric_id !== grading_rubric_id) {
                job.info('Rubric used for this submission does not match the current rubric in use. Skipping test.');
                continue;
            }
            const locals = {
            ...
            if (rubric_id) {
                const rubric_grading_items = await queryRows(sql.select_rubric_grading_items, { manual_rubric_grading_id: submission.manual_rubric_grading_id }, RubricItemSchema);
                const referenceRubricDescriptions = new Set();
                ...
                try {
                    ...
                    if (response.parsed) {
                        const { appliedRubricItems, appliedRubricDescription } = aiGradingUtil.parseAiRubricItems({
                        ...
                        await runInTransactionAsync(async () => {
                            ...
                            const manual_rubric_grading = await manualGrading.insertRubricGrading(rubric_id, assessment_question.max_points ?? 0, assessment_question.max_manual_points, appliedRubricItems, 0);
                            const score = manual_rubric_grading.computed_points / assessment_question.max_manual_points;
                            const grading_job_id = await queryRow(sql.insert_grading_job, {
                                submission_id: submission.id,
        ...
        if (error_count > 0) {
            ...
            if (rubric_id) {
                ...
                const rmse = aiGradingUtil.rootMeanSquaredError(testScoreResults.map((item) => item.reference_score), testScoreResults.map((item) => item.ai_score));
                job.info(`RMSE: ${rmse}`);
                const r = aiGradingUtil.pearsonCorrelation(testScoreResults.map((item) => item.reference_score), testScoreResults.map((item) => item.ai_score));
...
//# sourceMappingURL=ai-grading-test.js.map