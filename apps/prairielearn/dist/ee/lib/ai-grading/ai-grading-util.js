...
import { RubricItemSchema, SubmissionGradingContextEmbeddingSchema, SubmissionSchema, VariantSchema, } from '../../../lib/db-types.js';
import { buildQuestionUrls } from '../../../lib/question-render.js';
import { getQuestionCourse } from '../../../lib/question-variant.js';
import * as questionServers from '../../../question-servers/index.js';
import { createEmbedding, vectorToString } from '../contextEmbeddings.js';
const sql = loadSqlEquiv(import.meta.url);
export const OPEN_AI_MODEL = 'gpt-4o-2024-11-20';
...
export function calculateApiCost(usage) {
    if (!usage) {
        return 0;
    }
...
export async function generatePrompt({ questionPrompt, submission_text, example_submissions, rubric_items, }) {
    const messages = [];
...
export async function generateSubmissionEmbedding({ course, question, instance_question, urlPrefix, openai, }) {
...
export function parseAiRubricItems({ ai_rubric_items, rubric_items, }) {
...
export function rubricItemAccuracy(testRubricResults, item) {
    let match = 0;
    testRubricResults.forEach((test) => {
        if ((test.ai_items.has(item.description) && test.reference_items.has(item.description)) ||
...
//# sourceMappingURL=ai-grading-util.js.map