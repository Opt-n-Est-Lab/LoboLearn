...
const BENCHMARKS = [
    ...
    {
        prompt: 'Write a question that asks the user to multiply two integers. You should randomly generate two integers A and B, display them to the user, and then ask the user to provide the product C = A * B. Provide an integer input box for the user to enter the product. The correct answer is the product of A and B.',
    },
    {
        prompt: 'Write a question that asks the user to calculate the area of a rectangle given fixed dimensions. Provide a numeric input box for the user to enter the area. The correct answer is the product of the rectangle dimensions.',
    },
    {
    ...
    {
        prompt: 'Write a question that asks the user to calculate the greatest common divisor (GCD) of two integers A and B. Randomly generate integers A and B. Provide an integer input box for the user to enter the GCD. The correct answer is the GCD of the two randomly generated integers A and B.',
    },
    {
        prompt: 'Write a question that asks the user to select all prime numbers from a given list of integers. Randomly generate the list to include both primes and non-primes. Provide a checkbox input for the user to select multiple answers. The correct answers are all prime numbers in the generated list.',
    },
    {
        prompt: 'Write a question that asks the user to convert a randomly generated angle from radians to degrees. Provide a numerical input box for the user to enter the degree equivalent. The correct answer is the degree equivalent of the given angle in radians.',
    },
    {
        prompt: 'Generate a random polynomial of the form ax + b = c. Ask the student to solve for x. Provide a numerical input box for the user to enter the solution. The correct answer is the solution for x (x = (c - b) / a).',
    },
    {
    ...
    {
        prompt: 'Write a question that asks the user to calculate the pressure of a gas using the ideal gas law PV = nRT. Randomly generate values for n, V, T, and R. Provide a numeric input box for the user to enter the pressure in atmospheres. The correct answer is the pressure calculated using the ideal gas law.',
    },
    {
    ...
    {
        prompt: 'sum two randomly generated numbers from 0 to 100, each with two decimal digits. students should input their solution as decimal number, and correctness should be checked using a relative tolerance of 1e-3. the correct answer is the summation of the two numbers provided in the question prompt',
    },
    {
    ...
    {
        prompt: 'given a specimen subjected to an applied deformation, initial length is x0 final length is xf, elastic modulus is E, find the stress induced in the material. use units of Newtons and meters. student input should be a float that represents the stress in MPa. correct answer is E*(xf-x0)/x0',
    },
    {
        prompt: 'given a mass (M) on a spring of stiffness (K) with an initial position (x), velocity (xdot) and acceleration (xddot) write a symbolic expression for the kinetic and potential energy of the system. student input should be symbolic expressions. kinetic energy is 1/2*m*xdot^2. potential energy is 1/2*k*x^2',
    },
    {
    ...
    {
        prompt: 'Given positive integers a and b, find the price P given demand D. a and b should be positive integers, and D should also be a positive integer. Students should input the answer, labeled P, as a decimal. The answer input box should have units of "units". the correct answer is P = (a-D)/b',
    },
    {
    ...
    {
        prompt: 'A car is accelerating forward with acceleration a (ranging from 2 to 5, with precision .1), with initial velocity and position = 0. Find the position and velocity of the car after elapsed time t (integer from 2 to 20). 2 numerical input boxes. v = v0 + a*t, x = x0 + v0*t + 1/2*a*t^2',
    },
    {
        prompt: 'Find the population of rabbits given an exponential growth model. Generate a value of alpha (scalar integer), N in years, Xi = initial population, and Xf = population after N years. Find the population after alpha*N years. numerical input box. Y = Xi*(Xf/Xi)^alpha',
    },
    {
        prompt: 'A toy car is pushed off a table with height h at speed v0. Assume acceleration due to gravity as 9.81 m/s^2. H is a number with 1 decimal digit selected at random between 1 and 2 meters. V0 is a an integer between 1 and 4 m/s. How long does it take for the car to reach the ground?. students should enter the solution using a decimal number. The answer should be in seconds. the answer is computed as sqrt(2 * h / g) where g = 9.81 m/s^2',
    },
...
export async function benchmarkAiQuestionGeneration({ client, authnUserId, }) {
    ...
    serverJob.executeInBackground(async (job) => {
        ...
        for (const benchmark of BENCHMARKS) {
            ...
            const prompts = await queryRows(sql.select_ai_question_generation_prompts, { question_id: result.questionId }, AiQuestionGenerationPromptSchema);
...
async function evaluateGeneratedQuestion({ client, authnUserId, originalSystemPrompt, userPrompt, html, python, }) {
    const systemPrompt = [
        ...
        .flat()
        .join('\n');
    const generatedQuestion = ['```html', html.trim(), '```'];
    if (python) {
    ...
    const completion = await client.beta.chat.completions.parse({
        ...
        response_format: zodResponseFormat(QuestionGenerationEvaluationSchema, 'question_generation_evaluation'),
        user: openAiUserFromAuthn(authnUserId),
    });
    return completion.choices[0].message.parsed;
}
//# sourceMappingURL=ai-question-generation-benchmark.js.map