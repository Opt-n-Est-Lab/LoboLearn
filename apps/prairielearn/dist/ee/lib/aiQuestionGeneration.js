 ...
 */
function promptPreamble(context) {
    return `# Introduction
...
async function checkRender(status, errors, courseId, userId, questionId) {
    ...
    return locals.issues
        .map((issue) => issue.system_data?.courseErrData?.outputBoth)
 ...
 */
export async function makeContext(client, prompt, mandatoryElementNames, authnUserId) {
    ...
    const mandatoryElements = mandatoryElementNames.length > 0
        ? await queryRows(sql.select_documents_by_chunk_id, {
    ...
    const docs = await queryRows(sql.select_nearby_documents, { embedding: vectorToString(embedding), limit: numAdditionalDocs }, QuestionGenerationContextEmbeddingSchema);
    // Ensure that documentation for at least one element is always included.
    const elementDoc = await queryRow(sql.select_nearby_documents_from_file, {
 ...
 */
function extractFromCompletion(completion, job) {
 ...
 */
export async function generateQuestion({ client, courseId, authnUserId, prompt, userId, hasCoursePermissionEdit, }) {
...
Keep in mind you are not just generating an example; you are generating an actual question that the user will use directly.`;
        ...
        errors.push(...(await checkRender(saveResults.status, errors, courseId, userId, saveResults.question_id)));
 ...
 */
function traverseForTagNames(ast) {
    const nodeNames = new Set([ast.nodeName]);
    if (ast.childNodes) {
 ...
 * @param hasCoursePermissionEdit Whether the saving generating/saving has course permission edit privlidges.
 */
async function regenInternal({ job, client, authnUserId, originalPrompt, revisionPrompt, originalHTML, originalPython, remainingAttempts, isAutomated, questionId, questionQid, courseId, userId, hasCoursePermissionEdit, jobSequenceId, }) {
 ...
 * @returns A server job ID for the generation task and a promise to return the associated saved data on completion.
 */
export async function regenerateQuestion(client, courseId, authnUserId, originalPrompt, revisionPrompt, originalHTML, originalPython, questionQid, userId, hasCoursePermissionEdit) {
    const serverJob = await createServerJob({
        courseId,
...
//# sourceMappingURL=aiQuestionGeneration.js.map