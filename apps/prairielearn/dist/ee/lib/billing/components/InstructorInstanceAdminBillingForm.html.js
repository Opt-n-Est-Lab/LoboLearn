...
import { planGrantsMatchPlanFeatures } from '../plans-types.js';
export function instructorInstanceAdminBillingState(input) {
    ...
    const computeEnabledByCourseInstance = planGrantsMatchPlanFeatures(input.courseInstancePlanGrants, ['compute']);
    const computeEnabled = (!studentBillingEnabled && (computeEnabledByInstitution || computeEnabledByCourseInstance)) ||
        input.desiredRequiredPlans.includes('compute');
    let studentBillingCanChange = input.editable;
    const studentBillingDidChange = input.initialRequiredPlans.includes('basic') !== input.desiredRequiredPlans.includes('basic');
    let studentBillingAlert = null;
    ...
    const computeDidChange = input.initialRequiredPlans.includes('compute') !==
        input.desiredRequiredPlans.includes('compute');
    let computeAlert = null;
    if (!studentBillingEnabled && (computeEnabledByInstitution || computeEnabledByCourseInstance)) {
        computeCanChange = false;
        computeAlert = {
            message: 'This course instance already has access to compute features without additional payment.',
            color: 'success',
        };
    }
    return {
...
export function InstructorInstanceAdminBillingForm(props) {
    const { enrollmentCount, enrollmentLimit, enrollmentLimitSource, externalGradingQuestionCount, workspaceQuestionCount, editable, csrfToken, } = props;
    const { studentBillingEnabled, studentBillingCanChange, studentBillingAlert, computeEnabled, computeCanChange, computeAlert, } = instructorInstanceAdminBillingState(props);
...
function MaybeAlert(props) {
    if (!props)
        return null;
    return html `<div class="alert alert-${props.color}">${props.message}</div>`;
}
function enrollmentLimitExplanation({ studentBillingEnabled, enrollmentLimit, enrollmentLimitSource, }) {
...
function pluralizeQuestionCount(count) {
...
//# sourceMappingURL=InstructorInstanceAdminBillingForm.html.js.map