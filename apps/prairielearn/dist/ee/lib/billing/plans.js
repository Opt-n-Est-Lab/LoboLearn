...
import { InstitutionSchema, PlanGrantSchema, } from '../../../lib/db-types.js';
import {} from '../../../lib/types.js';
import { deleteCourseInstanceRequiredPlan, insertCourseInstanceRequiredPlan, } from '../../models/course-instance-required-plans.js';
import { deletePlanGrant, ensurePlanGrant, updatePlanGrant } from '../../models/plan-grants.js';
import { PLAN_NAMES, getFeaturesForPlans, } from './plans-types.js';
 ...
 */
export async function getPlanGrantsForContext(context) {
    return await queryRows(sql.select_plan_grants_for_context, {
 ...
 */
export async function getPlanGrantsForPartialContexts(context) {
    return await queryRows(sql.select_plan_grants_for_partial_contexts, {
        institution_id: context.institution_id ?? null,
        course_instance_id: context.course_instance_id ?? null,
        user_id: context.user_id ?? null,
    }, PlanGrantSchema);
}
export async function getPlanGrantsForCourseInstance({ institution_id, course_instance_id, }) {
    return await getPlanGrantsForContext({ institution_id, course_instance_id });
}
export function getPlanNamesFromPlanGrants(planGrants) {
    const planNames = new Set();
    planGrants.forEach((planGrant) => planNames.add(planGrant.plan_name));
    return Array.from(planNames);
}
export async function getRequiredPlansForCourseInstance(course_instance_id) {
    return await queryRows(sql.select_required_plans_for_course_instance, { course_instance_id }, z.enum(PLAN_NAMES));
}
export async function updateRequiredPlansForCourseInstance(course_instance_id, plans, authn_user_id) {
...
export async function reconcilePlanGrantsForInstitution(institution_id, plans, authn_user_id) {
    await runInTransactionAsync(async () => {
...
export async function reconcilePlanGrantsForCourseInstance(course_instance_id, plans, authn_user_id) {
    await runInTransactionAsync(async () => {
        ...
        await reconcilePlanGrants({
            institution_id: institution.id,
            course_instance_id,
        }, existingPlanGrants, plans, authn_user_id);
    });
}
export async function reconcilePlanGrantsForCourseInstanceUser(context, plans, authn_user_id) {
    await runInTransactionAsync(async () => {
...
async function reconcilePlanGrants(context, existingPlanGrants, desiredPlans, authn_user_id) {
    const newPlans = desiredPlans.filter((plan) => !existingPlanGrants.find((p) => p.plan_name === plan.plan));
    const updatedPlanGrants = existingPlanGrants.map((planGrant) => ({
        planGrant,
        newType: desiredPlans.find((p) => p.plan === planGrant.plan_name)?.grantType,
    }));
    const deletedPlanGrants = existingPlanGrants.filter((plan) => !desiredPlans.find((p) => p.plan === plan.plan_name));
...
export function planGrantsSatisfyRequiredPlans(planGrants, requiredPlans) {
...
async function getInstitutionForCourseInstance(course_instance_id) {
    return await queryRow(sql.select_institution_for_course_instance, { course_instance_id }, InstitutionSchema);
}
function getPlansForPlanGrants(planGrants) {
    const plans = new Set();
 ...
 */
export function getMissingPlanGrants(existingPlanGrants, requiredPlans) {
