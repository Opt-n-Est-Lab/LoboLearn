...
import { getPlanGrantsForContext, getPlanGrantsForCourseInstance, getPlanGrantsForPartialContexts, getPlanNamesFromPlanGrants, getRequiredPlansForCourseInstance, reconcilePlanGrantsForCourseInstance, reconcilePlanGrantsForInstitution, updateRequiredPlansForCourseInstance, } from './plans.js';
describe('plans', () => {
    ...
    describe('reconcilePlanGrantsForInstitution', () => {
        it('persists updates', async () => {
            await helperDb.runInTransactionAndRollback(async () => {
                await reconcilePlanGrantsForInstitution('1', [{ plan: 'compute', grantType: 'invoice' }], '1');
    ...
    describe('reconcilePlanGrantsForCourseInstance', () => {
        it('persists updates', async () => {
            await helperDb.runInTransactionAndRollback(async () => {
                await reconcilePlanGrantsForCourseInstance('1', [{ plan: 'compute', grantType: 'invoice' }], '1');
                let planGrants = await getPlanGrantsForCourseInstance({
                    institution_id: '1',
                    course_instance_id: '1',
                });
                assert.isOk(planGrants.find((pg) => pg.plan_name === 'compute' && pg.type === 'invoice'));
                await reconcilePlanGrantsForCourseInstance('1', [{ plan: 'compute', grantType: 'gift' }], '1');
