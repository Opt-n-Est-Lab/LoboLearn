...
function extractElementSections(ast) {
    ...
    while (startIndex !== -1) {
        ...
        let endIndex = ast.children.findIndex((child, index) => {
            return (index > startIndex && child.type === 'heading' && (child.depth === 2 || child.depth === 3));
        });
...
function removeHeadingAndContent(headingName, contents) {
...
function cleanElementSections(elementSections) {
...
function writeOutTables(elementSections) {
    elementSections.forEach((section) => {
        section.content.forEach((node) => {
            if (node.type === 'table') {
                const firstRow = node.children[0];
                if (firstRow.children.length === 4 &&
                    ...
                    for (let rowIdx = 1; rowIdx < node.children.length; rowIdx++) {
                        ...
                        if (defaultValProcessed !== '-' && defaultValProcessed !== '—') {
                            statements.push(attribute, { type: 'text', value: ': of type ' }, type, { type: 'text', value: ', default val: ' }, defaultVal, { type: 'text', value: ', description: ' }, description);
                        }
                        else {
                            statements.push(attribute, { type: 'text', value: ': of type ' }, type, { type: 'text', value: ', description: ' }, description);
                        }
...
export async function buildContextForElementDocs(rawMarkdown) {
