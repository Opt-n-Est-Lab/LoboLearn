...
import { buildContextForElementDocs } from './context-parsers/documentation.js';
 ...
 */
export function openAiUserFromAuthn(authnUserId) {
 ...
 */
export async function createEmbedding(client, text, openAiUser) {
 ...
 */
async function insertDocumentChunk(client, filepath, doc, job, openAiUser) {
    const chunk = await queryOptionalRow(sql.check_doc_chunk_exists, { doc_path: filepath, chunk_id: doc.chunkId }, QuestionGenerationContextEmbeddingSchema);
    ...
    await queryRows(sql.insert_embedding, {
        doc_path: filepath,
        doc_text: doc.text,
 ...
 */
export async function syncContextDocuments(client, authnUserId) {
    const serverJob = await createServerJob({
        type: 'sync_question_generation_context',
        description: 'Generate embeddings for context documents',
    });
    serverJob.executeInBackground(async (job) => {
        const templateQuestionsPath = path.join(REPOSITORY_ROOT_PATH, 'exampleCourse/questions/template');
        ...
        for await (const file of klaw(templateQuestionsPath)) {
            ...
            if (fileText) {
                await insertDocumentChunk(client, path.relative(REPOSITORY_ROOT_PATH, file.path), { text: fileText, chunkId: '' }, job, openAiUserFromAuthn(authnUserId));
        ...
        for (const doc of files) {
            await insertDocumentChunk(client, path.relative(REPOSITORY_ROOT_PATH, elementDocsPath), doc, job, openAiUserFromAuthn(authnUserId));
        }
        await queryRows(sql.delete_unused_doc_chunks, {
...
//# sourceMappingURL=contextEmbeddings.js.map