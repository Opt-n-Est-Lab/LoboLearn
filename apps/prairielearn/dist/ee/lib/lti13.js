...
import { AssessmentInstanceSchema, AssessmentSchema, DateFromISOString, Lti13CourseInstanceSchema, Lti13InstanceSchema, UserSchema, } from '../../lib/db-types.js';
...
export class Lti13Claim {
    ...
    assertValid() {
        if (!this.valid || Math.floor(Date.now() / 1000) > this.claims.exp) {
            ...
            throw new HttpStatusError(403, 'LTI session invalid or timed out, please try logging in again.');
        }
    }
    isRoleTestUser() {
        this.assertValid();
    ...
    isRoleInstructor() {
        ...
        */
        let role_instructor = this.roles.some((val) => [
            'http://purl.imsglobal.org/vocab/lis/v2/membership#Instructor',
            'http://purl.imsglobal.org/vocab/lis/v2/membership#ContentDeveloper',
        ].includes(val));
        // TA roles may also have Instructor roles, so check this next. We don't
        // currently consider TAs to be instructors.
        if (this.roles.includes('http://purl.imsglobal.org/vocab/lis/v2/membership/Instructor#TeachingAssistant')) {
...
export async function validateLti13CourseInstance(resLocals) {
    const feature_enabled = await features.enabledFromLocals('lti13', resLocals);
    if (!feature_enabled) {
        return false;
    }
    const hasLti13CourseInstance = await queryRow(sql.select_ci_validation, {
...
export async function getAccessToken(lti13_instance_id) {
    const lti13_instance = await selectLti13Instance(lti13_instance_id);
    let tokenSet = lti13_instance.access_tokenset;
    const fiveMinutesInTheFuture = new Date(Date.now() + 5 * 60 * 1000);
    if (lti13_instance.access_token_expires_at &&
...
export async function getLineitems(instance) {
...
export async function getLineitem(instance, lineitem_id_url) {
    const token = await getAccessToken(instance.lti13_instance.id);
...
export async function syncLineitems(instance, job) {
    job.info(`Polling for external assignments from ${instance.lti13_instance.name} ${instance.lti13_course_instance.context_label}`);
    ...
    await runInTransactionAsync(async () => {
        const output = await queryRow(sql.sync_lti13_assessments, {
            lti13_course_instance_id: instance.lti13_course_instance.id,
...
export async function createAndLinkLineitem(instance, job, assessment) {
    if (instance.lti13_course_instance.lineitems_url == null) {
        throw new HttpStatusError(400, 'Lineitems not defined');
    }
    const createBody = {
    ...
    job.info(`Creating assignment for ${assessment.label} in ${instance.lti13_instance.name} ${instance.lti13_course_instance.context_label}`);
    const token = await getAccessToken(instance.lti13_instance.id);
...
export async function queryAndLinkLineitem(instance, lineitem_id_url, unsafe_assessment_id) {
...
export async function unlinkAssessment(lti13_course_instance_id, assessment_id) {
    await queryAsync(sql.delete_lti13_assessment, {
        lti13_course_instance_id,
        assessment_id,
    });
}
export async function linkAssessment(lti13_course_instance_id, unsafe_assessment_id, lineitem) {
    const assessment = await queryRow(sql.select_assessment_in_lti13_course_instance, {
 ...
 */
export async function fetchRetry(input, opts, incomingfetchRetryOpts) {
 ...
 */
export async function fetchRetryPaginated(input, opts, incomingfetchRetryOpts) {
    const output = [];
...
class Lti13ContextMembership {
    ...
    static async loadForInstance({ lti13_instance, lti13_course_instance, }) {
        if (lti13_course_instance.context_memberships_url === null) {
            throw new HttpStatusError(403, 'LTI 1.3 course instance context_memberships_url not configured');
        }
        ...
        const filteredMemberships = ltiMemberships.filter((member) => {
            // Skip invalid cases
...
export async function updateLti13Scores(unsafe_assessment_id, instance, job) {
    // Get the assessment metadata
    const assessment = await queryRow(sql.select_assessment_for_lti13_scores, {
        unsafe_assessment_id,
        lti13_course_instance_id: instance.lti13_course_instance.id,
    ...
    const token = await getAccessToken(instance.lti13_instance.id);
    const assessment_instances = await queryRows(sql.select_assessment_instances_for_scores, {
        assessment_id: assessment.id,
