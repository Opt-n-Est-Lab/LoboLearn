...
function hasUserAcceptedTerms(user) {
 ...
 */
export async function shouldRedirectToTermsPage(user, ip) {
    if (!config.requireTermsAcceptance || hasUserAcceptedTerms(user))
        return false;
 ...
 */
export function redirectToTermsPage(res, redirectUrl) {
    if (redirectUrl) {
...
export async function redirectToTermsPageIfNeeded(res, user, ip, redirectUrl) {
