 ...
 */
function assertInt(tag, key, val, errors) {
    if (!(/^\d+$/.test(val) || mustacheTemplateRegex.test(val))) {
        errors.push(`${tag}: value for attribute ${key} must be an integer, but value provided is "${val}"`);
    }
}
/**
 ...
 * @param errors The list of errors to add to.
 */
function assertFloat(tag, key, val, errors) {
    if (!(/^(\d+)\.?(\d*)(e-\d+)?$/.test(val) || mustacheTemplateRegex.test(val))) {
        errors.push(`${tag}: value for attribute ${key} must be an floating-point number, but value provided is "${val}"`);
    }
}
/**
 ...
 * @param errors The list of errors to add to.
 */
function assertInChoices(tag, key, val, choices, errors) {
    if (!(choices.includes(val) || mustacheTemplateRegex.test(val))) {
        errors.push(`${tag}: value for attribute ${key} must be in ${choices}, but value provided is ${val}`);
    }
}
/**
 ...
 * @param errors The list of errors to add to.
 */
function assertBool(tag, key, val, errors) {
    assertInChoices(tag, key, val, [
        'true',
        't',
        '1',
        'True',
 ...
 */
function checkTag(ast, optimistic) {
    if ('tagName' in ast) {
        switch (ast.tagName) {
 ...
 * @returns The list of errors for the tag, if any.
 */
function checkMultipleChoice(ast) {
    const errors = [];
        ...
        for (const attr of ast.attrs) {
            ...
            switch (key) {
                ...
                case 'number-answers':
                    ...
                    break;
                case 'order':
                    assertInChoices('pl-multiple-choice', key, val, ['random', 'ascend', 'descend', 'fixed'], errors);
                    break;
    ...
    if (!usedAllOfTheAbove && usedAllOfTheAboveFeedback) {
        errors.push('pl-multiple-choice: if using all-of-the-above-feedback, you must also use all-of-the-above.');
    }
    if (!usedNoneOfTheAbove && usedNoneOfTheAboveFeedback) {
        errors.push('pl-multiple-choice: if using none-of-the-above-feedback, you must also use none-of-the-above.');
    }
 ...
 * @returns The list of errors for the tag, if any.
 */
function checkIntegerInput(ast) {
    const errors = [];
    let answersName = null;
    if ('attrs' in ast) {
        for (const attr of ast.attrs) {
            const key = attr.name;
            const val = attr.value;
            switch (key) {
                ...
                case 'correct-answer':
                    if (val.match(mustacheTemplateExtractorRegex)) {
                        errors.push("pl-integer-input: correct-answer attribute value must not be a Mustache template. If the correct answer depends on dynamic parameters, set `data['correct_answers']` accordingly in `server.py` and remove this attribute.");
                    }
                    break;
                case 'label':
                case 'suffix':
 ...
 * @returns The list of errors for the tag, if any.
 */
function checkNumericalInput(ast) {
    const errors = [];
    let answersName = null;
    let usedRelabs = true;
    let usedRtol = false;
        ...
        for (const attr of ast.attrs) {
            ...
            switch (key) {
                ...
                case 'correct-answer':
                    ...
                    if (val.match(mustacheTemplateExtractorRegex)) {
                        errors.push("pl-number-input: correct-answer attribute value must not be a Mustache template. If the correct answer depends on dynamic parameters, set `data['correct_answers']` accordingly in `server.py` and remove this attribute.");
                    }
                    break;
                case 'label':
                case 'suffix':
    ...
    if ((usedRtol || usedAtol) && !usedRelabs) {
        errors.push('pl-number-input: comparison modes decdigs and sigfigs use digits, not rtol or atol.');
    }
 ...
 * @returns The list of errors for the tag, if any.
 */
function checkAnswerMultipleChoice(ast) {
    const errors = [];
    if ('attrs' in ast) {
        for (const attr of ast.attrs) {
            switch (attr.name) {
 ...
 * @returns The list of errors for the tag, if any.
 */
function checkAnswerCheckbox(ast) {
    const errors = [];
    if ('attrs' in ast) {
        for (const attr of ast.attrs) {
            switch (attr.name) {
...
function checkStringInput(ast) {
    ...
    return {
        ...
        mandatoryPythonCorrectAnswers: !usedCorrectAnswer && answersName ? new Set([answersName]) : undefined,
    };
}
/**
...
function checkCheckbox(ast) {
    ...
    if (usedPartialCreditMethod && !usedPartialCredit) {
        errors.push('pl-checkbox: if partial-credit-method is set, then partial-credit must be set to true.');
    }
    let errorsChildren = [];
    if ('childNodes' in ast) {
        for (const child of ast.childNodes) {
 ...
 */
function dfsCheckParseTree(ast, optimistic) {
    let { errors, mandatoryPythonCorrectAnswers } = checkTag(ast, optimistic);
    mandatoryPythonCorrectAnswers ??= new Set();
 ...
 */
export function validateHTML(file, optimistic, usesServerPy) {
...
//# sourceMappingURL=validateHTML.js.map