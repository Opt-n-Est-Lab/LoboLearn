 ...
 */
export async function getEnrollmentCountsForInstitution({ institution_id, created_since, }) {
    const result = await queryOptionalRow(sql.select_enrollment_counts_for_institution, { institution_id, created_since }, EnrollmentCountsSchema);
    return {
        paid: result?.paid ?? 0,
        free: result?.free ?? 0,
    };
 ...
 * string like '1 year' or '6 months'.
 */
export async function getEnrollmentCountsForCourse({ course_id, created_since, }) {
    const result = await queryOptionalRow(sql.select_enrollment_counts_for_course, {
        course_id,
        created_since,
 ...
 */
export async function getEnrollmentCountsForCourseInstance(course_instance_id) {
    const result = await queryOptionalRow(sql.select_enrollment_counts_for_course_instance, { course_instance_id }, EnrollmentCountsSchema);
    return {
        paid: result?.paid ?? 0,
        free: result?.free ?? 0,
    };
}
export var PotentialEnterpriseEnrollmentStatus;
(function (PotentialEnterpriseEnrollmentStatus) {
    PotentialEnterpriseEnrollmentStatus["ALLOWED"] = "allowed";
    PotentialEnterpriseEnrollmentStatus["LIMIT_EXCEEDED"] = "limit_exceeded";
    PotentialEnterpriseEnrollmentStatus["PLAN_GRANTS_REQUIRED"] = "plan_grants_required";
})(PotentialEnterpriseEnrollmentStatus || (PotentialEnterpriseEnrollmentStatus = {}));
/**
 ...
 * return `false`.
 */
export async function checkPotentialEnterpriseEnrollment({ institution, course, course_instance, authz_data, }) {
    ...
    const courseInstanceEnrollmentCounts = await getEnrollmentCountsForCourseInstance(course_instance.id);
    ...
    const courseYearlyEnrollmentLimit = course.yearly_enrollment_limit ?? institutionYearlyEnrollmentLimit;
    const courseInstanceEnrollmentLimit = course_instance.enrollment_limit ??
        course.course_instance_enrollment_limit ??
        institution.course_instance_enrollment_limit;
    if (freeInstitutionEnrollmentCount + 1 > institutionYearlyEnrollmentLimit ||
