...
import { PlanGrantSchema } from '../../lib/db-types.js';
import {} from '../../lib/types.js';
import { insertAuditLog } from '../../models/audit-log.js';
const sql = loadSqlEquiv(import.meta.url);
export async function ensurePlanGrant({ plan_grant, authn_user_id, }) {
    const newPlanGrant = await queryRow(sql.ensure_plan_grant, {
...
export async function updatePlanGrant({ plan_grant, type, authn_user_id, }) {
    const updatedPlanGrant = await queryRow(sql.update_plan_grant, { id: plan_grant.id, type }, PlanGrantSchema);
    await insertAuditLog({
        authn_user_id,
        table_name: 'plan_grants',
        action: 'update',
...
export async function deletePlanGrant({ plan_grant, authn_user_id, }) {
    const deletedPlanGrant = await queryRow(sql.delete_plan_grant, { id: plan_grant.id }, PlanGrantSchema);
    await insertAuditLog({
        authn_user_id,
        table_name: 'plan_grants',
        action: 'delete',
