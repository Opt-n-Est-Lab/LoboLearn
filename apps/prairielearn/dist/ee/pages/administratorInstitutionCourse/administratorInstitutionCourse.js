...
import { AdministratorInstitutionCourse, CourseInstanceRowSchema, } from './administratorInstitutionCourse.html.js';
const sql = loadSqlEquiv(import.meta.url);
const router = Router({ mergeParams: true });
router.get('/', asyncHandler(async (req, res) => {
    const institution = await getInstitution(req.params.institution_id);
    const course = await queryRow(sql.select_course, {
        institution_id: req.params.institution_id,
        course_id: req.params.course_id,
    }, CourseSchema);
    const rows = await queryRows(sql.select_course_instances, { course_id: course.id }, CourseInstanceRowSchema);
    res.send(AdministratorInstitutionCourse({
        institution,
        course,
        rows,
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'update_enrollment_limits') {
        ...
        await runInTransactionAsync(async () => {
            const updatedCourse = await queryRow(sql.update_enrollment_limits, {
                course_id: course.id,
...
//# sourceMappingURL=administratorInstitutionCourse.js.map