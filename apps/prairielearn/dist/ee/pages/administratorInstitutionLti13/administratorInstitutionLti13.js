...
import {} from './administratorInstitutionLti13.types.js';
...
router.get('/:unsafe_lti13_instance_id?', asyncHandler(async (req, res) => {
    ...
    const lti13Instances = await queryRows(sql.select_instances, {
        institution_id: req.params.institution_id,
    }, Lti13InstanceSchema);
    const platform_defaults_hardcoded = [
        {
            platform: 'Unknown',
            display_order: 0,
            issuer_params: {},
    ...
    const platform_defaults = _.sortBy([...platform_defaults_hardcoded, ...config.lti13InstancePlatforms], ['display_order', 'platform']);
    let paramInstance;
    // Handle the / (no id passed case)
    if (typeof req.params.unsafe_lti13_instance_id === 'undefined') {
        if (lti13Instances.length > 0) {
            return res.redirect(`/pl/administrator/institution/${institution.id}/lti13/${lti13Instances[0].id}`);
        }
        ...
        if (!paramInstance) {
            throw new error.HttpStatusError(404, `LTI 1.3 instance ${req.params.unsafe_lti13_instance_id} not found`);
        }
    }
    res.send(AdministratorInstitutionLti13({
        institution,
        lti13Instances,
...
router.post('/:unsafe_lti13_instance_id?', asyncHandler(async (req, res) => {
    if (req.body.__action === 'add_key') {
        ...
        const new_li = await queryRows(sql.insert_instance, {
            ...lti13_instance_defaults,
            institution_id: req.params.institution_id,
        }, z.string());
        flash('success', `Instance #${new_li} added.`);
        return res.redirect(`/pl/administrator/institution/${req.params.institution_id}/lti13/${new_li}`);
    }
...
//# sourceMappingURL=administratorInstitutionLti13.js.map