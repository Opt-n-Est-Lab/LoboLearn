...
export function AdministratorInstitutionSaml({ institution, samlProvider, institutionAuthenticationProviders, host, resLocals, }) {
    const hasSamlProvider = !!samlProvider;
    const hasEnabledSaml = institutionAuthenticationProviders.some((p) => p.name === 'SAML');
    const missingAttributeMappings = !samlProvider?.uid_attribute || !samlProvider?.uin_attribute || !samlProvider?.name_attribute;
...
function DeleteSamlConfigurationModal({ csrfToken }) {
    return Modal({
        id: 'deleteModal',
...
export function DecodedAssertion({ xml, profile }) {
    return html `
...
//# sourceMappingURL=administratorInstitutionSaml.html.js.map