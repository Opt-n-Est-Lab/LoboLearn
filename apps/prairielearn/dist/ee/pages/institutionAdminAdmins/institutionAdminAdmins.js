...
import { deleteInstitutionAdministrator, ensureInstitutionAdministrator, } from '../../models/institution-administrator.js';
import { InstitutionAdminAdmins, InstitutionAdminAdminsRowSchema, } from './institutionAdminAdmins.html.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const rows = await queryValidatedRows(sql.select_admins, { institution_id: institution.id }, InstitutionAdminAdminsRowSchema);
    res.send(InstitutionAdminAdmins({
        institution,
        rows,
        uidsLimit: MAX_UIDS,
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'addAdmins') {
        ...
        if (invalidUids.length > 0) {
            flash('error', `The following users either don't exist or aren't in this institution: ${invalidUids.join(', ')}`);
        }
...
//# sourceMappingURL=institutionAdminAdmins.js.map