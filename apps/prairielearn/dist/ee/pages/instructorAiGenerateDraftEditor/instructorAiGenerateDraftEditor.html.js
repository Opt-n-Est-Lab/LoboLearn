...
export function InstructorAiGenerateDraftEditor({ resLocals, prompts, question, variantId, }) {
...
function PromptHistory({ prompts, urlPrefix, csrfToken, showJobLogs, }) {
    return prompts.map((prompt, index, filteredPrompts) => {
      ...
      >
        <div>
          ...
          <div>
            ${run(() => {
            if (!showJobLogs || !prompt.job_sequence_id)
                return '';
        ...
        ${run(() => {
            ...
            if (index === filteredPrompts.length - 1)
                return '';
            return html `
            <form method="post">
...
function QuestionAndFilePreview({ prompts, resLocals, }) {
    return html `
...
function QuestionCodeEditors({ htmlContents, pythonContents, csrfToken, }) {
    return html `
...
function FinalizeModal({ csrfToken }) {
    return Modal({
        id: 'finalizeModal',
...
//# sourceMappingURL=instructorAiGenerateDraftEditor.html.js.map