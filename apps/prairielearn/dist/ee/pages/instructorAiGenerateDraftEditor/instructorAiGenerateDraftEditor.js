...
async function saveGeneratedQuestion(res, htmlFileContents, pythonFileContents, title, qid) {
    const files = {};
    if (htmlFileContents) {
...
async function saveRevisedQuestion({ course, question, user, authn_user, authz_data, urlPrefix, html, python, prompt, promptType, }) {
    const client = getCourseFilesClient();
    const files = {
...
function assertCanCreateQuestion(resLocals) {
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const prompts = await queryRows(sql.select_ai_question_generation_prompts, {
    ...
    res.send(InstructorAiGenerateDraftEditor({
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'regenerate_question') {
        ...
        const result = await regenerateQuestion(client, res.locals.course.id, res.locals.authn_user.user_id, prompts[0]?.user_prompt, req.body.prompt, prompts[prompts.length - 1].html || '', prompts[prompts.length - 1].python || '', question.qid, res.locals.authn_user.user_id, res.locals.authz_data.has_course_permission_edit);
        ...
        const qid = await saveGeneratedQuestion(res, prompts[prompts.length - 1].html || undefined, prompts[prompts.length - 1].python || undefined, req.body.title, req.body.qid);
        const client = getCourseFilesClient();
        ...
        if (result.status === 'error') {
            throw new error.HttpStatusError(500, 'Draft deletion failed, but question creation succeeded.');
        }
        ...
        const prompt = await queryRow(sql.select_ai_question_generation_prompt_by_id_and_question, { prompt_id: req.body.unsafe_prompt_id, question_id: req.params.question_id }, AiQuestionGenerationPromptSchema);
        await saveRevisedQuestion({
            course: res.locals.course,
            question,
            user: res.locals.user,
        ...
        res.redirect(`${res.locals.urlPrefix}/ai_generate_editor/${req.params.question_id}?variant_id=${variantId}`);
    }
    else {
...
//# sourceMappingURL=instructorAiGenerateDraftEditor.js.map