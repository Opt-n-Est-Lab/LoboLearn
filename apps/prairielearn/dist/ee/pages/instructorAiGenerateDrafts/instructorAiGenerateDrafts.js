...
import { DraftMetadataWithQidSchema, GenerationFailure, InstructorAIGenerateDrafts, } from './instructorAiGenerateDrafts.html.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const drafts = await queryRows(sql.select_draft_generation_info_by_course_id, { course_id: res.locals.course.id }, DraftMetadataWithQidSchema);
...
router.get('/generation_logs.json', asyncHandler(async (req, res) => {
    ...
    const file = await queryRows(sql.select_ai_question_generation_prompts_by_course_id, { course_id: res.locals.course.id }, AiQuestionGenerationPromptSchema);
    res.json(file);
}));
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'generate_question') {
        ...
        const questions = await queryRows(sql.select_draft_questions_by_course_id, { course_id: res.locals.course.id.toString() }, IdSchema);
...
//# sourceMappingURL=instructorAiGenerateDrafts.js.map