...
import {} from '../../lib/billing/plans-types.js';
import { getPlanGrantsForContext, getRequiredPlansForCourseInstance, updateRequiredPlansForCourseInstance, } from '../../lib/billing/plans.js';
import { InstructorCourseInstanceBilling, } from './instructorInstanceAdminBilling.html.js';
const router = Router({ mergeParams: true });
const sql = loadSqlEquiv(import.meta.url);
async function loadPageData(res) {
    ...
    const enrollmentCount = await queryRow(sql.course_instance_enrollment_count, { course_instance_id: res.locals.course_instance.id }, z.number());
    const enrollmentLimit = res.locals.course_instance.enrollment_limit ??
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const { requiredPlans, institutionPlanGrants, courseInstancePlanGrants, enrollmentCount, enrollmentLimit, enrollmentLimitSource, } = await loadPageData(res);
    const { external_grading_question_count, workspace_question_count } = await queryRow(sql.question_counts, { course_id: res.locals.course.id }, z.object({
        external_grading_question_count: z.number(),
        workspace_question_count: z.number(),
    }));
    res.send(InstructorCourseInstanceBilling({
        requiredPlans,
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const desiredRequiredPlans = [];
    if (req.body.student_billing_enabled === '1')
        desiredRequiredPlans.push('basic');
    if (req.body.compute_enabled === '1')
        desiredRequiredPlans.push('compute');
    ...
    await updateRequiredPlansForCourseInstance(res.locals.course_instance.id, desiredRequiredPlans, res.locals.authn_user.user_id);
    res.redirect(req.originalUrl);
}));
export default router;
//# sourceMappingURL=instructorInstanceAdminBilling.js.map