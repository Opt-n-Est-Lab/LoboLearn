...
import { AssessmentSchema, Lti13AssessmentsSchema, Lti13CourseInstanceSchema, } from '../../../lib/db-types.js';
import { createServerJob } from '../../../lib/server-jobs.js';
import { getCanonicalHost } from '../../../lib/url.js';
import { insertAuditLog } from '../../../models/audit-log.js';
import { Lti13CombinedInstanceSchema, createAndLinkLineitem, getLineitems, queryAndLinkLineitem, syncLineitems, unlinkAssessment, updateLti13Scores, validateLti13CourseInstance, } from '../../lib/lti13.js';
import { AssessmentRowSchema, InstructorInstanceAdminLti13, LineitemsInputs, } from './instructorInstanceAdminLti13.html.js';
...
router.get('/:unsafe_lti13_course_instance_id?', asyncHandler(async (req, res) => {
    if (!res.locals.authz_data.has_course_instance_permission_edit) {
        throw new error.HttpStatusError(403, 'Access denied (must be a student data editor)');
    }
    const instances = await queryRows(sql.select_lti13_instances, {
    ...
    if (!req.params.unsafe_lti13_course_instance_id) {
        ...
        res.redirect(`/pl/course_instance/${res.locals.course_instance.id}/instructor/instance_admin/lti13_instance/${lti13_course_instance_id}`);
        return;
    }
    const instance = instances.find((i) => i.lti13_course_instance.id === req.params.unsafe_lti13_course_instance_id);
    if (!instance) {
    ...
    const assessments = await queryRows(sql.select_assessments, {
        course_instance_id: res.locals.course_instance.id,
        authz_data: res.locals.authz_data,
        req_date: res.locals.req_date,
        assessments_group_by: res.locals.course_instance.assessments_group_by,
    }, AssessmentRowSchema);
    const lineitems = await queryRows(sql.select_lti13_assessments, {
...
router.post('/:unsafe_lti13_course_instance_id', asyncHandler(async (req, res) => {
    if (!res.locals.authz_data.has_course_instance_permission_edit) {
        throw new error.HttpStatusError(403, 'Access denied (must be a student data editor)');
    }
    const instance = await queryRow(sql.select_lti13_instance, {
        course_instance_id: res.locals.course_instance.id,
    ...
    if (req.body.__action === 'delete_lti13_course_instance') {
        await runInTransactionAsync(async () => {
            const deleted_lti13_course_instance = await queryRow(sql.delete_lti13_course_instance, {
                course_instance_id: res.locals.course_instance.id,
                lti13_course_instance_id: req.params.unsafe_lti13_course_instance_id,
        ...
        res.redirect(`/pl/course_instance/${res.locals.course_instance.id}/instructor/instance_admin/assessments`);
    }
        ...
        const serverJob = await createServerJob(serverJobOptions);
        serverJob.executeInBackground(async (job) => {
            const assessment = await queryRow(sql.select_assessment_to_create, {
        ...
        const group_id = res.locals.course_instance.assessments_group_by === 'Set'
            ? req.body.assessment_set_id || null
            : req.body.assessment_module_id || null;
        const lineitems = await queryRows(sql.delete_lti13_assessments, {
            lti13_course_instance_id: instance.lti13_course_instance.id,
            course_instance_id: instance.lti13_course_instance.course_instance_id,
            group_id,
            assessments_group_by: res.locals.course_instance.assessments_group_by,
        }, Lti13AssessmentsSchema);
        flash('success', `${lineitems.length} assessment${lineitems.length === 1 ? '' : 's'} unlinked.`);
        return res.redirect(req.originalUrl);
    }
        ...
        const group_id = res.locals.course_instance.assessments_group_by === 'Set'
            ? req.body.assessment_set_id || null
            : req.body.assessment_module_id || null;
        const assessments = await queryRows(sql.select_assessments_to_create, {
            course_instance_id: instance.lti13_course_instance.course_instance_id,
            group_id,
            assessments_group_by: res.locals.course_instance.assessments_group_by,
        }, AssessmentSchema.extend({
        ...
        const assessment = await queryRow(sql.select_assessment_in_course_instance, {
            unsafe_assessment_id: req.body.unsafe_assessment_id,
            course_instance_id: res.locals.course_instance.id,
        }, AssessmentSchema);
        if (assessment === null) {
...
//# sourceMappingURL=instructorInstanceAdminLti13.js.map