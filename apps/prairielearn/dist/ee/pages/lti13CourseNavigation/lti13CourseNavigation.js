...
import { Lti13CourseNavigationDone, Lti13CourseNavigationInstructor, Lti13CourseNavigationNotReady, } from './lti13CourseNavigation.html.js';
...
async function courseInstancesAllowedToLink({ course_id, user_id, authn_user_id, is_administrator, authn_is_administrator, }) {
...
async function coursesAllowedToLink({ user_id, is_administrator, }) {
...
router.get('/course_instances', asyncHandler(async (req, res) => {
    ...
    if (course_instances.length === 0) {
        ...
        options = joinHtml(course_instances.map((ci) => {
...
router.get('/', asyncHandler(async (req, res) => {
    if ('done' in req.query) {
        res.send(Lti13CourseNavigationDone({
    ...
    const lti13_course_instance = await queryOptionalRow(sql.select_lti13_course_instance, {
        lti13_instance_id: req.params.lti13_instance_id,
    ...
    if (lti13_course_instance) {
        ...
        res.redirect(`/pl/course_instance/${lti13_course_instance.course_instance_id}/${role_instructor ? 'instructor/' : ''}`);
        return;
    }
    // Students get a "come back later" message
    if (!role_instructor) {
        res.send(Lti13CourseNavigationNotReady({
            resLocals: res.locals,
            courseName,
    ...
    res.send(Lti13CourseNavigationInstructor({
        resLocals: res.locals,
        courseName,
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const course_instance = await queryOptionalRow(sql.select_lti13_institution_course_instance, {
    ...
    const hasCourseInstanceAllowed = courseInstancesAllowed.some((ci) => ci.id === course_instance.id);
...
//# sourceMappingURL=lti13CourseNavigation.js.map