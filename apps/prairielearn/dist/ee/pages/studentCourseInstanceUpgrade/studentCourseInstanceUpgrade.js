...
import { CourseInstanceSchema, CourseSchema, InstitutionSchema, UserSchema, } from '../../../lib/db-types.js';
import { getCanonicalHost } from '../../../lib/url.js';
import { checkPlanGrantsForLocals } from '../../lib/billing/plan-grants.js';
import { getMissingPlanGrants, getPlanGrantsForPartialContexts, getRequiredPlansForCourseInstance, } from '../../lib/billing/plans.js';
import { getOrCreateStripeCustomerId, getPriceForPlan, getPricesForPlans, getStripeClient, } from '../../lib/billing/stripe.js';
import { ensurePlanGrant } from '../../models/plan-grants.js';
import { getStripeCheckoutSessionByStripeObjectId, insertStripeCheckoutSessionForUserInCourseInstance, markStripeCheckoutSessionCompleted, updateStripeCheckoutSessionData, } from '../../models/stripe-checkout-sessions.js';
import { CourseInstanceStudentUpdateSuccess, StudentCourseInstanceUpgrade, } from './studentCourseInstanceUpgrade.html.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    res.send(StudentCourseInstanceUpgrade({
        course,
        course_instance,
        missingPlans,
        planPrices,
...
router.post('/', asyncHandler(async (req, res) => {
    if (req.body.__action === 'upgrade') {
        ...
        if (!session.url)
            throw new error.HttpStatusError(500, 'Stripe session URL not found');
...
router.get('/success', asyncHandler(async (req, res) => {
    ...
    if (!req.query.session_id)
        throw new error.HttpStatusError(400, 'Missing session_id');
    ...
    if (localSession.completed_at) {
        ...
        res.send(CourseInstanceStudentUpdateSuccess({
            course,
            course_instance,
            paid: true,
            resLocals: res.locals,
    ...
    if (localSession.course_instance_id !== course_instance.id ||
...
//# sourceMappingURL=studentCourseInstanceUpgrade.js.map