...
import { getConfiguredUser, getOrCreateUser, withUser, } from '../../../tests/utils/auth.js';
import { reconcilePlanGrantsForCourseInstance, reconcilePlanGrantsForCourseInstanceUser, updateRequiredPlansForCourseInstance, } from '../../lib/billing/plans.js';
...
describe('studentCourseInstanceUpgrade', () => {
    ...
    it('is not displayed if there are no unsatisfied required plans', async () => {
        await withUser(studentUser, async () => {
            ...
            await reconcilePlanGrantsForCourseInstanceUser({ institution_id: '1', course_instance_id: '1', user_id: user.user_id }, [{ plan: 'basic', grantType: 'stripe' }], '1');
            const res = await fetch(assessmentsUrl);
            assert.isOk(res.ok);
            assert.equal(res.url, assessmentsUrl);
        });
...
//# sourceMappingURL=studentCourseInstanceUpgrade.test.js.map