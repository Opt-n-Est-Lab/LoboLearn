...
router.use('/:lti13_instance_id/', asyncHandler(async (req, res, next) => {
    const lti13_instance = await selectLti13Instance(req.params.lti13_instance_id);
    if (await features.enabled('lti13', {
...
router.use('/:lti13_instance_id/', asyncHandler(async (req, res, next) => {
    const lti13_instance = await selectLti13Instance(req.params.lti13_instance_id);
    const instAuthProviders = await getInstitutionAuthenticationProviders(lti13_instance.institution_id);
...
router.use('/:lti13_instance_id/course_navigation', authnMiddleware, // authentication, set res.locals.authn_user
csrfToken, lti13CourseNavigation);
