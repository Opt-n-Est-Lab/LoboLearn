...
import {} from './lib/code-caller/code-caller-shared.js';
...
async function handleInput(line, codeCaller) {
    ...
    try {
        ({ result, output } = await codeCaller.call(request.type, request.directory, request.file, request.fcn, request.args));
    }
    catch (err) {
        callErr = err;
    }
