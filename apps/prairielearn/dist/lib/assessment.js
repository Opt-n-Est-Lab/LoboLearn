...
import { AssessmentInstanceSchema, ClientFingerprintSchema, CourseSchema, DateFromISOString, IdSchema, QuestionSchema, VariantSchema, } from './db-types.js';
 ...
 */
export async function checkBelongs(assessment_instance_id, assessment_id) {
    if ((await sqldb.queryOptionalRow(sql.check_belongs, { assessment_instance_id, assessment_id }, IdSchema)) == null) {
 ...
 */
export async function makeAssessmentInstance({ assessment, user_id, authn_user_id, mode, time_limit_min, date, client_fingerprint_id, }) {
    return await sqldb.runInTransactionAsync(async () => {
        ...
        const { assessment_instance_id, created } = await sqldb.queryRow(sql.insert_assessment_instance, {
 ...
 */
export async function updateAssessmentInstance(assessment_instance_id, authn_user_id, recomputeGrades = true) {
    const updated = await sqldb.runInTransactionAsync(async () => {
        const assessmentInstance = await sqldb.queryOptionalRow(sql.select_and_lock_assessment_instance, { assessment_instance_id }, AssessmentInstanceSchema);
        ...
        const newInstanceQuestionIds = await sqldb.queryRows(sql.insert_instance_questions, { assessment_instance_id, assessment_id: assessmentInstance.assessment_id, authn_user_id }, IdSchema);
        const newMaxPoints = await sqldb.queryOptionalRow(sql.update_assessment_instance_max_points, { assessment_instance_id, authn_user_id }, AssessmentInstanceSchema.pick({ max_points: true, max_bonus_points: true }));
        // If assessment was not updated, grades do not need to be recomputed.
        if (newInstanceQuestionIds.length === 0 && newMaxPoints == null)
            return false;
 ...
 */
export async function gradeAssessmentInstance(assessment_instance_id, user_id, authn_user_id, requireOpen, close, overrideGradeRate, client_fingerprint_id) {
    ...
    const variants = await sqldb.queryRows(sql.select_variants_for_assessment_instance_grading, { assessment_instance_id }, z.object({ variant: VariantSchema, question: QuestionSchema, variant_course: CourseSchema }));
    ...
    await async.eachSeries(variants, async (row) => {
        ...
        await gradeVariant(row.variant, check_submission_id, row.question, row.variant_course, user_id, authn_user_id, overrideGradeRate);
    });
 ...
 */
export async function gradeAllAssessmentInstances(assessment_id, user_id, authn_user_id, close, overrideGradeRate) {
    debug('gradeAllAssessmentInstances()');
    const { assessment_label, course_instance_id, course_id } = await sqldb.queryRow(sql.select_assessment_info, { assessment_id }, AssessmentInfoSchema);
    ...
    serverJob.executeInBackground(async (job) => {
        ...
        const instances = await sqldb.queryRows(sql.select_instances_to_grade, { assessment_id }, InstancesToGradeSchema);
        ...
        await async.eachSeries(instances, async (row) => {
            ...
            await gradeAssessmentInstance(row.assessment_instance_id, user_id, authn_user_id, requireOpen, close, overrideGradeRate, null);
        });
    });
...
export async function updateAssessmentStatisticsForCourseInstance(course_instance_id) {
    const rows = await sqldb.queryRows(sql.select_assessments_for_statistics_update, { course_instance_id }, IdSchema);
...
export async function updateAssessmentStatistics(assessment_id) {
    await sqldb.runInTransactionAsync(async () => {
        ...
        const needs_statistics_update = await sqldb.queryRow(sql.select_assessment_needs_statisics_update, { assessment_id }, z.boolean());
...
export async function updateAssessmentInstanceScore(assessment_instance_id, score_perc, authn_user_id) {
    await sqldb.runInTransactionAsync(async () => {
        const { max_points } = await sqldb.queryRow(sql.select_and_lock_assessment_instance, { assessment_instance_id }, AssessmentInstanceSchema);
...
export async function updateAssessmentInstancePoints(assessment_instance_id, points, authn_user_id) {
    await sqldb.runInTransactionAsync(async () => {
        const { max_points } = await sqldb.queryRow(sql.select_and_lock_assessment_instance, { assessment_instance_id }, AssessmentInstanceSchema);
...
export async function selectAssessmentInstanceLog(assessment_instance_id, include_files) {
    const log = await sqldb.queryRows(sql.assessment_instance_log, { assessment_instance_id, include_files }, InstanceLogSchema);
...
export async function selectAssessmentInstanceLogCursor(assessment_instance_id, include_files) {
    return sqldb.queryValidatedCursor(sql.assessment_instance_log, { assessment_instance_id, include_files }, InstanceLogSchema);
}
export async function updateAssessmentQuestionStats(assessment_question_id) {
    await sqldb.queryAsync(sql.calculate_stats_for_assessment_question, { assessment_question_id });
}
export async function updateAssessmentQuestionStatsForAssessment(assessment_id) {
    await sqldb.runInTransactionAsync(async () => {
        const assessment_questions = await sqldb.queryRows(sql.select_assessment_questions, { assessment_id }, IdSchema);
...
export async function deleteAssessmentInstance(assessment_id, assessment_instance_id, authn_user_id) {
    const deleted_id = await sqldb.queryOptionalRow(sql.delete_assessment_instance, { assessment_id, assessment_instance_id, authn_user_id }, IdSchema);
    if (deleted_id == null) {
        throw new error.HttpStatusError(403, 'This assessment instance does not exist in this assessment.');
    }
}
export async function deleteAllAssessmentInstancesForAssessment(assessment_id, authn_user_id) {
 ...
 */
export function canDeleteAssessmentInstance(resLocals) {
    return (
...
export async function selectAssessmentInstanceLastSubmissionDate(assessment_instance_id) {
    return await sqldb.queryRow(sql.select_assessment_instance_last_submission_date, { assessment_instance_id }, DateFromISOString.nullable());
}
