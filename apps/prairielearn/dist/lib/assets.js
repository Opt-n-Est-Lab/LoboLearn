 ...
 */
function getHashForPath(hashes, assetPath) {
 ...
 * returns `@scope/foo`.
 */
function getPackageNameForAssetPath(assetPath) {
...
function getPackageVersion(packageName) {
    ...
    try {
        ...
        const pkgJsonPath = path.resolve(pkgPath.slice(0, lastNodeModulesIndex + nodeModulesToken.length), packageName, 'package.json');
 ...
 */
function getNodeModulesAssetHash(assetPath) {
...
export function applyMiddleware(app) {
    ...
    router.use('/node_modules/:cachebuster', staticNodeModules('.', {
        // In dev mode, we assume that `node_modules` won't change while the server
        // is running, so we'll enable long-term caching.
        maxAge: '1y',
        immutable: true,
    }));
    router.use('/public/:cachebuster', express.static(path.join(APP_ROOT_PATH, 'public'), {
 ...
 */
export function nodeModulesAssetPath(assetPath) {
    const assetsPrefix = assertAssetsPrefix();
 ...
 */
export function coreElementAssetPath(assetPath) {
    const assetsPrefix = assertAssetsPrefix();
 ...
 */
export function courseElementAssetBasePath(courseHash, urlPrefix) {
    if (!courseHash) {
 ...
 */
export function courseElementAssetPath(courseHash, urlPrefix, assetPath) {
 ...
 * allow for clients to immutably cache assets.
 */
export function courseElementExtensionAssetBasePath(courseHash, urlPrefix) {
    if (!courseHash) {
        // If for some reason we don't have a course hash, fall back to the
        // non-cached path so that we don't accidentally instruct the client
        // to indefinitely cache a file without a proper cachebuster.
 ...
 * immutably cache assets.
 */
export function courseElementExtensionAssetPath(courseHash, urlPrefix, assetPath) {
