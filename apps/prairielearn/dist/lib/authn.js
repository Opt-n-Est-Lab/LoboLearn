...
export async function loadUser(req, res, authnParams, optionsParams = {}) {
    ...
    if (authnParams.user_id !== undefined) {
        ...
        if (result === 'invalid_authn_provider') {
            throw new HttpRedirect(`/pl/login?unsupported_provider=true&institution_id=${user_institution_id}`);
        }
    }
    const selectedUser = await sqldb.queryOptionalRow(sql.select_user, { user_id }, z.object({
        user: UserSchema,
    ...
    res.locals.is_institution_administrator =
        res.locals.is_administrator ||
            (await sqldb.queryRow(sql.select_is_institution_admin, {
