 ...
 * object.
 */
export function chunkMetadataFromDatabaseChunk(chunk) {
    switch (chunk.type) {
        case 'elements':
 ...
 */
export function pathForChunk(chunkMetadata) {
    switch (chunkMetadata.type) {
        ...
        case 'clientFilesCourseInstance':
            return path.join('courseInstances', chunkMetadata.courseInstanceName, 'clientFilesCourseInstance');
        case 'clientFilesAssessment':
            return path.join('courseInstances', chunkMetadata.courseInstanceName, 'assessments', chunkMetadata.assessmentName, 'clientFilesAssessment');
    }
}
/**
 ...
 * directory.
 */
export function coursePathForChunk(coursePath, chunkMetadata) {
 ...
 */
export async function identifyChangedFiles(coursePath, oldHash, newHash) {
    ...
    const { stdout: topLevelStdout } = await util.promisify(child_process.exec)('git rev-parse --show-toplevel', { cwd: coursePath });
    const topLevel = topLevelStdout.trim();
    const { stdout: diffStdout } = await util.promisify(child_process.exec)(`git diff --name-only ${oldHash}..${newHash}`, {
        cwd: coursePath,
    ...
    const courseChangedFiles = absoluteChangedFiles.filter((absoluteChangedFile) => contains(coursePath, absoluteChangedFile));
    // Convert all absolute paths back into relative paths.
    return courseChangedFiles.map((absoluteChangedFile) => path.relative(coursePath, absoluteChangedFile));
}
/**
 ...
 */
export function identifyChunksFromChangedFiles(changedFiles, courseData) {
    const courseChunks = {
        elements: false,
 ...
 */
export async function getAllChunksForCourse(courseId) {
 ...
 * (re)generated.
 */
export async function diffChunks({ coursePath, courseId, courseData, changedFiles, }) {
    ...
    await async.each(Object.entries(courseData.courseInstances), async ([ciid, courseInstanceInfo]) => {
        const hasClientFilesCourseInstanceDirectory = await fs.pathExists(path.join(coursePath, 'courseInstances', ciid, 'clientFilesCourseInstance'));
        ...
        await async.each(Object.keys(courseInstanceInfo.assessments), async (tid) => {
            const hasClientFilesAssessmentDirectory = await fs.pathExists(path.join(coursePath, 'courseInstances', ciid, 'assessments', tid, 'clientFilesAssessment'));
    ...
    await Promise.all(Object.entries(existingCourseChunks.courseInstances).map(async ([ciid, courseInstanceInfo]) => {
        const courseInstanceExists = !!courseData.courseInstances[ciid];
        const clientFilesCourseInstanceExists = await fs.pathExists(path.join(coursePath, 'courseInstances', ciid, 'clientFilesCourseInstance'));
        ...
        await Promise.all([...courseInstanceInfo.assessments].map(async (tid) => {
            const assessmentExists = !!courseData.courseInstances[ciid]?.assessments[tid];
            const clientFilesAssessmentExists = await fs.pathExists(path.join(coursePath, 'courseInstances', ciid, 'assessments', tid, 'clientFilesAssessment'));
...
export async function createAndUploadChunks(coursePath, courseId, chunksToGenerate) {
 ...
 */
export async function deleteChunks(courseId, chunksToDelete) {
 ...
 */
export function getChunksDirectoriesForCourseId(courseId) {
 ...
 */
export function getRuntimeDirectoryForCourse(course) {
...
export async function updateChunksForCourse({ coursePath, courseId, courseData, oldHash, newHash, }) {
    let changedFiles = [];
    if (oldHash && newHash) {
 ...
 */
export async function generateAllChunksForCourseList(course_ids, authn_user_id) {
 ...
 */
async function _generateAllChunksForCourseWithJob(course_id, job) {
...
const ensureChunk = async (courseId, chunk) => {
    ...
    switch (chunk.type) {
        ...
        case 'clientFilesCourseInstance':
            ...
            relativeTargetPath = path.join('courseInstances', chunk.course_instance_name, 'clientFilesCourseInstance');
            break;
        case 'clientFilesAssessment':
            if (!chunk.course_instance_name) {
                throw new Error(`course_instance_name is missing for chunk ${chunk.uuid}`);
            ...
            relativeTargetPath = path.join('courseInstances', chunk.course_instance_name, 'assessments', chunk.assessment_name, 'clientFilesAssessment');
            break;
        case 'question':
            if (!chunk.question_name) {
                throw new Error(`question_name is missing for chunk ${chunk.uuid}`);
 ...
 */
export async function ensureChunksForCourseAsync(courseId, chunks) {
    if (!config.chunksConsumer) {
 ...
 */
export async function getTemplateQuestionIds(question) {
 ...
 */
export function logChunkChangesToJob({ updatedChunks, deletedChunks }, job) {
