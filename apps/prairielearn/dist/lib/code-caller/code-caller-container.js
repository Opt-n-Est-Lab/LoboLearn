...
export class CodeCallerContainer {
    ...
    async call(type, directory, file, fcn, args) {
        ...
        const containerTimeout = type === 'ping'
    ...
    async _handleContainerExit(err, code) {
        ...
        if (this.state === WAITING) {
            this._logError(`CodeCallerContainer container exited while in state = WAITING, code = ${JSON.stringify(code)}, err = ${err}`);
            this.container = null;
            this.state = EXITED;
        }
            ...
            this.container = null;
            this.state = EXITED;
            this._callCallback(new Error(`CodeCallerContainer container exited unexpectedly, code = ${JSON.stringify(code)}, err = ${err}`));
        }
    ...
    _checkState(allowedStates) {
        if (allowedStates && !allowedStates.includes(this.state)) {
            ...
            return this._logError(`Expected CodeCallerContainer to be in states [${allowedStatesList}] but actually have state ${String(this.state)}`);
        }
        ...
        if (containerNull != null) {
            if (containerNull && this.container != null) {
                return this._logError(`CodeCallerContainer state ${String(this.state)}: container should be null`);
            }
            if (!containerNull && this.container == null) {
                return this._logError(`CodeCallerContainer state ${String(this.state)}: container should not be null`);
            }
        }
        if (callbackNull != null) {
            if (callbackNull && this.callback != null) {
                return this._logError(`CodeCallerContainer state ${String(this.state)}: callback should be null`);
            }
            if (!callbackNull && this.callback == null) {
                return this._logError(`CodeCallerContainer state ${String(this.state)}: callback should not be null`);
            }
        }
        if (timeoutIDNull != null) {
            if (timeoutIDNull && this.timeoutID != null) {
                return this._logError(`CodeCallerContainer state ${String(this.state)}: timeoutID should be null`);
            }
            if (!timeoutIDNull && this.timeoutID == null) {
                return this._logError(`CodeCallerContainer state ${String(this.state)}: timeoutID should not be null`);
            }
        }
        return true;
    }
...
async function cleanupMountDirectories() {
    try {
        ...
        await Promise.all(outDirs.map(async (dir) => {
...
//# sourceMappingURL=code-caller-container.js.map