...
export class CodeCallerNative {
    ...
    async restart() {
        ...
        if (this.state === CREATED) {
            ...
            this.forbiddenModules = [];
            if (result !== 'success')
                throw new Error(`Error while restarting: ${result}`);
            this.debug('exit restart()');
    ...
    _handleChildExit(code, signal) {
        ...
        if (this.state === WAITING) {
            this._logError('CodeCallerNative child process exited while in state = WAITING, code = ' +
                String(code) +
                ', signal = ' +
                String(signal));
            this.child = null;
            ...
            this.child = null;
            this.state = EXITED;
            this._callCallback(new Error('CodeCallerNative child process exited unexpectedly, code = ' +
                String(code) +
                ', signal = ' +
                String(signal)));
        }
    ...
    _handleChildError(error) {
        ...
        this._checkState([WAITING, IN_CALL, EXITING]);
        if (this.state === WAITING) {
            this._logError('CodeCallerNative child process raised error while in state = WAITING, message = ' +
                String(error));
            this.state = EXITING;
            this.child?.kill('SIGTERM');
        }
        else if (this.state === IN_CALL) {
            this._logError('CodeCallerNative child process raised error while in state = IN_CALL, message = ' +
                String(error));
            this._clearTimeout();
            this.state = EXITING;
            this.child?.kill('SIGTERM');
            ...
            this._logError('CodeCallerNative child process raised error while in state = EXITING, message = ' +
                String(error));
        }
        this._checkState();
    ...
    _restartIsFinished() {
        ...
        if (!this._checkState([RESTARTING]))
            return;
    ...
    _checkState(allowedStates) {
        if (allowedStates && !allowedStates.includes(this.state)) {
            ...
            return this._logError('Expected CodeCallerNative states ' +
                allowedStatesList +
        ...
        if (childNull != null) {
            if (childNull && this.child != null) {
                return this._logError('CodeCallerNative state "' + String(this.state) + '": child should be null');
            }
            if (!childNull && this.child == null) {
                return this._logError('CodeCallerNative state "' + String(this.state) + '": child should not be null');
            }
        }
        if (callbackNull != null) {
            if (callbackNull && this.callback != null) {
                return this._logError('CodeCallerNative state "' + String(this.state) + '": callback should be null');
            }
            if (!callbackNull && this.callback == null) {
                return this._logError('CodeCallerNative state "' + String(this.state) + '": callback should not be null');
            }
        }
        if (timeoutIDNull != null) {
            if (timeoutIDNull && this.timeoutID != null) {
                return this._logError('CodeCallerNative state "' + String(this.state) + '": timeoutID should be null');
            }
            if (!timeoutIDNull && this.timeoutID == null) {
                return this._logError('CodeCallerNative state "' + String(this.state) + '": timeoutID should not be null');
            }
        }
        return true;
    }
}
//# sourceMappingURL=code-caller-native.js.map