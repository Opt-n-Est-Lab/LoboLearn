...
import { getCourseCommitHash, getLockNameForCoursePath, getOrUpdateCourseCommitHash, selectCourseById, updateCourseCommitHash, } from '../models/course.js';
 ...
 */
export async function checkAssessmentInstanceBelongsToCourseInstance(assessment_instance_id, course_instance_id) {
    if ((await sqldb.queryOptionalRow(sql.check_belongs, { assessment_instance_id, course_instance_id }, IdSchema)) == null) {
...
export async function pullAndUpdateCourse({ courseId, userId, authnUserId, path, branch, repository, commit_hash, }) {
