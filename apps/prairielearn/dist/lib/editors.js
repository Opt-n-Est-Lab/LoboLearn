...
import {} from '@js-temporal/polyfill';
...
import { getCourseCommitHash, getLockNameForCoursePath, getOrUpdateCourseCommitHash, updateCourseCommitHash, } from '../models/course.js';
...
import {} from './db-types.js';
import { EXAMPLE_COURSE_PATH } from './paths.js';
import { formatJsonWithPrettier } from './prettier.js';
import { createServerJob } from './server-jobs.js';
const sql = sqldb.loadSqlEquiv(import.meta.url);
const debug = debugfn('prairielearn:editors');
async function syncCourseFromDisk(course, startGitHash, job, courseData) {
    const endGitHash = await getCourseCommitHash(course.path);
    const syncResult = await syncFromDisk.syncDiskToSqlWithLock(course.id, course.path, job, courseData);
...
async function cleanAndResetRepository(course, revision, env, job) {
...
export function getUniqueNames({ shortNames, longNames, shortName = 'New', longName = 'New', }) {
    function getNumberShortName(oldShortNames) {
        ...
        oldShortNames.forEach((oldShortName) => {
            ...
            const found = shortNameCompare === oldShortNameCompare ||
    ...
    function getNumberLongName(oldLongNames) {
        let numberOfMostRecentCopy = 1;
        ...
        oldLongNames.forEach((oldLongName) => {
            if (typeof oldLongName !== 'string')
                return;
            const found = oldLongName === longName || oldLongName.match(new RegExp(`^${longName} \\(([0-9]+)\\)$`));
            if (found) {
        ...
        await serverJob.executeUnsafe(async (job) => {
            ...
            await namedLocks.doWithLock(lockName, { timeout: 5000 }, async () => {
                ...
                try {
                    ...
                    // question sharing.
                    const possibleCourseData = await courseDB.loadFullCourse(this.course.id, this.course.path);
                    const sharingConfigurationValid = await syncFromDisk.checkSharingConfigurationValid(this.course.id, possibleCourseData, logger);
...
export class AssessmentCopyEditor extends Editor {
    ...
    async write() {
        ...
        const names = this.getNamesForCopy(this.assessment.tid, oldNamesShort, this.assessment.title, oldNamesLong);
...
export class AssessmentDeleteEditor extends Editor {
    ...
    async write() {
        ...
        const deletePath = path.join(this.course.path, 'courseInstances', this.course_instance.short_name, 'assessments');
...
export class CourseInstanceCopyEditor extends Editor {
    ...
    async write() {
        ...
        debug('Get all existing short names');
        const oldNamesShort = await this.getExistingShortNames(courseInstancesPath, 'infoCourseInstance.json');
        debug('Generate short_name and long_name');
        const names = this.getNamesForCopy(this.course_instance.short_name, oldNamesShort, this.course_instance.long_name, oldNamesLong);
...
export class CourseInstanceRenameEditor extends Editor {
    ...
    course_instance;
    constructor(params) {
        const { locals: { course_instance }, ciid_new, } = params;
        super({
            ...params,
    ...
    async write() {
        ...
        debug(`Move files\n from ${oldPath}\n to ${newPath}`);
        await fs.move(oldPath, newPath, { overwrite: false });
        await this.removeEmptyPrecedingSubfolders(path.join(this.course.path, 'courseInstances'), this.course_instance.short_name);
        return {
            pathsToAdd: [oldPath, newPath],
...
export class CourseInstanceAddEditor extends Editor {
    ...
    async write() {
        ...
        let allowAccess = undefined;
        if (this.start_access_date || this.end_access_date) {
            allowAccess = {
                startDate: this.start_access_date
                    ? formatDate(new Date(this.start_access_date.epochMilliseconds), this.course.display_timezone, {
                        includeTz: false,
                    })
                    : undefined,
                endDate: this.end_access_date
                    ? formatDate(new Date(this.end_access_date.epochMilliseconds), this.course.display_timezone, {
                        includeTz: false,
                    })
                    : undefined,
            };
    ...
    async write() {
        ...
        const { qid, title } = await run(async () => {
            if (!(this.qid && this.title) && this.isDraft) {
                let draftNumber = await sqldb.queryRow(sql.update_draft_number, { course_id: this.course.id }, z.number());
                while (fs.existsSync(path.join(questionsPath, '__drafts__', `draft_${draftNumber}`))) {
                    //increment and sync to postgres
                    draftNumber = await sqldb.queryRow(sql.update_draft_number, { course_id: this.course.id }, z.number());
                }
...
export class QuestionModifyEditor extends Editor {
    ...
    files;
    constructor(params) {
        const { locals: { question }, files, } = params;
        super({
            ...params,
...
export class QuestionDeleteEditor extends Editor {
    ...
    async write() {
        ...
        for (const question of this.questions) {
            ...
            await this.removeEmptyPrecedingSubfolders(path.join(this.course.path, 'questions'), question.qid);
        }
        return {
            pathsToAdd: this.questions.flatMap((question) => question.qid !== null ? path.join(this.course.path, 'questions', question.qid) : []),
            commitMessage: this.questions.length === 1
...
export class QuestionRenameEditor extends Editor {
    ...
    question;
    constructor(params) {
        const { locals: { question }, qid_new, } = params;
        super({
            ...params,
    ...
    async write() {
        ...
        for (const assessment of assessments) {
            const infoPath = path.join(this.course.path, 'courseInstances', assessment.course_instance_directory, 'assessments', assessment.assessment_directory, 'infoAssessment.json');
...
export class QuestionCopyEditor extends Editor {
    ...
    async write() {
        ...
        const oldNamesShort = await this.getExistingShortNames(questionsPath, 'info.json');
        debug('Generate qid and title');
        const names = this.getNamesForCopy(this.question.qid, oldNamesShort, this.question.title, oldNamesLong);
...
export class QuestionTransferEditor extends Editor {
    ...
    constructor(params) {
        ...
        const from_course = from_course_short_name == null ? 'unknown course' : `course ${from_course_short_name}`;
        super({
            ...params,
...
export class FileDeleteEditor extends Editor {
    ...
    deletePath;
    constructor(params) {
        const { locals: { course }, container, deletePath, } = params;
        let prefix = '';
...
export class FileRenameEditor extends Editor {
    ...
    newPath;
    constructor(params) {
        const { locals: { course }, container, oldPath, newPath, } = params;
        let prefix = '';
        if (course.path !== container.rootPath) {
            prefix = `${path.basename(container.rootPath)}: `;
        }
...
export class FileUploadEditor extends Editor {
    ...
    fileContents;
    constructor(params) {
        const { locals: { course }, container, filePath, fileContents, } = params;
        let prefix = '';
        if (course.path !== container.rootPath) {
            prefix = `${path.basename(container.rootPath)}: `;
        }
...
export class FileModifyEditor extends Editor {
    ...
    origHash;
    constructor(params) {
        const { locals: { course }, container, filePath, editContents, origHash, } = params;
        let prefix = '';
        if (course.path !== container.rootPath) {
            prefix = `${path.basename(container.rootPath)}: `;
        }
