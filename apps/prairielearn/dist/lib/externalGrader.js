...
import { selectOptionalGradingJobById, updateGradingJobAfterGrading, } from '../models/grading-job.js';
import { config } from './config.js';
import { CourseSchema, GradingJobSchema, IdSchema, QuestionSchema, SubmissionSchema, VariantSchema, } from './db-types.js';
import {} from './externalGraderCommon.js';
...
export async function beginGradingJob(grading_job_id) {
    ...
    const { grading_job, submission, variant, question, course } = await sqldb.queryRow(sql.select_grading_job_info, { grading_job_id }, GradingJobInfoSchema);
    if (!question.external_grading_enabled) {
        ...
        processGradingResult(ret).catch((err) => logger.error(`Error processing results for grading job ${grading_job.id}`, err));
        return;
    }
    logger.verbose(`Submitting external grading job ${grading_job.id}.`);
    const gradeRequest = grader.handleGradingRequest(grading_job, submission, variant, question, course);
    ...
    gradeRequest.on('results', (gradingResult) => {
        ...
        processGradingResult(gradingResult).then(() => logger.verbose(`Successfully processed grading job ${grading_job.id}`), (err) => logger.error(`Error processing grading job ${grading_job.id}`, err));
    });
...
export async function processGradingResult(content) {
    try {
        ...
        const assessment_instance_id = await sqldb.queryOptionalRow(sql.select_assessment_for_grading_job, { grading_job_id: content.gradingId }, IdSchema);
...
//# sourceMappingURL=externalGrader.js.map