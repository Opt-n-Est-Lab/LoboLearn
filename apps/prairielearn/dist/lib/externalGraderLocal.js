...
export class ExternalGraderLocal {
    handleGradingRequest(grading_job, submission, variant, question, course) {
        ...
        const timeout = Math.min(question.external_grading_timeout ?? config.externalGradingDefaultTimeout, config.externalGradingMaximumTimeout);
        ...
        (async () => {
            ...
            if (config.externalGradingPullImagesFromDockerHub) {
                try {
                    ...
                    logger.warn(`Error pulling "${question.external_grading_image}" image; attempting to fall back to cached version.`, err);
                }
            }
...
//# sourceMappingURL=externalGraderLocal.js.map