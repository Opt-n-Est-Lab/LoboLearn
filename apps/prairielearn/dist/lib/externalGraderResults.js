    ...
    (async () => {
        ...
        while (true) {
            ...
            await Promise.all(messages.map(async (message) => {
                try {
                    if (!message.Body)
                        throw new Error('Message is missing Body');
...
async function loadQueueUrl(sqs) {
    ...
    const data = await sqs.send(new GetQueueUrlCommand({ QueueName: config.externalGradingResultsQueueName }));
    const queueUrl = data.QueueUrl;
    if (!queueUrl) {
        throw new Error(`Could not get URL for queue ${config.externalGradingResultsQueueName}`);
    }
    logger.verbose(`External grading results queue ${config.externalGradingResultsQueueName}: got URL ${queueUrl}`);
    return queueUrl;
}
...
//# sourceMappingURL=externalGraderResults.js.map