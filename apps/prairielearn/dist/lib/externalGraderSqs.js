...
async function sendJobToQueue(jobId, question, config) {
    ...
    await async.series([
        async () => {
            if (QUEUE_URL)
                return;
            const data = await sqs.send(new GetQueueUrlCommand({
        ...
        async () => {
            const messageBody = {
                ...
                timeout: Math.min(question.external_grading_timeout ?? config.externalGradingDefaultTimeout, config.externalGradingMaximumTimeout),
...
//# sourceMappingURL=externalGraderSqs.js.map