...
export class FeatureManager {
    ...
    async enabled(name, context = {}) {
        ...
        if (name in featureOverrides)
            return featureOverrides[name];
        // Allow global overrides, e.g. for tests.
        if (name in this.globalOverrides)
            return this.globalOverrides[name];
        // Allow config to globally override a feature.
        if (name in config.features)
            return config.features[name];
        const featureIsEnabled = await queryOptionalRow(sql.is_feature_enabled, {
            name,
            ...DEFAULT_CONTEXT,
            ...context,
        }, z.boolean());
        ...
        if (config.devMode && 'course_id' in context) {
            ...
            if (Array.isArray(devModeFeatures)) {
                ...
                if (devModeFeatures.includes(name))
                    return true;
            }
     ...
     * @returns Whether or not the feature is enabled
     */
    async enabledFromLocals(name, locals) {
     ...
     */
    async runWithGlobalOverrides(overrides, fn) {
