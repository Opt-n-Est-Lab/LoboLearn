 ...
 *
 */
export async function questionFilePath(filename, questionDirectory, coursePath, question, nTemplates = 0) {
    ...
    if (question.template_directory) {
        ...
        if (result.rowCount === 0) {
            throw new error.HttpStatusError(500, `Could not find template question "${question.template_directory}" from question "${question.directory}"`);
        }
        const templateQuestion = result.rows[0];
        return await questionFilePath(filename, templateQuestion.directory, coursePath, templateQuestion, nTemplates + 1);
    }
    else {
