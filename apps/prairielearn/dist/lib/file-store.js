...
import { FileSchema, IdSchema } from './db-types.js';
 ...
 */
export async function uploadFile({ display_filename, contents, type, assessment_id, assessment_instance_id, instance_question_id, user_id, authn_user_id, storage_type, }) {
    ...
    const file_id = await sqldb.queryRow(sql.insert_file, {
        display_filename,
        storage_filename,
        type,
        assessment_id,
...
export async function getFile(file_id, data_type = 'buffer') {
    ...
    if (file.storage_type === StorageTypes.FileSystem) {
        if (config.filesRoot == null) {
            throw new Error(`config.filesRoot must be non-null to get file_id ${file_id} from file store`);
        }
        const filename = path.join(config.filesRoot, file.storage_filename);
        const contents = data_type === 'buffer' ? await fsPromises.readFile(filename) : fs.createReadStream(filename);
        return {
            contents,
            file,
        };
        ...
        if (config.fileStoreS3Bucket == null) {
            throw new Error(`config.fileStoreS3Bucket must be non-null to get file_id ${file_id} from file store`);
        }
        const contents = data_type === 'buffer'
