...
export async function getGettingStartedTasks({ course_id, }) {
    ...
    const courseHasAddedStaff = await queryRow(sql.select_course_has_staff, { course_id }, z.boolean());
    // Check if the course has at least one non-deleted question
    const courseHasQuestions = await queryRow(sql.select_course_has_questions, { course_id }, z.boolean());
    // Check if the course has at least one non-deleted assessment
    const courseHasAssessments = await queryRow(sql.select_course_has_assessments, { course_id }, z.boolean());
    const tasks = [
        {
            header: 'Add course staff (optional)',
            description: 'Invite users to the course staff to help manage and deliver the course. If you are working alone, you can skip this task.',
            link: 'staff',
            isComplete: courseHasAddedStaff,
        },
        {
            header: 'Create a question',
            description: "A question is a problem or task that tests a student's understanding of a specific concept.",
            link: 'questions',
            isComplete: courseHasQuestions,
        },
        {
            header: 'Create a course instance',
            description: 'A course instance contains the assessments and other configuration for a single offering of a course.',
            link: 'instances',
            isComplete: courseHasCourseInstance,
        },
        {
            header: 'Create an assessment',
            description: "An assessment is a collection of questions designed to build or evaluate a student's knowledge.",
