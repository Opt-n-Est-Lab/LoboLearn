 ...
 */
async function createEmptyRepository(client, repo) {
 ...
 */
async function addFileToRepo(client, repo, path, contents) {
...
export async function createCourseRepoJob(options, authn_user) {
    ...
    serverJob.executeInBackground(async (job) => {
        try {
            ...
            try {
                await sendCourseRequestMessage(`*Failed to create course "${options.short_name}"*\n\n` +
                    '```\n' +
                    `${err.message.trim()}\n` +
                    '```');
            }
