...
import { DateFromISOString, IdSchema, IntervalSchema, QuestionSchema, SubmissionSchema, VariantSchema, } from './db-types.js';
...
export async function insertSubmission({ submitted_answer, raw_submitted_answer, format_errors, gradable, broken, params, true_answer, feedback, credit, mode, variant_id, user_id, auth_user_id, client_fingerprint_id, }) {
    return await sqldb.runInTransactionAsync(async () => {
        ...
        // not be updated.
        const variant = await sqldb.queryRow(sql.update_variant_true_answer, { variant_id, params, true_answer }, VariantForSubmissionSchema);
        ...
        const delta = await sqldb.queryOptionalRow(sql.select_and_update_last_access, { user_id: variant.user_id, group_id: variant.group_id }, IntervalSchema.nullable());
        const submission_id = await sqldb.queryRow(sql.insert_submission, {
            variant_id,
            auth_user_id,
 ...
 */
export async function saveSubmission(submissionData, variant, question, variant_course) {
    const submission = {
        ...submissionData,
    ...
    if (question.workspace_image != null) {
        const workspace_id = await sqldb.queryOptionalRow(sql.select_workspace_id, { variant_id: submission.variant_id }, IdSchema);
    ...
    const { courseIssues, data } = await questionModule.parse(submission, variant, question, question_course);
    const studentMessage = 'Error parsing submission';
    const courseData = { variant, question, submission, course: variant_course };
    await writeCourseIssues(courseIssues, variant, submission.user_id, submission.auth_user_id, studentMessage, courseData);
...
async function selectSubmissionForGrading(variant_id, check_submission_id) {
    return sqldb.runInTransactionAsync(async () => {
        await sqldb.callAsync('variants_lock', [variant_id]);
        const variantData = await sqldb.queryOptionalRow(sql.select_variant_data, { variant_id }, VariantDataSchema);
        if (variantData == null)
            return null;
        ...
        if (variantData.instance_question_id == null) {
            if (variantData.grading_method === 'Manual')
                return null;
        }
        else {
        ...
        const submission = await sqldb.queryOptionalRow(sql.select_last_submission_of_variant, { variant_id }, SubmissionSchema);
        if (submission == null)
            return null;
        ...
        if (submission.score != null || // already graded
 ...
 */
export async function gradeVariant(variant, check_submission_id, question, variant_course, user_id, authn_user_id, overrideGradeRateCheck) {
    const question_course = await getQuestionCourse(question, variant_course);
    const submission = await selectSubmissionForGrading(variant.id, check_submission_id);
    if (submission == null)
        return;
    if (!overrideGradeRateCheck) {
        const resultNextAllowed = await sqldb.callRow('instance_questions_next_allowed_grade', [variant.instance_question_id], NextAllowedGradeSchema);
        if (resultNextAllowed.allow_grade_left_ms > 0)
            return;
    }
    ...
    if (question.grading_method === 'External') {
        ...
        const questionModule = questionServers.getModule(question.type);
        const { courseIssues, data } = await questionModule.grade(submission, variant, question, question_course);
        const hasFatalIssue = courseIssues.some((issue) => issue.fatal);
        const studentMessage = 'Error grading submission';
        const courseData = { variant, question, submission, course: variant_course };
        await writeCourseIssues(courseIssues, variant, user_id, submission.auth_user_id, studentMessage, courseData);
        ...
        // LTI updates.
        if (!grading_job_post_update.gradable)
            return;
        const assessment_instance_id = await sqldb.queryOptionalRow(sql.select_assessment_for_submission, { submission_id: submission.id }, IdSchema.nullable());
 ...
 * @returns submission_id
 */
export async function saveAndGradeSubmission(submissionData, variant, question, course, overrideGradeRateCheck) {
    const { submission_id, variant: updated_variant } = await saveSubmission(submissionData, variant, question, course);
    await gradeVariant(
    ...
    // function.
    updated_variant, submission_id, question, course, submissionData.user_id, submissionData.auth_user_id, overrideGradeRateCheck);
