...
import { loadSqlEquiv, queryOptionalRow, queryRow, queryRows, runInTransactionAsync, } from '@prairielearn/postgres';
...
export async function uploadInstanceGroups(assessment_id, csvFile, user_id, authn_user_id) {
    ...
    const { assessment_label, course_id, course_instance_id } = await queryRow(sql.select_assessment_info, { assessment_id }, AssessmentInfoSchema);
        ...
        await namedLocks.doWithLock(lockName, {
            ...
            await runInTransactionAsync(async () => {
                for (const { uid, groupname } of groupAssignments) {
                    await createOrAddToGroup(groupname, assessment_id, [uid], authn_user_id).then(() => successCount++, (err) => {
 ...
 * @returns The job sequence ID.
 */
export async function randomGroups(assessment_id, user_id, authn_user_id, max_group_size, min_group_size) {
    ...
    serverJob.executeInBackground(async (job) => {
        ...
        await namedLocks.doWithLock(lockName, {
            ...
            const studentsToGroup = await queryRows(sql.select_enrolled_students_without_group, { assessment_id }, UserSchema);
            _.shuffle(studentsToGroup);
            const numStudents = studentsToGroup.length;
            job.verbose(`There are ${numStudents} students enrolled in ${assessment_label} without a group`);
            job.verbose('----------------------------------------');
            job.verbose(`Creating groups with a size between ${min_group_size} and ${max_group_size}`);
            let groupsCreated = 0, studentsGrouped = 0;
            await runInTransactionAsync(async () => {
                // Find a group name of the format `groupNNN` that is not used
                const unusedGroupNameSuffix = await queryOptionalRow(sql.select_unused_group_name_suffix, { assessment_id }, z.number());
                // Create groups using the groups of maximum size where possible
                const userGroups = _.chunk(studentsToGroup.map((user) => user.uid), max_group_size);
                ...
                while (smallGroup && smallGroup.length < min_group_size) {
                    ...
                    if (usersToMove.length === 0) {
                        job.warn(`Could not create groups with the desired sizes. One group will have a size of ${smallGroup.length}`);
                        break;
                    }
            ...
            if (studentsGrouped !== 0) {
                job.verbose(`Successfully grouped ${studentsGrouped} students into ${groupsCreated} groups`);
            }
            if (errorCount !== 0) {
