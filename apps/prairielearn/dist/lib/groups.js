...
import { GroupConfigSchema, GroupRoleSchema, GroupSchema, IdSchema, UserSchema, } from './db-types.js';
...
export async function getGroupConfig(assessmentId) {
    return await sqldb.queryRow(sql.get_group_config, { assessment_id: assessmentId }, GroupConfigSchema);
}
/**
...
export async function getGroupId(assessmentId, userId) {
    return await sqldb.queryOptionalRow(sql.get_group_id, { assessment_id: assessmentId, user_id: userId }, IdSchema);
}
export async function getGroupInfo(group_id, groupConfig) {
...
async function getRolesInfo(groupId, groupMembers) {
    ...
    const result = await sqldb.queryRows(sql.get_role_assignments, { group_id: groupId }, RoleAssignmentSchema);
    const roleAssignments = _.groupBy(result, 'uid');
    // Get info on all group roles for the assessment
    const groupRoles = await sqldb.queryRows(sql.get_group_roles, { group_id: groupId }, GroupRoleWithCountSchema);
    // Identify errors for any roles where count is not between max and min (if they exist)
    const validationErrors = groupRoles.filter((role) => (role.minimum && role.count < role.minimum) || (role.maximum && role.count > role.maximum));
    ...
    const rolesAreBalanced = groupMembers.length < minimumRolesToFill ||
        Object.values(roleAssignments).every((roles) => roles.length === 1);
    // Check if users have no roles
    const usersWithoutRoles = groupMembers.filter((member) => roleAssignments[member.uid] === undefined);
    return {
        roleAssignments,
        groupRoles,
        validationErrors,
 ...
 */
export async function getQuestionGroupPermissions(instance_question_id, group_id, user_id) {
    const userPermissions = await sqldb.queryOptionalRow(sql.select_question_permissions, { instance_question_id, group_id, user_id }, z.object({ can_submit: z.boolean(), can_view: z.boolean() }));
    return userPermissions ?? { can_submit: false, can_view: false };
}
export async function getUserRoles(group_id, user_id) {
    return await sqldb.queryRows(sql.select_user_roles, { group_id, user_id }, GroupRoleSchema);
}
async function selectUserInCourseInstance({ uid, course_instance_id, }) {
    ...
    if ((await sqldb.callRow('users_is_instructor_in_course_instance', [user.user_id, course_instance_id], z.boolean())) ||
...
export async function addUserToGroup({ assessment_id, group_id, uid, authn_user_id, enforceGroupSize, }) {
    await sqldb.runInTransactionAsync(async () => {
        const group = await sqldb.queryOptionalRow(sql.select_and_lock_group, { group_id, assessment_id }, GroupForUpdateSchema);
        if (group == null) {
        ...
        const groupRoleId = group.has_roles
            ? await sqldb.queryOptionalRow(sql.select_suitable_group_role, { assessment_id, group_id: group.id, cur_size: group.cur_size }, IdSchema)
            : null;
...
export async function joinGroup(fullJoinCode, assessment_id, uid, authn_user_id) {
    ...
    try {
        await sqldb.runInTransactionAsync(async () => {
            const group = await sqldb.queryOptionalRow(sql.select_and_lock_group_by_name, { group_name, assessment_id }, GroupSchema);
...
export async function createGroup(group_name, assessment_id, uids, authn_user_id) {
    if (group_name.length > 30) {
        throw new GroupOperationError('The group name is too long. Use at most 30 alphanumerical characters.');
    }
    if (!group_name.match(/^[0-9a-zA-Z]+$/)) {
        throw new GroupOperationError('The group name is invalid. Only alphanumerical characters (letters and digits) are allowed.');
    }
    if (uids.length === 0) {
    ...
    try {
        await sqldb.runInTransactionAsync(async () => {
            let group_id;
            try {
                group_id = await sqldb.queryRow(sql.create_group, { assessment_id, authn_user_id, group_name }, IdSchema);
            }
            catch (err) {
...
export async function createOrAddToGroup(group_name, assessment_id, uids, authn_user_id) {
    await sqldb.runInTransactionAsync(async () => {
        const group = await sqldb.queryOptionalRow(sql.select_and_lock_group_by_name, { group_name, assessment_id }, GroupSchema);
        if (group == null) {
...
export function getGroupRoleReassignmentsAfterLeave(groupInfo, leavingUserId) {
    ...
    for (const roleId of roleIdsToReassign) {
        ...
        const userIdWithNoRoles = groupInfo.groupMembers.find((m) => !idsEqual(m.user_id, leavingUserId) &&
        ...
        const idxToUpdate = groupRoleAssignmentUpdates.findIndex(({ group_role_id }) => {
            const roleMin = groupInfo.rolesInfo?.groupRoles.find((role) => idsEqual(role.id, group_role_id))?.minimum ??
                0;
            return roleMin === 0;
        });
        if (idxToUpdate !== -1) {
        ...
        const assigneeUserId = groupInfo.groupMembers.find((m) => !idsEqual(m.user_id, leavingUserId) &&
            !groupRoleAssignmentUpdates.some((u) => idsEqual(u.group_role_id, roleId) && idsEqual(u.user_id, m.user_id)))?.user_id;
...
export async function leaveGroup(assessmentId, userId, authnUserId, checkGroupId = null) {
    await sqldb.runInTransactionAsync(async () => {
        ...
        if (checkGroupId != null && !idsEqual(groupId, checkGroupId)) {
            throw new error.HttpStatusError(403, 'Group ID does not match the user ID and assessment ID provided');
        }
        ...
        if (groupConfig.has_roles) {
            ...
            if (currentSize > 1) {
                ...
                const minRolesToFill = _.sum(groupInfo.rolesInfo?.groupRoles.map((role) => role.minimum ?? 0));
...
export function canUserAssignGroupRoles(groupInfo, user_id) {
    const assignerRoles = groupInfo.rolesInfo?.groupRoles
 ...
 */
export async function updateGroupRoles(requestBody, assessmentId, groupId, userId, hasStaffPermission, authnUserId) {
    await sqldb.runInTransactionAsync(async () => {
        const groupConfig = await getGroupConfig(assessmentId);
        const groupInfo = await getGroupInfo(groupId, groupConfig);
        ...
        const assignerRoleIds = groupInfo.rolesInfo?.groupRoles
            .filter((role) => role.can_assign_roles)
            .map((role) => role.id) ?? [];
        const assignerRoleFound = roleAssignments.some((roleAssignment) => assignerRoleIds.includes(roleAssignment.group_role_id));
...
export async function deleteGroup(assessment_id, group_id, authn_user_id) {
    const deleted_group_id = await sqldb.queryOptionalRow(sql.delete_group, { assessment_id, group_id, authn_user_id }, IdSchema);
...
export function getRoleNamesForUser(groupInfo, user) {
