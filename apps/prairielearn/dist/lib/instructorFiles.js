...
function getContextPaths(locals) {
    ...
    if (locals.navPage === 'course_admin') {
        ...
        const rootPath = path.join(coursePath, 'courseInstances', locals.course_instance.short_name, 'assessments', locals.assessment.tid);
        return {
            rootPath,
            invalidRootPaths: [],
...
export function getPaths(requestedPath, locals) {
    ...
    const hasEditPermission = locals.authz_data.has_course_permission_edit && !locals.course.example_course;
    const { rootPath, invalidRootPaths, cannotMove, clientDir, serverDir, testsDir, urlPrefix } = getContextPaths(locals);
...
//# sourceMappingURL=instructorFiles.js.map