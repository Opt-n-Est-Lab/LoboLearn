...
export async function updateScore(assessment_instance_id) {
    ...
    oauthParams.oauth_signature = oauthSignature.generate('POST', info.lis_outcome_service_url, oauthParams, info.secret, undefined, { encodeSignature: false });
    ...
    const imsx_codeMajor = _.get(result, [
    ...
    if (imsx_codeMajor === 'success') {
        logger.info(`ltiOutcomes.updateScore() ai_id=${assessment_instance_id} score=${score} returned ${result.imsx_POXEnvelopeResponse.imsx_POXHeader.imsx_POXResponseHeaderInfo.imsx_statusInfo.imsx_codeMajor}`);
    }
    else {
        logger.info(`ltiOutcomes.updateScore() ai_id=${assessment_instance_id} score=${score} did not return success, debugging follows:`);
...
*/
//# sourceMappingURL=ltiOutcomes.js.map