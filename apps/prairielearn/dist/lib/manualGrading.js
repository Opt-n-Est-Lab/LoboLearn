...
import { AssessmentQuestionSchema, IdSchema, RubricGradingItemSchema, RubricGradingSchema, RubricItemSchema, RubricSchema, } from './db-types.js';
...
const RubricDataSchema = RubricSchema.extend({
    rubric_items: z.array(RubricItemSchema.extend({
 ...
 */
export async function nextUngradedInstanceQuestionUrl(urlPrefix, assessment_id, assessment_question_id, user_id, prior_instance_question_id) {
    const instance_question_id = await sqldb.queryOptionalRow(sql.select_next_ungraded_instance_question, { assessment_id, assessment_question_id, user_id, prior_instance_question_id }, IdSchema);
 ...
 */
export async function selectRubricData({ assessment_question, submission, }) {
    // If there is no assessment question (e.g., in question preview), there is no rubric
    if (!assessment_question?.manual_rubric_id)
        return null;
    const rubric_data = await sqldb.queryOptionalRow(sql.select_rubric_data, {
    ...
    await async.eachLimit(rubric_data?.rubric_items || [], 3, async (item) => {
        item.description_rendered = await markdownToHtml(mustache.render(item.description || '', mustache_data), { inline: true });
        item.explanation_rendered = await markdownToHtml(mustache.render(item.explanation || '', mustache_data));
        item.grader_note_rendered = await markdownToHtml(mustache.render(item.grader_note || '', mustache_data));
    });
    return rubric_data;
}
/**
...
export async function populateManualGradingData(submission) {
    if (submission.manual_rubric_grading_id) {
        submission.rubric_grading = await sqldb.queryOptionalRow(sql.select_rubric_grading_data, { rubric_grading_id: submission.manual_rubric_grading_id }, RubricGradingDataSchema);
    }
    if (submission.feedback?.manual) {
        submission.feedback_manual_html = await markdownToHtml(submission.feedback?.manual?.toString() || '');
    }
}
 ...
 */
export async function updateAssessmentQuestionRubric(assessment_question_id, use_rubric, replace_auto_points, starting_points, min_points, max_extra_points, rubric_items, tag_for_manual_grading, authn_user_id) {
    ...
    if (use_rubric) {
        ...
        rubric_items.forEach((item) => {
            ...
            if (item.description.length > 100) {
                throw new Error('Rubric item description is too long, must be no longer than 100 characters. Use the explanation for further comments.');
            }
        });
    }
    await sqldb.runInTransactionAsync(async () => {
        const assessment_question = await sqldb.queryRow(sql.select_assessment_question_for_update, { assessment_question_id }, AssessmentQuestionSchema);
        if (use_rubric) {
            const max_points = (replace_auto_points
            ...
            if (max_points <= Number(min_points)) {
                throw new Error(`Question has no range of possible points. Rubric points are limited to a minimum of ${min_points} and a maximum of ${max_points}.`);
            }
        }
        ...
        if (!use_rubric) {
            ...
            new_rubric_id = await sqldb.queryRow(sql.insert_rubric, { starting_points, min_points, max_extra_points, replace_auto_points }, IdSchema);
        }
        else {
        ...
        if (use_rubric) {
            ...
            await async.eachOfSeries(rubric_items.map((item, number) => ({
...
async function recomputeInstanceQuestions(assessment_question_id, authn_user_id) {
    await sqldb.runInTransactionAsync(async () => {
        ...
        const instance_questions = await sqldb.queryRows(sql.select_instance_questions_to_update, { assessment_question_id }, InstanceQuestionToUpdateSchema);
        await async.eachSeries(instance_questions, async (instance_question) => {
            await updateInstanceQuestionScore(instance_question.assessment_id, instance_question.instance_question_id, instance_question.submission_id, null, // check_modified_at,
            { manual_rubric_data: instance_question }, authn_user_id);
        });
    });
}
 ...
 */
export async function insertRubricGrading(rubric_id, max_points, max_manual_points, rubric_items, adjust_points) {
    return sqldb.runInTransactionAsync(async () => {
        const { rubric_data, rubric_item_data } = await sqldb.queryRow(sql.select_rubric_items, { rubric_id, rubric_items: rubric_items?.map((item) => item.rubric_item_id) || [] }, z.object({ rubric_data: RubricSchema, rubric_item_data: z.array(RubricItemSchema) }));
        const sum_rubric_item_points = _.sum(rubric_items?.map((item) => (item.score ?? 1) *
            (rubric_item_data.find((db_item) => idsEqual(db_item.id, item.rubric_item_id))?.points ??
                0)));
        const computed_points = Math.min(Math.max(rubric_data.starting_points + sum_rubric_item_points, rubric_data.min_points), (rubric_data.replace_auto_points ? max_points : max_manual_points) +
            rubric_data.max_extra_points) + Number(adjust_points || 0);
        const rubric_grading_id = await sqldb.queryRow(sql.insert_rubric_grading, {
            rubric_id,
            computed_points,
 ...
 */
export async function updateInstanceQuestionScore(assessment_id, instance_question_id, submission_id, check_modified_at, score, authn_user_id, is_ai_graded = false) {
    return sqldb.runInTransactionAsync(async () => {
        const current_submission = await sqldb.queryRow(sql.select_submission_for_score_update, { assessment_id, instance_question_id, submission_id, check_modified_at }, SubmissionForScoreUpdateSchema);
        ...
        if (score?.partial_scores) {
            ...
            new_auto_score_perc =
                (100 *
                    _.sumBy(Object.values(score.partial_scores), (value) => (value?.score ?? 0) * (value?.weight ?? 1))) /
        ...
        if (current_submission.manual_rubric_id && score?.manual_rubric_data?.rubric_id) {
            const manual_rubric_grading = await insertRubricGrading(score?.manual_rubric_data?.rubric_id, current_submission.max_points ?? 0, current_submission.max_manual_points ?? 0, score?.manual_rubric_data?.applied_rubric_items || [], score?.manual_rubric_data?.adjust_points ?? 0);
            score.manual_points =
        ...
        else if (current_submission.manual_rubric_id &&
            score?.points == null &&
...
//# sourceMappingURL=manualGrading.js.map