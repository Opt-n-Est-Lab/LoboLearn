 ...
 */
export function getNavPageTabs(hasEnhancedNavigation) {
    const navPagesTabs = {
        public_question: [
            {
        ...
        question: [
            {
                ...
                iconClasses: 'fa fa-edit',
                tabLabel: 'Files',
                renderCondition: ({ authz_data, course, question }) => authz_data.has_course_permission_view && question.course_id === course.id,
            },
            {
                activeSubPage: 'preview',
                urlSuffix: ({ question }) => `/question/${question.id}/preview`,
            ...
            {
                activeSubPage: 'issues',
                urlSuffix: ({ question }) => `/course_admin/issues?q=is%3Aopen+qid%3A${encodeURIComponent(question.qid)}`,
                iconClasses: 'fas fa-bug',
                tabLabel: 'Issues',
                htmlSuffix: (resLocals) => IssueBadge({ count: resLocals.open_issue_count, suppressLink: true, className: 'ms-2' }),
                renderCondition: ({ course, question }) => question.course_id === course.id,
            },
        ],
        admin: [
...
//# sourceMappingURL=navPageTabs.js.map