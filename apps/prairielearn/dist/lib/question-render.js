...
import {} from '../components/QuestionContainer.types.js';
...
import { SubmissionBasicSchema, SubmissionDetailedSchema, SubmissionPanel, } from '../components/SubmissionPanel.html.js';
import { selectAndAuthzVariant, selectVariantsByInstanceQuestion } from '../models/variant.js';
import * as questionServers from '../question-servers/index.js';
import { config } from './config.js';
import { AssessmentInstanceSchema, AssessmentQuestionSchema, AssessmentSchema, AssessmentSetSchema, CourseInstanceSchema, CourseSchema, GradingJobSchema, GroupConfigSchema, IdSchema, IssueSchema, SubmissionSchema, } from './db-types.js';
 ...
 */
async function render(variant_course, renderSelection, variant, question, submission, submissions, question_course, locals) {
    const questionModule = questionServers.getModule(question.type);
    const { courseIssues, data } = await questionModule.render(renderSelection, variant, question, submission, submissions, question_course, locals);
...
export function buildQuestionUrls(urlPrefix, variant, question, instance_question) {
    ...
    if (!instance_question) {
        ...
        urls = {
            ...
            calculationQuestionGeneratedFileUrl: questionUrl + 'generatedFilesQuestion/variant/' + variant.id,
            clientFilesCourseUrl: questionUrl + 'clientFilesCourse',
            clientFilesQuestionGeneratedFileUrl: questionUrl + 'generatedFilesQuestion/variant/' + variant.id,
            baseUrl: urlPrefix,
        };
    }
    else {
...
function buildLocals({ variant, question, instance_question, group_role_permissions, assessment, assessment_instance, assessment_question, group_config, authz_result, }) {
    const locals = {
 ...
 */
export async function getAndRenderVariant(variant_id, variant_seed, locals, options) {
    ...
    resultLocals.question_is_shared = await sqldb.queryRow(sql.select_is_shared, { question_id: locals.question.id }, z.boolean());
    const variant = await run(async () => {
        if (variant_id != null) {
            ...
            return await ensureVariant(locals.question.id, instance_question_id, locals.user.user_id, locals.authn_user.user_id, course_instance_id, locals.course, question_course, options, require_open, locals.client_fingerprint_id ?? null);
        }
    });
    resultLocals.variant = variant;
    const { urlPrefix, course, course_instance, question, instance_question, assessment, assessment_instance, assessment_question, group_config, group_role_permissions, authz_data, authz_result, } = locals;
    ...
    // actually render.
    const basicSubmissions = await sqldb.queryRows(sql.select_basic_submissions, { variant_id: variant.id }, SubmissionBasicSchema);
    ...
    const submissions = await run(async () => {
        ...
        const submissionDetails = await sqldb.queryRows(sql.select_detailed_submissions, { submission_ids: submissionsToRender.map((s) => s.id) }, SubmissionDetailedSchema);
    ...
    const htmls = await render(course, renderSelection, variant, question, submission, submissions.slice(0, MAX_RECENT_SUBMISSIONS), question_course, locals);
    ...
    resultLocals.issues = await sqldb.queryRows(sql.select_issues, {
 ...
 */
export async function renderPanelsForSubmission({ unsafe_submission_id, question, instance_question, variant, user, urlPrefix, questionContext, questionRenderContext, authorizedEdit, renderScorePanels, groupRolePermissions, }) {
    const submissionInfo = await sqldb.queryOptionalRow(sql.select_submission_info, {
    ...
    const { submission, next_instance_question, assessment_question, assessment_instance, assessment, assessment_set, variant_course, question_course, course_instance, submission_index, submission_count, grading_job, formatted_date, user_uid, question_number, group_config, } = submissionInfo;
    const previous_variants = variant.instance_question_id == null || assessment_instance == null
        ? null
    ...
    await async.parallel([
        async () => {
            ...
            const htmls = await render(variant_course, { answer: renderScorePanels && locals.showTrueAnswer, submissions: true, question: false }, variant, question, submission, submissions, question_course, locals);
        ...
        async () => {
            ...
            if (assessment == null || assessment_set == null || assessment_instance == null)
                return;
        ...
        async () => {
            ...
            const group_info = await run(async () => {
                if (!assessment_instance?.group_id || !group_config)
                    return null;
        ...
        async () => {
            ...
            if (variant.instance_question_id == null || next_instance_question.id == null)
                return;
            let nextQuestionGroupRolePermissions = null;
            let userGroupRoles = 'None';
            if (assessment_instance?.group_id && group_config?.has_roles) {
                nextQuestionGroupRolePermissions = await getQuestionGroupPermissions(next_instance_question.id, assessment_instance.group_id, user.user_id);
                userGroupRoles =
...
//# sourceMappingURL=question-render.js.map