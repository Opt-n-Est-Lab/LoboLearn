 ...
 */
async function createTestSubmission(variant, question, variant_course, test_type, user_id, authn_user_id) {
    ...
    const { courseIssues, data } = await questionModule.test(variant, question, question_course, test_type);
 ...
 * @param authn_user_id - The currently authenticated user.
 */
async function testVariant(variant, question, course, test_type, user_id, authn_user_id) {
    const expected_submission_id = await createTestSubmission(variant, question, course, test_type, user_id, authn_user_id);
    ...
    const { submission_id: test_submission_id, variant: updated_variant } = await saveSubmission(submission_data, variant, question, course);
    await gradeVariant(updated_variant, test_submission_id, question, course, user_id, authn_user_id, true);
 ...
 * @param test_type - The type of test to run.
 */
async function testQuestion(question, course_instance, variant_course, test_type, authn_user_id, user_id) {
    ...
    try {
        variant = await ensureVariant(question.id, instance_question_id, authn_user_id, authn_user_id, course_instance_id, variant_course, question_course, options, require_open, client_fingerprint_id);
    }
    finally {
    ...
    if (!variant.broken_at) {
        ...
        try {
            ({ expected_submission, test_submission } = await testVariant(variant, question, variant_course, test_type, user_id, authn_user_id));
        }
        finally {
 ...
 * @param authn_user_id - The currently authenticated user.
 */
async function runTest(logger, showDetails, question, course_instance, course, test_type, user_id, authn_user_id) {
    logger.verbose('Testing ' + question.qid);
    const { variant, expected_submission, test_submission, stats } = await testQuestion(question, course_instance, course, test_type, authn_user_id, user_id);
    if (showDetails) {
    ...
    const issueCount = await sqldb.queryRow(sql.select_issue_count_for_variant, { variant_id: variant.id }, z.number());
    if (issueCount > 0) {
 ...
 */
export async function startTestQuestion(count, showDetails, question, course_instance, course, user_id, authn_user_id) {
    let success = true;
    ...
    serverJob.executeInBackground(async (job) => {
        for (const iter of Array(count * test_types.length).keys()) {
            ...
            const result = await runTest(job, showDetails, question, course_instance, course, type, user_id, authn_user_id);
...
//# sourceMappingURL=question-testing.js.map