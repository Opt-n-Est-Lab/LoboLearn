...
import { IdSchema, VariantSchema } from './db-types.js';
...
async function makeVariant(question, course, options) {
    ...
    if (question.workspace_image !== null) {
        ...
        variant.params['_workspace_required_file_names'] = (question.workspace_graded_files || []).filter((file) => !fg.isDynamicPattern(file, workspaceFastGlobDefaultOptions));
        if (!('_required_file_names' in variant.params)) {
            variant.params['_required_file_names'] = [];
        }
        variant.params['_required_file_names'] = variant.params['_required_file_names'].concat(variant.params['_workspace_required_file_names']);
    }
    if (!variant.broken) {
        const { courseIssues: prepareCourseIssues, data } = await questionModule.prepare(question, course, variant);
 ...
 */
export async function getDynamicFile(filename, variant, question, variant_course, user_id, authn_user_id) {
    ...
    const { courseIssues, data: fileData } = await questionModule.file(filename, variant, question, question_course);
...
async function lockAssessmentInstanceForInstanceQuestion(instance_question_id) {
    const assessment_instance_id = await sqldb.queryOptionalRow(sql.select_and_lock_assessment_instance_for_instance_question, { instance_question_id }, IdSchema);
    if (assessment_instance_id == null) {
        throw new error.HttpStatusError(404, 'Instance question not found');
    }
}
async function selectVariantForInstanceQuestion(instance_question_id, require_open) {
    return await sqldb.queryOptionalRow(sql.select_variant_for_instance_question, { instance_question_id, require_open }, VariantWithFormattedDateSchema);
}
/**
 ...
 */
async function makeAndInsertVariant(question_id, instance_question_id, user_id, authn_user_id, course_instance_id, variant_course, question_course, options, require_open, client_fingerprint_id) {
    const question = await selectQuestion(question_id, instance_question_id);
    const { courseIssues, variant: variantData } = await makeVariant(question, question_course, options);
    const variant = await sqldb.runInTransactionAsync(async () => {
        let real_user_id = user_id;
        let real_group_id = null;
        let new_number = 1;
        if (instance_question_id != null) {
            await lockAssessmentInstanceForInstanceQuestion(instance_question_id);
            const instance_question = await sqldb.queryOptionalRow(sql.select_instance_question_data, { instance_question_id }, InstanceQuestionDataSchema);
            ...
            const existing_variant = await selectVariantForInstanceQuestion(instance_question_id, require_open);
            ...
            new_number = await sqldb.queryOptionalRow(sql.next_variant_number, { instance_question_id }, z.number().nullable());
        }
        else {
            if (question_id == null) {
                throw new Error('Attempt to create a variant without a question ID or instance question ID');
            }
            if (user_id == null) {
        ...
        return await sqldb.queryRow(sql.insert_variant, {
            ...variantData,
 ...
 */
export async function ensureVariant(question_id, instance_question_id, user_id, authn_user_id, course_instance_id, variant_course, question_course, options, require_open, client_fingerprint_id) {
    if (instance_question_id != null) {
    ...
    return await makeAndInsertVariant(question_id, instance_question_id, user_id, authn_user_id, course_instance_id, variant_course, question_course, options, require_open, client_fingerprint_id);
}
/**
...
//# sourceMappingURL=question-variant.js.map