 ...
 * @returns The ID of the job sequence.
 */
export async function uploadAssessmentInstanceScores(assessment_id, csvFile, user_id, authn_user_id) {
    if (csvFile == null) {
        throw new Error('No CSV file uploaded');
    }
    const { assessment_label, course_instance_id, course_id } = await sqldb.queryRow(sql.select_assessment_info, { assessment_id }, AssessmentInfoSchema);
...
async function updateInstanceQuestionFromJson(json, assessment_id, authn_user_id) {
    ...
    const uid_or_group = getJsonPropertyOrNull(json, 'group_name') ?? getJsonPropertyOrNull(json, 'uid');
    const ai_number = getJsonPropertyOrNull(json, 'instance');
    const qid = getJsonPropertyOrNull(json, 'qid');
    return await sqldb.runInTransactionAsync(async () => {
        const submission_data = await sqldb.queryOptionalRow(sql.select_submission_to_update, {
            assessment_id,
            submission_id,
            uid_or_group,
            ai_number,
        ...
        if (submission_data == null) {
            throw new Error(`Could not locate submission with id=${submission_id}, instance=${ai_number}, uid/group=${uid_or_group}, qid=${qid} for this assessment.`);
        }
        if (uid_or_group !== null && submission_data.uid_or_group !== uid_or_group) {
            throw new Error(`Found submission with id=${submission_id}, but uid/group does not match ${uid_or_group}.`);
        }
        ...
        if (Object.values(new_score).some((value) => value != null)) {
            await manualGrading.updateInstanceQuestionScore(assessment_id, submission_data.instance_question_id, submission_data.submission_id, null, // modified_at
            new_score, authn_user_id);
            return true;
        }
        else {
            return false;
...
async function getAssessmentInstanceId(json, assessment_id) {
    if ('uid' in json) {
        return {
            id: json.uid,
            assessment_instance_id: await sqldb.queryOptionalRow(sql.select_assessment_instance_uid, {
                assessment_id,
                uid: json.uid,
        ...
        return {
            id: json.group_name,
            assessment_instance_id: await sqldb.queryOptionalRow(sql.select_assessment_instance_group, {
                assessment_id,
...
//# sourceMappingURL=score-upload.js.map