...
import { IdSchema, JobSchema, JobSequenceSchema } from './db-types.js';
import { JobSequenceWithJobsSchema } from './server-jobs.types.js';
...
class ServerJobImpl {
    ...
    output = '';
    lastSent = Date.now();
    constructor(jobSequenceId, jobId) {
    ...
    async executeInternal(fn, shouldThrow) {
        try {
            await fn(this);
            await this.finish();
        }
 ...
 */
export async function createServerJob(options) {
    const { job_sequence_id, job_id } = await queryRow(sql.insert_job_sequence, {
...
export async function selectJobsByJobSequenceId(jobSequenceId) {
...
export function connection(socket) {
    socket.on('joinJob', function (msg, callback) {
        ...
        queryRow(sql.select_job, { job_id: msg.job_id }, JobSchema).then((job) => {
    ...
    socket.on('joinJobSequence', function (msg, callback) {
        ...
        // Check authorization of the requester.
        if (!checkSignedToken(msg.token, { jobSequenceId: msg.job_sequence_id.toString() }, config.secretKey)) {
            logger.error(`joinJobSequence called with invalid token for job_sequence_id ${msg.job_sequence_id}`);
            return;
        }
        socket.join('jobSequence-' + msg.job_id);
        queryRow(sql.select_job_sequence, { job_sequence_id: msg.job_sequence_id }, JobSequenceSchema.extend({ job_count: z.coerce.number() })).then(({ job_count }) => {
            callback({ job_count });
        }, (err) => {
            Sentry.captureException(err);
            logger.error('socket.io joinJobSequence error selecting job_sequence_id ' + msg.job_sequence_id, err);
        });
    });
}
export async function errorAbandonedJobs() {
    const abandonedJobs = await queryRows(sql.select_abandoned_jobs, { timeout_secs: config.serverJobsAbandonedTimeoutSec }, z.object({ id: IdSchema, job_sequence_id: IdSchema.nullable() }));
...
export async function getJobSequence(job_sequence_id, course_id) {
    const jobSequence = await queryRow(sql.select_job_sequence_with_course_id_as_json, { job_sequence_id, course_id }, JobSequenceWithJobsSchema);
...
//# sourceMappingURL=server-jobs.js.map