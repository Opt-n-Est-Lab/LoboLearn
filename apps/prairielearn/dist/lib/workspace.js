...
import { CourseSchema, DateFromISOString, QuestionSchema, VariantSchema, WorkspaceHostSchema, WorkspaceSchema, } from './db-types.js';
...
export async function init() {
    ...
    socketServer.io
        ...
        if (!socket.handshake.auth.token ||
            !checkSignedToken(socket.handshake.auth.token, { workspace_id: socket.handshake.auth.workspace_id }, config.secretKey)) {
...
function connection(socket) {
    ...
    socket.on('joinWorkspace', (...args) => {
        ...
        sqldb.queryRow(sql.select_workspace, { workspace_id }, WorkspaceSchema).then((workspace) => callback({ workspace_id, state: workspace.state }), (err) => {
    ...
    socket.on('startWorkspace', () => {
        startup(workspace_id).catch(async (err) => {
            ...
            await workspaceUtils.updateWorkspaceState(workspace_id, 'stopped', `Error! Click "Reboot" to try again. Detail: ${err}`);
        });
    });
    socket.on('heartbeat', (...args) => {
        ...
        // TODO: remove this in the future once all clients have been updated.
        const callback = args.at(-1);
        sqldb.queryRow(sql.update_workspace_heartbeat_at_now, { workspace_id }, DateFromISOString).then((heartbeat_at) => callback({ workspace_id, heartbeat_at }), (err) => {
            callback({ errorMessage: err.message });
            Sentry.captureException(err);
        });
    });
...
async function controlContainer(workspace_id, action, options = {}) {
    const workspace_host = await sqldb.queryOptionalRow(sql.select_workspace_host, { workspace_id }, WorkspaceHostSchema);
    if (workspace_host == null) {
        throw new Error(`No host for workspace_id=${workspace_id}`);
    }
    if (!config.workspaceEnable)
        return;
    ...
    if (action === 'getGradedFiles') {
        ...
        if (contentDisposition == null)
            throw new Error('Content-Disposition is null');
        const match = contentDisposition.match(/^attachment; filename="(.*)"$/);
        if (!match)
            throw new Error(`Content-Disposition format error: ${contentDisposition}`);
...
async function startup(workspace_id) {
    ...
    await sqldb.runInTransactionAsync(async () => {
        ...
        const workspace = await sqldb.queryRow(sql.select_and_lock_workspace, { workspace_id }, WorkspaceSchema);
        ...
        const shouldTransitionToStopped = state === 'uninitialized' && workspace.state === 'uninitialized';
        if (shouldTransitionToStopped) {
            if (initializeResult !== null) {
                ...
                try {
                    ...
                    await fs.move(initializeResult.destinationPath, `${initializeResult.destinationPath}-bak-${timestampSuffix}`, { overwrite: true });
                }
                catch (err) {
        ...
        if (workspace.state === 'stopped' || shouldTransitionToStopped) {
            await workspaceUtils.updateWorkspaceState(workspace_id, 'launching', 'Assigning workspace host');
    ...
    while (true) {
        ...
        await workspaceUtils.updateWorkspaceMessage(workspace_id, `Deploying more computational resources (${t} seconds elapsed)`);
...
async function initialize(workspace_id) {
    const { workspace, variant, question, course } = await sqldb.queryRow(sql.select_workspace_data, { workspace_id }, WorkspaceDataSchema);
...
export async function generateWorkspaceFiles({ serverFilesCoursePath, questionBasePath, params, correctAnswers, targetPath, }) {
    ...
    const templateFiles = (await async
        .mapSeries(klaw(templatePath), async (file) => {
        const generatedFileName = path.relative(templatePath, file.path.replace(/\.mustache$/i, ''));
        try {
            if (!file.stats.isFile())
                return null;
            return {
                name: generatedFileName,
                buffer: mustache.render(await fsPromises.readFile(file.path, { encoding: 'utf-8' }), mustacheParams),
            };
        }
        catch (err) {
    ...
    const dynamicFiles = params?._workspace_files
        ?.map((file, i) => {
        ...
        try {
            ...
            if (file.questionFile || file.serverFilesCourseFile) {
                ...
                const localPath = path.join(basePath, file.questionFile ?? file.serverFilesCourseFile ?? '');
...
async function assignHost(workspace_id) {
    if (!config.workspaceEnable)
        return null;
    const workspaceHostId = await workspaceHostUtils.assignWorkspaceToHost(workspace_id, config.workspaceLoadHostCapacity);
...
export async function getGradedFiles(workspace_id) {
    ...
    if (workspace.state === 'running') {
        ...
        try {
            ...
            if (err instanceof SubmissionFormatError)
                throw err;
        }
    }
...
async function getGradedFilesFromFileSystem(workspace_id) {
    const { workspace_version, workspace_graded_files } = await sqldb.queryRow(sql.select_workspace_version_and_graded_files, { workspace_id }, WorkspaceVersionAndGradedFilesSchema);
    const zipPath = await tmp.tmpName({ postfix: '.zip' });
    ...
    let gradedFiles;
    try {
        gradedFiles = await workspaceUtils.getWorkspaceGradedFiles(remoteDir, workspace_graded_files ?? [], {
