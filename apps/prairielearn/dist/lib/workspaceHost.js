...
import { WorkspaceHostSchema, WorkspaceLogSchema, } from './db-types.js';
 ...
 */
export async function markWorkspaceHostUnhealthy(workspace_host_id, reason) {
    return await queryRow(sql.set_hosts_unhealthy, {
        workspace_host_id,
        reason,
    }, WorkspaceHostSchema);
}
 ...
 * @returns All updated workspace hosts
 */
export async function markAllWorkspaceHostsUnhealthy(reason) {
    return await queryRows(sql.set_hosts_unhealthy, { workspace_host_id: null, reason }, WorkspaceHostSchema);
}
export async function assignWorkspaceToHost(workspace_id, capacity) {
    return await queryOptionalRow(sql.assign_workspace_to_host, {
        workspace_id,
        capacity,
    }, z.string());
}
 ...
 */
export async function recaptureDrainingWorkspaceHosts(needed_hosts) {
    return await queryRow(sql.recapture_draining_hosts, { needed_hosts }, z.number());
}
export async function drainExtraWorkspaceHosts(surplus) {
 ...
 */
export async function findTerminableWorkspaceHosts(unhealthy_timeout_sec, launch_timeout_sec) {
    return await queryRows(sql.find_terminable_hosts, {
 ...
 */
export async function terminateWorkspaceHostsIfNotLaunching(instanceIds) {
    return await queryRows(sql.terminate_hosts_if_not_launching, { instance_ids: instanceIds }, WorkspaceLogSchema);
}
