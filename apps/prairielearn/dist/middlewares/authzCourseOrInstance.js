...
export async function authzCourseOrInstance(req, res) {
    ...
    if (params.course_id == null && params.course_instance_id == null) {
        throw new HttpStatusError(403, 'Access denied (both course_id and course_instance_id are null)');
    }
    ...
    res.locals.has_enhanced_navigation = await features.enabledFromLocals('enhanced-navigation', res.locals);
    res.locals.question_sharing_enabled = await features.enabledFromLocals('question-sharing', res.locals);
    ...
    // all override cookies and return with error
    if (!res.locals.authz_data.authn_has_course_permission_view &&
    ...
    // all override cookies and return with error
    if (!res.locals.authz_data.authn_has_course_permission_own &&
    ...
    if (isCourseInstance) {
        ...
        if (!res.locals.authz_data.authn_has_course_instance_permission_view &&
        ...
        // remove all override cookies and return with error
        if (!res.locals.authz_data.authn_has_course_instance_permission_edit &&
...
//# sourceMappingURL=authzCourseOrInstance.js.map