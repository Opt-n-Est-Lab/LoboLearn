...
export async function getClientFingerprintId(req, res) {
    const user_session_id = await sqldb.queryOptionalRow(sql.select_user_session_id, { session_id: req.session.id }, IdSchema);
    const params = {
        ip_address: req.ip,
    ...
    let client_fingerprint_id = await sqldb.queryOptionalRow(sql.select_client_fingerprint, params, IdSchema);
...
//# sourceMappingURL=clientFingerprint.js.map