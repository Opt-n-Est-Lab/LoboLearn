...
const redirectEffectiveAccessDenied = (err, req, res, next) => {
    ...
    if (res.locals?.authn_user?.user_id == null)
        return next(err);
    if (res.locals?.user?.user_id == null)
        return next(err);
    // we are only interested in cases where we are emulating a different user
    if (idsEqual(res.locals.authn_user.user_id, res.locals.user.user_id))
        return next(err);
    // check that we have a plainUrlPrefix
    if (res.locals.plainUrlPrefix == null)
        return next(err);
...
//# sourceMappingURL=redirectEffectiveAccessDenied.js.map