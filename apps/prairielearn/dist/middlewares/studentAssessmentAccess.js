...
export function checkStudentAssessmentAccess(req, res) {
    ...
    if (!showClosedAssessment && (!assessmentInstanceOpen || !assessmentActive)) {
        ...
        res.status(403).send(StudentAssessmentAccess({
...
export default asyncHandler(async (req, res, next) => {
    ...
    if (req.method === 'POST' && req.body.__action === 'regenerate_instance') {
        if (!canDeleteAssessmentInstance(res.locals)) {
            throw new HttpStatusError(403, 'You do not have permission to delete this assessment instance.');
        }
        await deleteAssessmentInstance(res.locals.assessment.id, res.locals.assessment_instance.id, res.locals.authn_user.user_id);
...
//# sourceMappingURL=studentAssessmentAccess.js.map