    ...
    const workspaceProxyOptions = {
        ...
        pathRewrite: async (_path, req) => {
            ...
            try {
                ...
                if (!match)
                    throw new Error(`Could not match path: ${path}`);
                ...
                if (workspace_url_rewrite == null) {
                    const sql = 'SELECT q.workspace_url_rewrite' +
                        ' FROM questions AS q' +
        ...
        router: async (req) => {
            ...
            const result = await queryZeroOrOneRowAsync("SELECT hostname FROM workspaces WHERE id = $workspace_id AND state = 'running';", { workspace_id });
...
//# sourceMappingURL=workspaceProxy.js.map