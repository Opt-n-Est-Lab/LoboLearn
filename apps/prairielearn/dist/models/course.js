...
export async function selectCourseById(course_id) {
    return await queryRow(sql.select_course_by_id, {
        course_id,
    }, CourseSchema);
}
export async function selectCourseByCourseInstanceId(course_instance_id) {
    return await queryRow(sql.select_course_by_instance_id, {
        course_instance_id,
    }, CourseSchema);
}
...
export async function getCourseCommitHash(coursePath) {
    try {
 ...
 */
export async function selectCoursesWithStaffAccess({ user_id, is_administrator, }) {
    const courses = await queryRows(sql.select_courses_with_staff_access, { user_id, is_administrator }, CourseWithPermissionsSchema);
    return courses;
}
/**
 ...
 * the results if necessary.
 */
export async function selectCoursesWithEditAccess({ user_id, is_administrator, }) {
...
export async function selectOrInsertCourseByPath(coursePath) {
    return await queryRow(sql.select_or_insert_course_by_path, { path: coursePath }, CourseSchema);
}
export async function deleteCourse({ course_id, authn_user_id, }) {
...
export async function insertCourse({ institution_id, short_name, title, display_timezone, path, repository, branch, authn_user_id, }) {
    return await runInTransactionAsync(async () => {
        const course = await queryRow(sql.insert_course, {
            institution_id,
            short_name,
            title,
            display_timezone,
 ...
 */
export async function updateCourseShowGettingStarted({ course_id, show_getting_started, }) {
 ...
 */
export async function updateCourseSharingName({ course_id, sharing_name }) {
