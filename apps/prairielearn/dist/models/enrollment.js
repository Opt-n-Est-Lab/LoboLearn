...
import { loadSqlEquiv, queryAsync, queryOptionalRow, runInTransactionAsync, } from '@prairielearn/postgres';
import { PotentialEnterpriseEnrollmentStatus, checkPotentialEnterpriseEnrollment, } from '../ee/models/enrollment.js';
...
export async function ensureEnrollment({ course_instance_id, user_id, }) {
 ...
 * an appropriate page.
 */
export async function ensureCheckedEnrollment({ institution, course, course_instance, authz_data, }) {
...
export async function getEnrollmentForUserInCourseInstance({ user_id, course_instance_id, }) {
    return await queryOptionalRow(sql.select_enrollment_for_user_in_course_instance, { user_id, course_instance_id }, EnrollmentSchema);
}
export async function generateAndEnrollUsers({ count, course_instance_id, }) {
