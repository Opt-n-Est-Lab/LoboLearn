...
import { callRow, loadSqlEquiv, queryAsync, queryOptionalRow, queryRow, queryRows, } from '@prairielearn/postgres';
import { IdSchema, SubmissionSchema, VariantSchema, } from '../lib/db-types.js';
...
export async function resetVariantsForAssessmentQuestion({ assessment_id, unsafe_assessment_question_id, authn_user_id, }) {
...
export async function resetVariantsForInstanceQuestion({ assessment_instance_id, unsafe_instance_question_id, authn_user_id, }) {
...
export async function selectVariantsByInstanceQuestion({ assessment_instance_id, instance_question_id, }) {
    return await queryRows(sql.select_variant_by_instance_question_id, { assessment_instance_id, instance_question_id }, SimpleVariantWithScoreSchema);
}
/**
 ...
 */
export async function selectUserOwnsVariant({ user_id, variant_id, }) {
...
export async function selectAndAuthzVariant(options) {
    const { unsafe_variant_id, variant_course, question_id, course_instance_id, instance_question_id = null, authz_data, authn_user, user, is_administrator, publicQuestionPreview, } = options;
    const variant = await queryOptionalRow(sql.select_variant_by_id, { variant_id: unsafe_variant_id }, VariantSchema);
    function denyAccess() {
    ...
    if (variant == null)
        denyAccess();
    if (!idsEqual(variant.question_id, question_id))
        denyAccess();
    if (!idsEqual(variant.course_id, variant_course.id))
        denyAccess();
    ...
    if (variant.instance_question_id && !instance_question_id) {
        ...
        let authnHasCourseInstancePermissionView = authz_data?.authn_has_course_instance_permission_view;
        ...
        // course instance.
        if (authnHasCourseInstancePermissionView == null ||
            hasCourseInstancePermissionView == null ||
            course_instance_id == null ||
            !idsEqual(course_instance_id, variant.course_instance_id)) {
            const authnUserPermissions = await callRow('authz_course_instance', [authn_user.user_id, variant.course_instance_id, is_administrator, new Date(), null], z.object({ has_course_instance_permission_view: z.boolean() }));
            const userPermissions = await callRow('authz_course_instance', [user.user_id, variant.course_instance_id, is_administrator, new Date(), null], z.object({ has_course_instance_permission_view: z.boolean() }));
