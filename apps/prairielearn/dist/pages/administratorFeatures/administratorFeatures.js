...
import { CourseInstanceSchema, CourseSchema, IdSchema, InstitutionSchema, } from '../../lib/db-types.js';
import { features } from '../../lib/features/index.js';
import { selectOptionalUserByUid } from '../../models/user.js';
import { AddFeatureGrantModalBody, AdministratorFeature, AdministratorFeatures, FeatureGrantRowSchema, } from './administratorFeatures.html.js';
const router = Router();
...
router.get('/', asyncHandler(async (req, res) => {
    res.send(AdministratorFeatures({
        features: features.allFeatures().sort(),
        resLocals: res.locals,
    }));
}));
router.get('/:feature', asyncHandler(async (req, res) => {
    const feature = validateFeature(req.params.feature);
    const featureGrants = await queryRows(sql.select_feature_grants, { name: feature }, FeatureGrantRowSchema);
    const institutions = await queryRows(sql.select_institutions, {}, InstitutionSchema);
    res.send(AdministratorFeature({
...
async function getEntitiesFromParams(params) {
    const institutions = await queryRows(sql.select_institutions, {}, InstitutionSchema);
    const courses = params.institution_id
        ? await queryRows(sql.select_courses_for_institution, { institution_id: params.institution_id }, CourseSchema)
        : [];
    const course_instances = params.course_id
        ? await queryRows(sql.select_course_instances_for_course, { course_id: params.course_id }, CourseInstanceSchema)
        : [];
    ...
    const courseHasCourseInstance = course_instances.some((ci) => ci.id === params.course_instance_id);
...
router.get('/:feature/modal', asyncHandler(async (req, res) => {
    const feature = validateFeature(req.params.feature);
    const query = AddFeatureGrantModalParamsSchema.parse(req.query);
    const { institutions, institution, courses, course, course_instances, course_instance } = await getEntitiesFromParams(query);
    res.send(AddFeatureGrantModalBody({
        feature,
...
router.post('/:feature', asyncHandler(async (req, res) => {
...
//# sourceMappingURL=administratorFeatures.js.map