...
router.get('/', asyncHandler(async (req, res, _next) => {
    ...
    const queries = await Promise.all(jsonList.map(async (f) => {
        const contents = AdministratorQueryJsonSchema.parse(await jsonLoad.readJSON(path.join(queriesDir, f)));
        const filePrefix = f.replace(/\.json$/, '');
        return { ...contents, filePrefix };
    }));
    res.send(AdministratorQueries({
        resLocals: res.locals,
        queries,
    }));
}));
export default router;
//# sourceMappingURL=administratorQueries.js.map