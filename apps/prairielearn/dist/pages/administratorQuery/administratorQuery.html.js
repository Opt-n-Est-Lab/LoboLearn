...
export function AdministratorQuery({ resLocals, query_run_id, query_run, queryFilename, info, recent_query_runs, }) {
    return PageLayout({
        resLocals,
            ...
            ? html `
              <div class="table-responsive">
                ...
                >
                  <thead>
                    <tr>
                      ${query_run.result.columns?.map((col) => renderHeader(query_run.result?.columns, col))}
                    </tr>
                  </thead>

                  <tbody>
                    ${query_run.result.rows?.map((row) => html `
                        <tr>
                          ${query_run.result?.columns?.map((col) => renderCell(row, col, query_run.result?.columns, info))}
                        </tr>
                      `)}
                  </tbody>
                </table>
          ...
          ${recent_query_runs.length > 0
            ? html `
                <table class="table table-sm" aria-label="Recent query runs">
                  ...
                  <tbody>
                    ${recent_query_runs.map((run) => html `
                        <tr>
                          <td>
...
function shouldRenderColumn(columns, col) {
...
function renderHeader(columns, col) {
    if (!shouldRenderColumn(columns, col))
        return '';
    return html `<th>${col}</th>`;
}
function renderCell(row, col, columns, info) {
    if (!shouldRenderColumn(columns, col))
        return '';
...
//# sourceMappingURL=administratorQuery.html.js.map