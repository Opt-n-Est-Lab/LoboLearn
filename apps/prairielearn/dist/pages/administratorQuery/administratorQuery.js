...
import { IdSchema, QueryRunSchema } from '../../lib/db-types.js';
import * as jsonLoad from '../../lib/json-load.js';
import { AdministratorQuery, AdministratorQuerySchema, QueryRunRowSchema, } from './administratorQuery.html.js';
...
router.get('/:query', asyncHandler(async (req, res, next) => {
    ...
    const info = AdministratorQuerySchema.parse(await jsonLoad.readJSON(path.join(queriesDir, jsonFilename)));
    let query_run_id = null;
    let query_run = null;
    ...
    if (req.query.format === 'json') {
        ...
        const recent_query_runs = await sqldb.queryRows(sql.select_recent_query_runs, { query_name: req.params.query }, QueryRunRowSchema);
        res.send(AdministratorQuery({
...
router.post('/:query', asyncHandler(async (req, res, _next) => {
    ...
    const query_run_id = await sqldb.queryRow(sql.insert_query_run, {
...
//# sourceMappingURL=administratorQuery.js.map