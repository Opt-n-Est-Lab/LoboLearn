...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const ltiResult = await sqldb.queryOptionalRow(sql.lookup_credential, { consumer_key: parameters.oauth_consumer_key }, LtiCredentialsSchema);
    if (!ltiResult)
        throw new HttpStatusError(403, 'Unknown consumer_key');
    const genSignature = oauthSignature.generate('POST', ltiRedirectUrl, parameters, 
    // TODO: column should be `NOT NULL`
    ltiResult.secret, undefined, { encodeSignature: false });
    ...
    if (!parameters.user_id) {
        throw new HttpStatusError(400, 'Authentication problem: UserID required. Anonymous access disabled.');
    }
    ...
    const authUid = parameters.user_id + '@' + parameters.context_id + '::ciid=' + ltiResult.course_instance_id;
    ...
    const userResult = await sqldb.callOptionalRow('users_select_or_insert_and_enroll_lti', [
        authUid,
        authName,
    ...
    if (linkResult.rows[0].assessment_id) {
        ...
        res.redirect(`${res.locals.urlPrefix}/course_instance/${ltiResult.course_instance_id}/assessment/${linkResult.rows[0].assessment_id}/`);
    }
    else {
        ...
        res.redirect(`${res.locals.urlPrefix}/course_instance/${ltiResult.course_instance_id}/instructor/instance_admin/lti`);
    }
}));
export default router;
//# sourceMappingURL=authCallbackLti.js.map