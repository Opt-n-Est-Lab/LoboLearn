...
router.get('/', asyncHandler(async (req, res, _next) => {
    ...
    const oauth2Client = new OAuth2Client(config.googleClientId, config.googleClientSecret, config.googleRedirectUrl);
    ...
    if (!identity.email)
        throw new Error('Google auth response missing email');
    const authnParams = {
        uid: identity.email,
...
//# sourceMappingURL=authCallbackOAuth2.js.map