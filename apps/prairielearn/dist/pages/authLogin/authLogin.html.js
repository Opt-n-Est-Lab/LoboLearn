...
export function AuthLogin({ institutionAuthnProviders, service, resLocals, }) {
    return LoginPageContainer({
      ...
      ${institutionAuthnProviders?.length
            ...
            <div class="login-methods">
              ${institutionAuthnProviders.map((provider) => html `
...
export function AuthLoginUnsupportedProvider({ supportedProviders, institutionId, service, resLocals, }) {
    ...
    const hasNonDefaultProviders = supportedProviders.find((p) => p.name !== 'LTI' && p.is_default === false);
    const showSaml = supportsSaml && defaultProvider?.name !== 'SAML';
    const showShib = config.hasShib &&
...
function DevModeLogin({ csrfToken }) {
    return html `
    <form method="POST">
      <div class="mb-3">
...
//# sourceMappingURL=authLogin.html.js.map