...
import { AuthLogin, AuthLoginUnsupportedProvider, } from './authLogin.html.js';
...
router.get('/', asyncHandler(async (req, res, _next) => {
    ...
    if (req.query.unsupported_provider === 'true') {
        ...
        const supportedProviders = await queryRows(sql.select_supported_providers_for_institution, {
            institution_id: institutionId,
        }, InstitutionSupportedProvidersSchema);
        res.send(AuthLoginUnsupportedProvider({
            institutionId,
            supportedProviders,
            service,
    ...
    const institutionAuthnProvidersRes = await queryRows(sql.select_institution_authn_providers, InstitutionAuthnProviderSchema);
    const institutionAuthnProviders = institutionAuthnProvidersRes
        .map((provider) => {
        // Special case: omit the default institution in production.
        if (provider.id === '1' && config.devMode === false)
            return null;
        let url = null;
