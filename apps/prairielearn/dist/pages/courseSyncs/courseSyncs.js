...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const jobSequences = await queryRows(sql.select_sync_job_sequences, { course_id: res.locals.course.id }, JobSequenceRowSchema);
    const images = await queryRows(sql.question_images, { course_id: res.locals.course.id }, ImageRowSchema);
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'pull') {
        ...
        const questionId = await queryOptionalRow(sql.check_question_with_image, { course_id: res.locals.course.id, image: req.body.single_image }, IdSchema);
        if (questionId == null) {
            throw new HttpStatusError(400, 'Image not found in any question for this course');
        }
        const jobSequenceId = await syncHelpers.ecrUpdate([{ image: req.body.single_image }], res.locals);
        res.redirect(`${res.locals.urlPrefix}/jobSequence/${jobSequenceId}`);
    }
...
//# sourceMappingURL=courseSyncs.js.map