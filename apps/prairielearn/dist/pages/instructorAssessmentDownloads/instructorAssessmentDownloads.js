...
import { AssessmentInstanceSchema, AssessmentQuestionSchema, GroupSchema, InstanceQuestionSchema, QuestionSchema, RubricGradingItemSchema, RubricGradingSchema, SubmissionSchema, UserSchema, VariantSchema, } from '../../lib/db-types.js';
import { getGroupConfig } from '../../lib/groups.js';
import { assessmentFilenamePrefix } from '../../lib/sanitize-name.js';
import { InstructorAssessmentDownloads, } from './instructorAssessmentDownloads.html.js';
...
function getFilenames(locals) {
    const prefix = assessmentFilenamePrefix(locals.assessment, locals.assessment_set, locals.course_instance, locals.course);
    const filenames = {
...
function extractFiles(row, makeFilename) {
    ...
    // v3 question data.
    return row.submitted_answer?._files?.map((file) => ({
        filename: makeFilename(file.name),
        contents: parseFileContents(file.contents),
    }));
}
function extractFilesForSubmissions(row) {
...
function extractFilesForManualGrading(row) {
    // This doesn't handle QIDs with slashes in them:
    // https://github.com/PrairieLearn/PrairieLearn/issues/7715
    //
    // We should probably rethink the directory structure that this will spit out.
...
async function pipeCursorToArchive(res, cursor, extractFiles) {
    ...
    for await (const rows of cursor.iterate(100)) {
        for (const row of rows) {
            ...
            const files = extractFiles(row)?.sort((a, b) => (a.filename ?? '').localeCompare(b.filename ?? ''));
            if (!files)
                continue;
            for (const file of files) {
                ...
                if (!file.filename || file.contents == null)
                    continue;
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    res.send(InstructorAssessmentDownloads({ resLocals: res.locals, filenames: getFilenames(res.locals) }));
}));
/*
...
router.get('/:filename', asyncHandler(async (req, res) => {
    ...
    let identityColumn = studentColumn.concat(usernameColumn.concat([
        ['Name', 'name'],
        ['Role', 'role'],
    ]));
    ...
    if (req.params.filename === filenames.scoresCsvFilename) {
        ...
        identityColumn = (res.locals.assessment.group_work ? groupNameColumn : studentColumn).map((pair) => [pair[1], pair[1]]);
        const columns = identityColumn.concat([
            ['Zone number', 'zone_number'],
            ['Zone title', 'zone_title'],
            ['qid', 'qid'],
    ...
    else if (req.params.filename === filenames.allSubmissionsCsvFilename ||
        ...
        const cursor = await sqldb.queryValidatedCursor(sql.assessment_instance_submissions, {
            assessment_id: res.locals.assessment.id,
            include_all,
            include_final,
            include_best,
    ...
    else if (req.params.filename === filenames.allFilesZipFilename ||
...
//# sourceMappingURL=instructorAssessmentDownloads.js.map