...
export function InstructorAssessmentInstance({ resLocals, logCsvFilename, assessment_instance_stats, assessment_instance_date_formatted, assessment_instance_duration, instance_questions, assessmentInstanceLog, }) {
    return PageLayout({
        resLocals,
      ...
      <script src="${nodeModulesAssetPath('tablesorter/dist/js/jquery.tablesorter.min.js')}"></script>
      <script src="${nodeModulesAssetPath('tablesorter/dist/js/jquery.tablesorter.widgets.min.js')}"></script>
    `,
        content: html `
          ...
          <tbody>
            ...
            ? html `
                      <button
                        ...
                        data-bs-content="${escapeHtml(EditTotalPointsForm({
                resLocals,
            }))}"
                      >
            ...
            ? html `
                      <button
                        ...
                        data-bs-content="${escapeHtml(EditTotalScorePercForm({
                resLocals,
            }))}"
                      >
                        <i class="fa fa-edit" aria-hidden="true"></i>
        ...
        >
          ...
          <tbody>
            ...
            return html `
                ...
                ? row.client_fingerprint && row.client_fingerprint_number !== null
                    ? html `
                          <td>
                            <button
                              type="button"
                              class="btn btn-xs color-${FINGERPRINT_COLORS[row.client_fingerprint_number % 6]}"
                              ...
                              data-bs-content="${escapeHtml(FingerprintContent({ fingerprint: row.client_fingerprint }))}"
                            >
                ...
                if (!resLocals.authz_data.has_course_permission_preview)
                    return text;
                return html `
...
function FingerprintContent({ fingerprint }) {
...
function EditTotalPointsForm({ resLocals }) {
    return html `
...
function EditTotalScorePercForm({ resLocals }) {
    return html `
...
function ResetQuestionVariantsModal({ csrfToken, groupWork, }) {
    return Modal({
...
//# sourceMappingURL=instructorAssessmentInstance.html.js.map