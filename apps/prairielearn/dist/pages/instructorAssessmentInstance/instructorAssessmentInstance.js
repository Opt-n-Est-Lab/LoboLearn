...
import { selectAssessmentInstanceLog, selectAssessmentInstanceLogCursor, updateAssessmentInstancePoints, updateAssessmentInstanceScore, } from '../../lib/assessment.js';
...
import { AssessmentInstanceStatsSchema, InstanceQuestionRowSchema, InstructorAssessmentInstance, } from './instructorAssessmentInstance.html.js';
...
function makeLogCsvFilename(locals) {
    return (assessmentFilenamePrefix(locals.assessment, locals.assessment_set, locals.course_instance, locals.course) +
...
router.get('/', asyncHandler(async (req, res, _next) => {
    ...
    const assessment_instance_stats = await sqldb.queryRows(sql.assessment_instance_stats, {
        assessment_instance_id: res.locals.assessment_instance.id,
    }, AssessmentInstanceStatsSchema);
    const dateDurationResult = await sqldb.queryRow(sql.select_date_formatted_duration, {
        assessment_instance_id: res.locals.assessment_instance.id,
    }, DateDurationResultSchema);
    const assessment_instance_date_formatted = dateDurationResult.assessment_instance_date_formatted;
    const assessment_instance_duration = dateDurationResult.assessment_instance_duration;
    const instance_questions = await sqldb.queryRows(sql.select_instance_questions, {
        assessment_instance_id: res.locals.assessment_instance.id,
    }, InstanceQuestionRowSchema);
    const assessmentInstanceLog = await selectAssessmentInstanceLog(res.locals.assessment_instance.id, false);
    res.send(InstructorAssessmentInstance({
...
router.get('/:filename', asyncHandler(async (req, res) => {
    ...
    if (req.params.filename === makeLogCsvFilename(res.locals)) {
        const cursor = await selectAssessmentInstanceLogCursor(res.locals.assessment_instance.id, false);
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'edit_total_points') {
        await updateAssessmentInstancePoints(res.locals.assessment_instance.id, req.body.points, res.locals.authn_user.user_id);
        await ltiOutcomes.updateScore(res.locals.assessment_instance.id);
        res.redirect(req.originalUrl);
    }
    else if (req.body.__action === 'edit_total_score_perc') {
        await updateAssessmentInstanceScore(res.locals.assessment_instance.id, req.body.score_perc, res.locals.authn_user.user_id);
        await ltiOutcomes.updateScore(res.locals.assessment_instance.id);
        res.redirect(req.originalUrl);
    }
    else if (req.body.__action === 'edit_question_points') {
        const { modified_at_conflict, grading_job_id } = await updateInstanceQuestionScore(res.locals.assessment.id, req.body.instance_question_id, null, // submission_id
        ...
        if (modified_at_conflict) {
            return res.redirect(`${res.locals.urlPrefix}/assessment/${res.locals.assessment.id}/manual_grading/instance_question/${req.body.instance_question_id}?conflict_grading_job_id=${grading_job_id}`);
        }
        res.redirect(req.originalUrl);
    }
...
//# sourceMappingURL=instructorAssessmentInstance.js.map