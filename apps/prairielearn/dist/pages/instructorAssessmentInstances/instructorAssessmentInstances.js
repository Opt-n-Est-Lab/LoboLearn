...
import { checkBelongs, deleteAllAssessmentInstancesForAssessment, deleteAssessmentInstance, gradeAllAssessmentInstances, gradeAssessmentInstance, } from '../../lib/assessment.js';
...
router.get('/raw_data.json', asyncHandler(async (req, res) => {
    ...
    const assessmentInstances = await sqldb.queryRows(sql.select_assessment_instances, {
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'close') {
        ...
        await gradeAssessmentInstance(assessment_instance_id, res.locals.user.user_id, res.locals.authn_user.user_id, requireOpen, close, overrideGradeRate, null);
        res.send(JSON.stringify({}));
    }
    else if (req.body.__action === 'delete') {
        const assessment_id = res.locals.assessment.id;
        const assessment_instance_id = req.body.assessment_instance_id;
        await deleteAssessmentInstance(assessment_id, assessment_instance_id, res.locals.authn_user.user_id);
        res.send(JSON.stringify({}));
    }
        ...
        const overrideGradeRate = true;
        const job_sequence_id = await gradeAllAssessmentInstances(assessment_id, res.locals.user.user_id, res.locals.authn_user.user_id, close, overrideGradeRate);
        res.redirect(res.locals.urlPrefix + '/jobSequence/' + job_sequence_id);
    }
    else if (req.body.__action === 'delete_all') {
        await deleteAllAssessmentInstancesForAssessment(res.locals.assessment.id, res.locals.authn_user.user_id);
        res.send(JSON.stringify({}));
    }
        ...
        const assessment_instance_id = req.body.assessment_instance_id;
        await checkBelongs(assessment_instance_id, assessment_id);
        const job_sequence_id = await regradeAssessmentInstance(assessment_instance_id, res.locals.user.user_id, res.locals.authn_user.user_id);
        res.redirect(res.locals.urlPrefix + '/jobSequence/' + job_sequence_id);
    }
        ...
        if (req.body.action === 'unlimited' || req.body.reopen_without_limit === 'true') {
            ...
            params.time_add = 0;
            params.exact_date = new Date(Temporal.PlainDateTime.from(req.body.date).toZonedDateTime(res.locals.course_instance.display_timezone).epochMilliseconds);
        }
...
//# sourceMappingURL=instructorAssessmentInstances.js.map