...
export function ManualGradingAssessment({ resLocals, questions, courseStaff, num_open_instances, }) {
    return PageLayout({
        resLocals,
...
function AssessmentQuestionRow({ resLocals, question, }) {
    const showGradingButton = resLocals.authz_data.has_course_instance_permission_edit &&
    ...
    const assignedGraders = (question.assigned_graders || [])
        .filter((u) => !idsEqual(u.user_id, resLocals.authz_data.user.user_id))
        .map((u) => u.name ?? u.uid);
    if (question.num_instance_questions_assigned > 0) {
        assignedGraders.unshift(html `<strong class="bg-warning rounded px-1">${currentUserName}</strong>`);
    }
...
function ProgressBar(partial, total) {
...
//# sourceMappingURL=assessment.html.js.map