...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const questions = await queryRows(sql.select_questions_manual_grading, {
    ...
    res.send(ManualGradingAssessment({
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'assign_graders') {
        ...
        const assignedGraderIds = Array.isArray(req.body.assigned_grader)
            ? req.body.assigned_grader
            : [req.body.assigned_grader];
        const allowedGraderIds = (await selectCourseInstanceGraderStaff({
            course_instance_id: res.locals.course_instance.id,
        })).map((user) => user.user_id);
        if (assignedGraderIds.some((graderId) => !allowedGraderIds.includes(graderId))) {
            flash('error', 'Selected graders do not have student data editor access to this course instance.');
            res.redirect(req.originalUrl);
            return;
        }
        await runInTransactionAsync(async () => {
            const numInstancesToGrade = await queryOptionalRow(sql.count_instance_questions_to_grade, {
                assessment_id: res.locals.assessment.id,
