...
import { GradingJobDataSchema, InstanceQuestion, } from './instanceQuestion.html.js';
...
async function prepareLocalsForRender(query, resLocals) {
    ...
    const variant_with_submission_id = await sqldb.queryOptionalRow(sql.select_variant_with_last_submission, { instance_question_id: resLocals.instance_question.id }, IdSchema);
    ...
    if (query.conflict_grading_job_id) {
        conflict_grading_job = await sqldb.queryOptionalRow(sql.select_grading_job_data, {
...
router.get('/grading_rubric_panels', asyncHandler(async (req, res) => {
    try {
...
const PostBodySchema = z.union([
    z.object({
        ...
        rubric_item_selected_manual: IdSchema.or(z.record(z.string(), IdSchema))
            .nullish()
            .transform((val) => val == null ? [] : typeof val === 'string' ? [val] : Object.values(val)),
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (body.__action === 'add_manual_grade') {
        const manual_rubric_data = res.locals.assessment_question.manual_rubric_id
            ...
            : undefined;
        const { modified_at_conflict, grading_job_id } = await manualGrading.updateInstanceQuestionScore(res.locals.assessment.id, res.locals.instance_question.id, body.submission_id, body.modified_at, {
        ...
        res.redirect(await manualGrading.nextUngradedInstanceQuestionUrl(res.locals.urlPrefix, res.locals.assessment.id, res.locals.assessment_question.id, res.locals.authz_data.user.user_id, res.locals.instance_question.id));
    }
    else if (body.__action === 'modify_rubric_settings') {
        try {
            await manualGrading.updateAssessmentQuestionRubric(res.locals.instance_question.assessment_question_id, body.use_rubric, body.replace_auto_points, body.starting_points, body.min_points, body.max_extra_points, Object.values(body.rubric_item), // rubric items
            body.tag_for_manual_grading, res.locals.authn_user.user_id);
        ...
        if (assigned_grader != null) {
            ...
            if (!courseStaff.some((staff) => idsEqual(staff.user_id, assigned_grader))) {
                throw new error.HttpStatusError(400, 'Assigned grader does not have Student Data Editor permission');
            }
        }
...
//# sourceMappingURL=instanceQuestion.js.map