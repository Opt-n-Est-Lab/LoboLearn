...
import {} from '../../../lib/manualGrading.js';
export function RubricInputSection({ resLocals, disable, }) {
    if (!resLocals.rubric_data)
        return '';
                ...
                <div class="js-manual-grading-percentage">
                  <div class="input-group input-group-sm">
                    <input
                      ...
                      data-max-points="${resLocals.assessment_question.max_manual_points ||
            resLocals.assessment_question.max_points}"
                      value="${Math.round(((rubric_grading?.adjust_points || 0) * 10000) /
            (resLocals.assessment_question.max_manual_points ||
                resLocals.assessment_question.max_points)) / 100 || ''}"
                      ${disable ? 'disabled' : ''}
                    />
...
function RubricItems({ rubric_items, rubric_grading_items, assessment_question, disable, }) {
    return rubric_items?.map((item) => RubricItem({
        item,
...
function RubricItem({ item, item_grading, assessment_question, disable, }) {
    return html `
    <div>
...
//# sourceMappingURL=rubricInputSection.html.js.map