...
import { AssessmentQuestionHeaders, AssessmentQuestionNumber, } from '../../components/AssessmentQuestions.html.js';
...
export function InstructorAssessmentQuestions({ resLocals, questions, }) {
    return PageLayout({
        resLocals,
        pageTitle: 'Questions',
...
function AssessmentQuestionsTable({ course, questions, urlPrefix, assessmentType, hasCoursePermissionPreview, hasCourseInstancePermissionEdit, }) {
    // If at least one question has a nonzero unlock score, display the Advance Score column
    const showAdvanceScorePercCol = questions.filter((q) => q.assessment_question_advance_score_perc !== 0).length >= 1;
      ...
      <table class="table table-sm table-hover" aria-label="Assessment questions">
        ...
        return html `
            ...
            : ''}
                ...
                <td>
                  ${question.other_assessments?.map((assessment) => AssessmentBadge({ urlPrefix, assessment }))}
                </td>
                <td class="text-right">
...
//# sourceMappingURL=instructorAssessmentQuestions.html.js.map