...
import { AssessmentCopyEditor, AssessmentDeleteEditor, AssessmentRenameEditor, FileModifyEditor, MultiEditor, propertyValueWithDefault, } from '../../lib/editors.js';
...
router.get('/', asyncHandler(async (req, res) => {
    const tids = await sqldb.queryRows(sql.tids, { course_instance_id: res.locals.course_instance.id }, z.string());
    const assessmentSets = await sqldb.queryRows(sql.select_assessment_sets, { course_id: res.locals.course.id }, AssessmentSetSchema);
    const assessmentModules = await sqldb.queryRows(sql.select_assessment_modules, { course_id: res.locals.course.id }, AssessmentModuleSchema);
    const host = getCanonicalHost(req);
    const studentLink = new URL(`${res.locals.plainUrlPrefix}/course_instance/${res.locals.course_instance.id}/assessment/${res.locals.assessment.id}`, host).href;
    const publicLink = new URL(`${res.locals.plainUrlPrefix}/public/course_instance/${res.locals.course_instance.id}/assessment/${res.locals.assessment.id}/questions`, host).href;
    const infoAssessmentPath = encodePath(path.join('courseInstances', res.locals.course_instance.short_name, 'assessments', res.locals.assessment.tid, 'infoAssessment.json'));
    ...
    if (infoAssessmentPathExists) {
        origHash = sha256(b64EncodeUnicode(await fs.readFile(fullInfoAssessmentPath, 'utf8'))).toString();
    }
    let assessmentGHLink = null;
    ...
    const canEdit = res.locals.authz_data.has_course_permission_edit && !res.locals.course.example_course;
    res.send(InstructorAssessmentSettings({
...
router.post('/', asyncHandler(async (req, res) => {
    if (req.body.__action === 'copy_assessment') {
        ...
        const assessmentId = await sqldb.queryRow(sql.select_assessment_id_from_uuid, { uuid: editor.uuid, course_instance_id: res.locals.course_instance.id }, IdSchema);
        ...
        const infoAssessmentPath = path.join(res.locals.course.path, 'courseInstances', res.locals.course_instance.short_name, 'assessments', res.locals.assessment.tid, 'infoAssessment.json');
        ...
        if (!/^[-A-Za-z0-9_/]+$/.test(req.body.aid)) {
            throw new error.HttpStatusError(400, `Invalid TID (was not only letters, numbers, dashes, slashes, and underscores, with no spaces): ${req.body.id}`);
        }
        ...
        assessmentInfo.allowIssueReporting = propertyValueWithDefault(assessmentInfo.allowIssueReporting, req.body.allow_issue_reporting === 'on', true);
        assessmentInfo.allowPersonalNotes = propertyValueWithDefault(assessmentInfo.allowPersonalNotes, req.body.allow_personal_notes === 'on', true);
        if (res.locals.assessment.type === 'Exam') {
            assessmentInfo.multipleInstance = propertyValueWithDefault(assessmentInfo.multipleInstance, req.body.multiple_instance === 'on', false);
            assessmentInfo.autoClose = propertyValueWithDefault(assessmentInfo.autoClose, req.body.auto_close === 'on', true);
            assessmentInfo.requireHonorCode = propertyValueWithDefault(assessmentInfo.requireHonorCode, req.body.require_honor_code === 'on', true);
        }
        ...
        const tid_new = run(() => {
            try {
                ...
                throw new error.HttpStatusError(400, `Invalid TID (could not be normalized): ${req.body.aid}`);
            }
        });
...
//# sourceMappingURL=instructorAssessmentSettings.js.map