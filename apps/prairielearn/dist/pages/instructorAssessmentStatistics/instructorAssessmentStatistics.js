...
import { AssessmentScoreHistogramByDateSchema, DurationStatSchema, InstructorAssessmentStatistics, UserScoreSchema, } from './instructorAssessmentStatistics.html.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const assessment = await sqldb.queryRow(sql.select_assessment, { assessment_id: res.locals.assessment.id }, AssessmentSchema);
    ...
    // change this.
    const durationStat = await sqldb.queryRow(sql.select_duration_stats, { assessment_id: res.locals.assessment.id }, DurationStatSchema);
    const assessmentScoreHistogramByDate = await sqldb.queryRows(sql.assessment_score_histogram_by_date, { assessment_id: res.locals.assessment.id }, AssessmentScoreHistogramByDateSchema);
    const userScores = await sqldb.queryRows(sql.user_scores, { assessment_id: res.locals.assessment.id }, UserScoreSchema);
    res.send(InstructorAssessmentStatistics({
...
//# sourceMappingURL=instructorAssessmentStatistics.js.map