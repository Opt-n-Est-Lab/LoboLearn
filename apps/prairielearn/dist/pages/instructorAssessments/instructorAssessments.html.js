...
import {} from './instructorAssessments.types.js';
...
export function InstructorAssessments({ resLocals, rows, assessmentIdsNeedingStatsUpdate, csvFilename, assessmentSets, assessmentModules, assessmentsGroupBy, }) {
    ...
    return PageLayout({
      ...
      ${EncodedData({ assessmentIdsNeedingStatsUpdate, urlPrefix }, 'stats-update-data')}
    `,
        content: html `
...
export function AssessmentStats({ row }) {
    const spinner = html `
...
function CreateAssessmentModal({ csrfToken, urlPrefix, assessmentSets, assessmentModules, assessmentsGroupBy, }) {
    return Modal({
      ...
      ${assessmentsGroupBy === 'Module'
            ...
            <div class="mb-3">
              ...
              <select
                ...
                required
              >
                ${assessmentModules.map((module) => html `<option value="${module.name}">${module.name}</option>`)}
              </select>
...
//# sourceMappingURL=instructorAssessments.html.js.map