...
import { loadSqlEquiv, queryCursor, queryOptionalRow, queryRow, queryRows, } from '@prairielearn/postgres';
import { updateAssessmentStatistics, updateAssessmentStatisticsForCourseInstance, } from '../../lib/assessment.js';
import { AssessmentModuleSchema, AssessmentSchema, AssessmentSetSchema, IdSchema, } from '../../lib/db-types.js';
import { AssessmentAddEditor } from '../../lib/editors.js';
import { courseInstanceFilenamePrefix } from '../../lib/sanitize-name.js';
import { AssessmentRowSchema, AssessmentStats, InstructorAssessments, } from './instructorAssessments.html.js';
const router = Router();
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const rows = await queryRows(sql.select_assessments, {
    ...
    const assessmentIdsNeedingStatsUpdate = rows
        ...
        .map((row) => row.id);
    const assessmentSets = await queryRows(sql.select_assessment_sets, { course_id: res.locals.course.id }, AssessmentSetSchema);
    let assessmentModules = [];
    if (res.locals.course_instance.assessments_group_by === 'Module') {
        assessmentModules = await queryRows(sql.select_assessment_modules, { course_id: res.locals.course.id }, AssessmentModuleSchema);
    }
    res.send(InstructorAssessments({
...
router.get('/stats/:assessment_id', asyncHandler(async (req, res) => {
    ...
    const row = await queryOptionalRow(sql.select_assessment, {
...
router.post('/', asyncHandler(async (req, res) => {
    if (req.body.__action === 'add_assessment') {
        ...
        if (!/^[-A-Za-z0-9_/]+$/.test(req.body.aid)) {
            throw new HttpStatusError(400, `Invalid aid (was not only letters, numbers, dashes, slashes, and underscores, with no spaces): ${req.body.aid}`);
        }
        if (!req.body.type) {
        ...
        const assessment_id = await queryRow(sql.select_assessment_id_from_uuid, { uuid: editor.uuid, course_instance_id: res.locals.course_instance.id }, IdSchema);
...
//# sourceMappingURL=instructorAssessments.js.map