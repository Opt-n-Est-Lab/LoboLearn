...
import {} from '../../models/course-instances.js';
export function InstructorCourseAdminInstances({ resLocals, courseInstances, }) {
    const initialStartDate = Temporal.Now.zonedDateTimeISO(resLocals.course.timeZone).with({
        hour: 0,
        minute: 1,
        second: 0,
    });
    const initialStartDateFormatted = formatDateYMDHM(new Date(initialStartDate.epochMilliseconds), resLocals.course.time_zone);
    const initialEndDate = initialStartDate.add({ months: 4 }).with({
        hour: 23,
        minute: 59,
        second: 0,
    });
    const initialEndDateFormatted = formatDateYMDHM(new Date(initialEndDate.epochMilliseconds), resLocals.course.time_zone);
    return PageLayout({
        resLocals,
...
function CreateCourseInstanceModal({ courseShortName, csrfToken, initialStartDateFormatted, initialEndDateFormatted, timezone, }) {
    return Modal({
...
//# sourceMappingURL=instructorCourseAdminInstances.html.js.map