...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const enrollmentCounts = await sqldb.queryRows(sql.select_enrollment_counts, { course_id: res.locals.course.id }, z.object({ course_instance_id: CourseInstanceSchema.shape.id, enrollment_count: z.number() }));
    ...
    if (req.body.__action === 'add_course_instance') {
        ...
        if (req.body.access_dates_enabled === 'on') {
            ...
            if (req.body.start_access_date) {
                startAccessDate = Temporal.PlainDateTime.from(req.body.start_access_date).toZonedDateTime(res.locals.course.display_timezone);
            }
            if (req.body.end_access_date) {
                endAccessDate = Temporal.PlainDateTime.from(req.body.end_access_date).toZonedDateTime(res.locals.course.display_timezone);
            }
        }
        if (startAccessDate &&
            endAccessDate &&
        ...
        const courseInstanceId = await sqldb.queryRow(sql.select_course_instance_id_from_uuid, {
            uuid: editor.uuid,
            course_id: res.locals.course.id,
        }, IdSchema);
...
//# sourceMappingURL=instructorCourseAdminInstances.js.map