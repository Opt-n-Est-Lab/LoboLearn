...
router.get('/', asyncHandler(async (req, res) => {
    const modules = await sqldb.queryRows(sql.select_assessment_modules, { course_id: res.locals.course.id }, AssessmentModuleSchema);
    res.send(InstructorCourseAdminModules({ resLocals: res.locals, modules }));
}));
export default router;
//# sourceMappingURL=instructorCourseAdminModules.js.map