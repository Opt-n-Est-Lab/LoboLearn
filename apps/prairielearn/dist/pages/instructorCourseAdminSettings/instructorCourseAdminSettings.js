...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const courseInfoExists = await fs.pathExists(path.join(res.locals.course.path, 'infoCourse.json'));
    const availableTimezones = await getCanonicalTimezones([res.locals.course.display_timezone]);
    let origHash = '';
    if (courseInfoExists) {
        origHash = sha256(b64EncodeUnicode(await fs.readFile(path.join(res.locals.course.path, 'infoCourse.json'), 'utf8'))).toString();
    }
    res.send(InstructorCourseAdminSettings({
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'update_configuration') {
        ...
        const courseInfo = JSON.parse(await fs.readFile(path.join(res.locals.course.path, 'infoCourse.json'), 'utf8'));
...
//# sourceMappingURL=instructorCourseAdminSettings.js.map