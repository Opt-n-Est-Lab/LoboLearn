...
import { InstructorCourseAdminSharing, SharingSetRowSchema, } from './instructorCourseAdminSharing.html.js';
const router = Router();
const sql = sqldb.loadSqlEquiv(import.meta.url);
async function selectCanChooseSharingName(course) {
    return (course.sharing_name === null ||
        !(await sqldb.queryOptionalRow(sql.select_shared_question_exists, {
            course_id: course.id,
        }, z.boolean().nullable())));
}
router.get('/', asyncHandler(async (req, res) => {
    ...
    const sharingSets = await sqldb.queryRows(sql.select_sharing_sets, { course_id: res.locals.course.id }, SharingSetRowSchema);
    const host = getCanonicalHost(req);
    const publicSharingLink = new URL(`${res.locals.plainUrlPrefix}/public/course/${res.locals.course.id}/questions`, host).href;
    const canChooseSharingName = await selectCanChooseSharingName(res.locals.course);
    res.send(InstructorCourseAdminSharing({
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'sharing_token_regenerate') {
        ...
        const consuming_course_id = await sqldb.queryOptionalRow(sql.course_sharing_set_add, {
        ...
        if (req.body.course_sharing_name.includes('/') ||
            req.body.course_sharing_name.includes('@') ||
            req.body.course_sharing_name === '') {
            throw new error.HttpStatusError(400, 'Course Sharing Name must be non-empty and is not allowed to contain "/" or "@".');
        }
        else {
            const canChooseSharingName = await selectCanChooseSharingName(res.locals.course);
            if (!canChooseSharingName) {
                throw new error.HttpStatusError(400, 'Unable to change sharing name. At least one question has been shared.');
            }
...
//# sourceMappingURL=instructorCourseAdminSharing.js.map