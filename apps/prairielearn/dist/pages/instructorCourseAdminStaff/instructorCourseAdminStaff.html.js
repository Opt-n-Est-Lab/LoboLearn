...
import { CourseInstancePermissionSchema, CourseInstanceSchema, CoursePermissionSchema, UserSchema, } from '../../lib/db-types.js';
...
function hasUnknownUsers(courseUsers) {
    return courseUsers.some((courseUser) => courseUser.user.name == null);
}
export function InstructorCourseAdminStaff({ resLocals, courseInstances, courseUsers, uidsLimit, githubAccessLink, }) {
    return PageLayout({
        resLocals,
        pageTitle: 'Staff',
        navContext: {
        ...
        <div class="card-header bg-primary text-white d-flex flex-wrap align-items-center">
          ...
          <div class="ms-auto">
            <button
              ...
              data-bs-content="${escapeHtml(CoursePermissionsRemoveStudentDataAccessForm({
            ...
            <button
              ...
              data-bs-content="${escapeHtml(CoursePermissionsDeleteNoAccessForm({
            csrfToken: resLocals.__csrf_token,
        }))}"
            ...
            <button
              ...
              data-bs-content="${escapeHtml(CoursePermissionsDeleteNonOwnersForm({
            csrfToken: resLocals.__csrf_token,
        }))}"
            ...
            <button
              ...
              data-bs-content="${escapeHtml(CoursePermissionsInsertForm({
            csrfToken: resLocals.__csrf_token,
            uidsLimit,
            courseInstances,
        }))}"
...
function CoursePermissionsRemoveStudentDataAccessForm({ csrfToken }) {
    return html `
...
function CoursePermissionsDeleteNoAccessForm({ csrfToken }) {
    return html `
...
function CoursePermissionsInsertForm({ csrfToken, uidsLimit, courseInstances, }) {
    return html `
            ...
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                >
                  ...
                  ${courseInstances.map((ci) => html ` <option value="${ci.id}">${ci.short_name}</option>`)}
                </select>
                <select
                  class="custom-select form-select-sm"
...
function CoursePermissionsDeleteNonOwnersForm({ csrfToken }) {
    return html `
...
function StaffTable({ csrfToken, courseUsers, authnUser, user, isAdministrator, }) {
    return html `
    <table class="table table-sm table-hover table-striped" aria-label="Course staff members">
      ...
      <tbody>
        ...
        const canChangeCourseRole = (courseUser.user.user_id !== authnUser.user_id &&
...
//# sourceMappingURL=instructorCourseAdminStaff.html.js.map