...
import { deleteAllCourseInstancePermissionsForCourse, deleteCourseInstancePermissions, deleteCoursePermissions, deleteCoursePermissionsForNonOwners, deleteCoursePermissionsForUsersWithoutAccess, insertCourseInstancePermissions, insertCoursePermissionsByUserUid, updateCourseInstancePermissionsRole, updateCoursePermissionsRole, } from '../../models/course-permissions.js';
import { CourseUsersRowSchema, InstructorCourseAdminStaff, } from './instructorCourseAdminStaff.html.js';
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const courseUsers = await sqldb.queryRows(sql.select_course_users, {
        course_id: res.locals.course.id,
    }, CourseUsersRowSchema);
    let githubAccessLink = null;
    ...
    res.send(InstructorCourseAdminStaff({
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'course_permissions_insert_by_user_uids') {
        ...
        if (uids.length === 0)
            throw new error.HttpStatusError(400, 'Empty list of UIDs');
        // Verify the requested course role is valid
        if (!['None', 'Previewer', 'Viewer', 'Editor', 'Owner'].includes(req.body.course_role)) {
            throw new error.HttpStatusError(400, `Invalid requested course role: ${req.body.course_role}`);
        }
        ...
        if (req.body.course_instance_id) {
            course_instance = course_instances.find((ci) => idsEqual(ci.id, req.body.course_instance_id));
            if (!course_instance) {
        ...
        if (course_instance &&
            ...
            throw new error.HttpStatusError(400, `Invalid requested course instance role: ${req.body.course_instance_role}`);
        }
        // Iterate through UIDs
        const result = await async.reduce(uids, { given_cp: [], not_given_cp: [], not_given_cip: [], errors: [] }, async (memo, uid) => {
            let user;
            try {
...
${given_cp_and_cip.join(',\n')}
        ...
        if (idsEqual(req.body.user_id, res.locals.user.user_id) &&
        ...
        if (idsEqual(req.body.user_id, res.locals.authn_user.user_id) &&
            !res.locals.authz_data.is_administrator) {
            throw new error.HttpStatusError(403, 'Owners cannot change their own course content access even if they are emulating another user');
        }
        ...
        if (idsEqual(req.body.user_id, res.locals.authn_user.user_id) &&
            !res.locals.authz_data.is_administrator) {
            throw new error.HttpStatusError(403, 'Owners cannot remove themselves from the course staff even if they are emulating another user');
        }
...
//# sourceMappingURL=instructorCourseAdminStaff.js.map