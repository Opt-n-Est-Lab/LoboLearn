...
router.get('/', asyncHandler(async (req, res) => {
    if (!(res.locals.authz_data.authn_has_course_permission_preview ||
        ...
        throw new HttpStatusError(403, 'Access denied (must be course previewer or student data viewer)');
    }
    const courseRoles = await queryRow(sql.select, {
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'reset') {
        ...
        setCookie(res, ['pl_requested_course_role', 'pl2_requested_course_role'], req.body.pl_requested_course_role, { maxAge: 60 * 60 * 1000 });
        setCookie(res, ['pl_requested_data_changed', 'pl2_requested_data_changed'], 'true');
        res.redirect(req.originalUrl);
    }
    else if (req.body.__action === 'changeCourseInstanceRole') {
        setCookie(res, ['pl_requested_course_instance_role', 'pl2_requested_course_instance_role'], req.body.pl_requested_course_instance_role, { maxAge: 60 * 60 * 1000 });
        setCookie(res, ['pl_requested_data_changed', 'pl2_requested_data_changed'], 'true');
        res.redirect(req.originalUrl);
    }
...
//# sourceMappingURL=instructorEffectiveUser.js.map