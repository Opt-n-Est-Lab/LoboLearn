...
router.post('/*', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'delete_file') {
        ...
        try {
            ...
            throw new Error(`Invalid old file path: ${req.body.working_path} / ${req.body.old_file_name}`);
        }
        if (!req.body.new_file_name) {
            throw new Error(`Invalid new file name (was falsy): ${req.body.new_file_name}`);
        }
        if (!/^(?:[-A-Za-z0-9_]+|\.\.)(?:\/(?:[-A-Za-z0-9_]+|\.\.))*(?:\.[-A-Za-z0-9_]+)?$/.test(req.body.new_file_name)) {
            throw new Error(`Invalid new file name (did not match required pattern): ${req.body.new_file_name}`);
        }
        let newPath;
        try {
            ...
            throw new Error(`Invalid new file path: ${req.body.working_path} / ${req.body.new_file_name}`);
        }
        if (oldPath === newPath) {
...
//# sourceMappingURL=instructorFileBrowser.js.map