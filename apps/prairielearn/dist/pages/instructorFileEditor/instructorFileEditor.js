...
router.get('/*', asyncHandler(async (req, res) => {
    ...
    if (res.locals.course.example_course) {
        res.status(403).send(InsufficientCoursePermissionsCardPage({
    ...
    const { errors: sync_errors, warnings: sync_warnings } = await getErrorsAndWarningsForFilePath(res.locals.course.id, relPath);
    const editorData = {
    ...
    if (draftEdit != null) {
        if (draftEdit.fileEdit.job_sequence_id != null) {
            draftEdit.jobSequence = await getJobSequence(draftEdit.fileEdit.job_sequence_id, res.locals.course.id);
        }
...
async function readDraftEdit({ user_id, course_id, dir_name, file_name, authn_user_id, }) {
    const fileEdit = await queryOptionalRow(sql.select_file_edit, { user_id, course_id, dir_name, file_name, max_age_sec: 24 * 60 * 60 }, FileEditSchema);
    ...
    const deletedFileEdits = await queryRows(sql.soft_delete_file_edit, { user_id, course_id, dir_name, file_name }, IdSchema.nullable());
...
async function writeDraftEdit({ user_id, authn_user_id, course_id, dir_name, file_name, orig_hash, editContents, }) {
    const deletedFileEdits = await queryRows(sql.soft_delete_file_edit, { user_id, course_id, dir_name, file_name }, IdSchema.nullable());
    for (const file_id of deletedFileEdits) {
        if (file_id != null) {
            await deleteFile(file_id, authn_user_id);
    ...
    const editID = await queryRow(sql.insert_file_edit, { user_id, course_id, dir_name, file_name, orig_hash, file_id }, IdSchema);
    return editID;
}
export default router;
//# sourceMappingURL=instructorFileEditor.js.map