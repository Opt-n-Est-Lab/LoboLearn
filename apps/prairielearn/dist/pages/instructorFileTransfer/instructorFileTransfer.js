...
import { FileTransferSchema } from '../../lib/db-types.js';
...
async function getFileTransfer(file_transfer_id, user_id) {
    const file_transfer = await sqldb.queryRow(sql.select_file_transfer, { id: file_transfer_id }, FileTransferSchema);
    ...
    if (!idsEqual(file_transfer.user_id, user_id)) {
        throw new Error(`must have same user_id: ${file_transfer.user_id} and ${user_id} (types: ${typeof file_transfer.user_id}, ${typeof user_id})`);
    }
    return file_transfer;
}
router.get('/:file_transfer_id', asyncHandler(async (req, res) => {
    if (config.filesRoot == null)
        throw new Error('config.filesRoot is null');
    const file_transfer = await getFileTransfer(req.params.file_transfer_id, res.locals.user.user_id);
...
//# sourceMappingURL=instructorFileTransfer.js.map