...
import { checkAssessmentInstanceBelongsToCourseInstance, getCourseOwners, } from '../../lib/course.js';
import { courseInstanceFilenamePrefix } from '../../lib/sanitize-name.js';
import { InstructorGradebook } from './instructorGradebook.html.js';
import { AssessmentInstanceScoreResultSchema, CourseAssessmentRowSchema, GradebookRowSchema, } from './instructorGradebook.types.js';
const router = Router();
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const courseAssessments = await queryRows(sql.course_assessments, { course_instance_id: res.locals.course_instance.id }, CourseAssessmentRowSchema);
    res.send(InstructorGradebook({
...
router.get('/raw_data.json', asyncHandler(async (req, res) => {
    if (!res.locals.authz_data.has_course_instance_permission_view) {
        throw new HttpStatusError(403, 'Access denied (must be a student data viewer)');
    }
    const userScores = await queryRows(sql.user_scores, { course_id: res.locals.course.id, course_instance_id: res.locals.course_instance.id }, GradebookRowSchema);
    res.json(userScores);
}));
router.get('/:filename', asyncHandler(async (req, res) => {
    ...
    if (req.params.filename === buildCsvFilename(res.locals)) {
        const assessments = await queryRows(sql.course_assessments, { course_instance_id: res.locals.course_instance.id }, CourseAssessmentRowSchema);
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'edit_total_score_perc') {
        await checkAssessmentInstanceBelongsToCourseInstance(req.body.assessment_instance_id, res.locals.course_instance.id);
        await updateAssessmentInstanceScore(req.body.assessment_instance_id, req.body.score_perc, res.locals.authn_user.user_id);
        const updatedScores = await queryRows(sql.assessment_instance_score, { assessment_instance_id: req.body.assessment_instance_id }, AssessmentInstanceScoreResultSchema);
...
//# sourceMappingURL=instructorGradebook.js.map