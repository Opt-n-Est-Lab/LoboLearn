...
router.get('/:job_id(\\d+)', asyncHandler(async (req, res) => {
    const gradingJobRow = await sqldb.queryOptionalRow(sql.select_job, {
...
router.get('/:job_id(\\d+)/file/:file', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (s3Object.Body === undefined) {
            throw new Error(`S3 object ${grading_job.s3_bucket}/${grading_job.s3_root_key}/${file} has no body.`);
        }
        return pipeline(s3Object.Body, res);
    }
    catch (err) {
...
//# sourceMappingURL=instructorGradingJob.js.map