...
import { AssessmentSetSchema, CourseInstanceSchema, IdSchema, IssueSchema, QuestionSchema, UserSchema, VariantSchema, } from '../../lib/db-types.js';
...
const formattedCommonQueries = Object.fromEntries(Object.entries(commonQueries).map(([key, value]) => [key, `?q=${encodeURIComponent(value)}`]));
export function InstructorIssues({ resLocals, issues, filterQuery, openFilteredIssuesCount, openCount, closedCount, chosenPage, }) {
        ...
        ${issues.length === 0
            ...
            : html `
              <div class="list-group list-group-flush">
                ${issues.map((row) => IssueRow({ issue: row, urlPrefix, authz_data, csrfToken: __csrf_token }))}
              </div>
            `}
...
function IssueRow({ issue, urlPrefix, authz_data, csrfToken, }) {
    const plainUrlPrefix = config.urlPrefix;
    const mailtoLink = `mailto:${issue.user_email || issue.user_uid || '-'}?subject=Reported%20PrairieLearn%20Issue&body=${encodeURIComponent(`Hello ${issue.user_name},\n\nRegarding the issue of:\n\n"${issue.student_message || '-'}"\n\nWe've...`)}`;
...
function getFormattedMessage(issue) {
    if (!issue.student_message)
        return html `&mdash;`;
    const message = joinHtml(issue.student_message.split(/\r?\n|\r/), html `<br />`);
    return issue.manually_reported ? html `"${message}"` : message;
}
function CloseMatchingIssuesModal({ openFilteredIssuesCount, issues, csrfToken, }) {
    return Modal({
...
function IssueActionButton({ issue, csrfToken }) {
    return html `
...
//# sourceMappingURL=instructorIssues.html.js.map