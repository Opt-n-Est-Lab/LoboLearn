...
function parseRawQuery(str) {
    ...
    const filters = {
        ...
        filter_assessments: null,
        filter_not_assessments: null,
    };
...
async function updateIssueOpen(issue_id, new_open, course_id, authn_user_id) {
    const updated_issue_id = await queryOptionalRow(sql.update_issue_open, { issue_id, new_open, course_id, authn_user_id }, IdSchema);
    if (!updated_issue_id) {
        throw new HttpStatusError(403, `Unable to ${new_open ? 'open' : 'close'} issue ${issue_id}: issue does not exist in this course.`);
    }
}
router.get('/', asyncHandler(async (req, res) => {
    const filterQuery = typeof req.query.q === 'string' ? req.query.q : 'is:open';
    const [closedCount, openCount] = await queryRows(sql.issues_count, { course_id: res.locals.course.id }, z.number());
    ...
    const issueRows = await queryRows(sql.select_issues, { course_id: res.locals.course.id, offset, limit: PAGE_SIZE, ...filters }, IssueRowSchema);
    ...
    const issues = issueRows.map((row) => ({
        ...
        hideAssessmentLink: row.course_instance_id != null && !linkableCourseInstanceIds.has(row.course_instance_id),
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'open') {
        await updateIssueOpen(req.body.issue_id, true, // open status
        res.locals.course.id, res.locals.authn_user.user_id);
        res.redirect(req.originalUrl);
    }
    else if (req.body.__action === 'close') {
        await updateIssueOpen(req.body.issue_id, false, // open status
        res.locals.course.id, res.locals.authn_user.user_id);
        res.redirect(req.originalUrl);
    }
    else if (req.body.__action === 'close_matching') {
        const issueIds = req.body.unsafe_issue_ids.split(',').filter((id) => id !== '');
        const closedCount = await queryRow(sql.close_issues, {
            issue_ids: issueIds,
            course_id: res.locals.course.id,
            authn_user_id: res.locals.authn_user.user_id,
        }, z.number());
...
//# sourceMappingURL=instructorIssues.js.map