...
async function update(locals) {
    ...
    serverJob.executeInBackground(async (job) => {
        ...
        const courseDirs = new Set(config.courseDirs.map((courseDir) => path.resolve(REPOSITORY_ROOT_PATH, courseDir)));
        ...
        await async.eachOfSeries(Array.from(courseDirs), async (courseDir, index) => {
            ...
            if (!hasInfoCourseFile) {
                ...
                if (index !== config.courseDirs.length - 1)
                    job.info('');
                return;
            }
        ...
        if (anyCourseHadJsonErrors) {
            throw new Error('One or more courses had JSON files that contained errors and were unable to be synced');
        }
    });
...
//# sourceMappingURL=instructorLoadFromDisk.js.map