...
export function InstructorQuestionSettings({ resLocals, questionTestPath, questionTestCsrfToken, questionGHLink, qids, assessmentsWithQuestion, sharingEnabled, sharingSetsIn, editableCourses, infoPath, origHash, canEdit, courseTopics, courseTags, }) {
              ...
              <div class="card-footer">
                ...
                ? html `
                      <button
                        ...
                        data-bs-content="${escapeHtml(CopyForm({
...
function CopyForm({ csrfToken, editableCourses, courseId, }) {
    return html `
...
function DeleteQuestionModal({ qid, assessmentsWithQuestion, csrfToken, }) {
    return Modal({
              ...
              ${assessmentsWithQuestion.map((a_with_q) => {
                return html `
                  <li class="list-group-item">
                    ...
                    ${a_with_q.assessments.map((assessment) => AssessmentBadge({
...
function QuestionTestsForm({ questionTestPath, questionTestCsrfToken, }) {
    return html `
...
function QuestionSharing({ question, sharingSetsIn, }) {
    ...
    if (sharingSetsIn.length > 0) {
        const sharedWithLabel = sharingSetsIn.length === 1 ? '1 sharing set' : `${sharingSetsIn.length} sharing sets`;
        details.push(html `
      <p>
...
function AssessmentBadges({ assessmentsWithQuestion, resLocals, }) {
    const courseInstanceId = resLocals.course_instance.id;
    const assessmentsInCourseInstance = assessmentsWithQuestion.find((a) => idsEqual(a.course_instance_id, courseInstanceId));
    if (!assessmentsInCourseInstance?.assessments ||
...
//# sourceMappingURL=instructorQuestionSettings.html.js.map