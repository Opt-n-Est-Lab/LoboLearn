...
import { FileModifyEditor, MultiEditor, QuestionCopyEditor, QuestionDeleteEditor, QuestionRenameEditor, propertyValueWithDefault, } from '../../lib/editors.js';
...
import { InstructorQuestionSettings, SelectedAssessmentsSchema, SharingSetRowSchema, } from './instructorQuestionSettings.html.js';
...
router.post('/test', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'test_once') {
        ...
        const jobSequenceId = await startTestQuestion(count, showDetails, res.locals.question, res.locals.course_instance, res.locals.course, res.locals.user.user_id, res.locals.authn_user.user_id);
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'update_question') {
        const infoPath = path.join(res.locals.course.path, 'questions', res.locals.question.qid, 'info.json');
        ...
        if (!/^[-A-Za-z0-9_/]+$/.test(req.body.qid)) {
            throw new error.HttpStatusError(400, `Invalid QID (was not only letters, numbers, dashes, slashes, and underscores, with no spaces): ${req.body.qid}`);
        }
        ...
        questionInfo.gradingMethod = propertyValueWithDefault(questionInfo.gradingMethod, req.body.grading_method, 'Internal');
        questionInfo.singleVariant = propertyValueWithDefault(questionInfo.singleVariant, req.body.single_variant === 'on', false);
        questionInfo.showCorrectAnswer = propertyValueWithDefault(questionInfo.showCorrectAnswer, req.body.show_correct_answer === 'on', true);
        ...
        const qid_new = run(() => {
            try {
                ...
                throw new error.HttpStatusError(400, `Invalid QID (could not be normalized): ${req.body.qid}`);
            }
        });
        ...
        if (idsEqual(req.body.to_course_id, res.locals.course.id)) {
            ...
            flash('success', 'Question copied successfully. You are now viewing your copy of the question.');
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const questionTestCsrfToken = generateSignedToken({ url: questionTestPath, authn_user_id: res.locals.authn_user.user_id }, config.secretKey);
    let questionGHLink = null;
    ...
    const assessmentsWithQuestion = await sqldb.queryRows(sql.select_assessments_with_question_for_display, { question_id: res.locals.question.id }, SelectedAssessmentsSchema);
    ...
    let sharingSetsIn;
    if (sharingEnabled) {
        const result = await sqldb.queryRows(sql.select_sharing_sets, {
    ...
    res.send(InstructorQuestionSettings({
...
//# sourceMappingURL=instructorQuestionSettings.js.map