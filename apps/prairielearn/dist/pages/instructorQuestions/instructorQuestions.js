...
async function getTemplateCourseQuestionOptions() {
    ...
    const sortedTemplateQuestionOptions = templateQuestions.sort((a, b) => a.title.localeCompare(b.title));
    if (!config.devMode) {
...
router.get('/', asyncHandler(async function (req, res) {
    ...
    const questions = await selectQuestionsForCourse(res.locals.course.id, courseInstances.map((ci) => ci.id));
    ...
    res.send(QuestionsPage({
        questions,
        templateQuestions,
        course_instances: courseInstances,
        showAddQuestionButton: res.locals.authz_data.has_course_permission_edit &&
            !res.locals.course.example_course &&
            courseDirExists,
        showAiGenerateQuestionButton: res.locals.authz_data.has_course_permission_edit &&
            !res.locals.course.example_course &&
            isEnterprise() &&
...
router.get('/qid/*', asyncHandler(async (req, res) => {
...
router.post('/', asyncHandler(async (req, res) => {
    if (req.body.__action === 'add_question') {
        ...
        if (req.body.start_from === 'Template') {
            ...
            res.redirect(`${res.locals.urlPrefix}/question/${result.question_id}/file_edit/questions/${result.question_qid}/question.html`);
        }
    }
    else {
...
//# sourceMappingURL=instructorQuestions.js.map