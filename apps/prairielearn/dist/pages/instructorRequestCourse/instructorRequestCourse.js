...
import { CourseRequestRowSchema, RequestCourse, } from './instructorRequestCourse.html.js';
const router = express.Router();
const sql = loadSqlEquiv(import.meta.url);
router.get('/', asyncHandler(async (req, res) => {
    const rows = await queryRows(sql.get_requests, { user_id: res.locals.authn_user.user_id }, CourseRequestRowSchema);
    let lti13Info = null;
    if (isEnterprise() && 'lti13_claims' in req.session) {
        try {
            ...
            lti13Info = {
                ...
                'cr-shortname': ltiClaim.get(['https://purl.imsglobal.org/spec/lti/claim/context', 'label']) ?? '',
                'cr-title': ltiClaim.get(['https://purl.imsglobal.org/spec/lti/claim/context', 'title']) ?? '',
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const referral_source = referral_source_option === 'other' ? referral_source_other : referral_source_option;
    let error = false;
    if (!short_name.match(/[A-Z]+ [A-Z0-9]+/)) {
        flash('error', 'The course rubric and number should be a series of letters, followed by a space, followed by a series of numbers and/or letters.');
        error = true;
    }
    if (title.length < 1) {
    ...
    const hasExistingCourseRequest = await queryRow(sql.get_existing_course_requests, {
    ...
    const course_request_id = await queryRow(sql.insert_course_request, {
        short_name,
        title,
        user_id: res.locals.authn_user.user_id,
        github_user,
    ...
    const canAutoCreateCourse = await queryRow(sql.can_auto_create_course, {
        user_id: res.locals.authn_user.user_id,
    }, z.boolean());
    if (config.courseRequestAutoApprovalEnabled && canAutoCreateCourse) {
        ...
        const existingSettingsResult = await queryRow(sql.get_existing_owner_course_settings, { user_id: res.locals.authn_user.user_id }, z.object({
        ...
        try {
            await opsbot.sendCourseRequestMessage('*Automatically creating course*\n' +
        ...
        // Do this in the background once we've redirected the response.
        try {
            await opsbot.sendCourseRequestMessage('*Incoming course request*\n' +
                `Course rubric: ${short_name}\n` +
                `Course title: ${title}\n` +
                `Requested by: ${first_name} ${last_name} (${work_email})\n` +
                `Logged in as: ${res.locals.authn_user.name} (${res.locals.authn_user.uid})\n` +
...
//# sourceMappingURL=instructorRequestCourse.js.map