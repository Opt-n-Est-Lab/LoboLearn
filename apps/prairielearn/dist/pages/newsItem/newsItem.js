...
const router = Router();
router.get('/:news_item_id(\\d+)', asyncHandler(async (req, res) => {
    const newsItem = await queryOptionalRow(sql.select_news_item_for_read, {
    ...
    const indexFilename = path.join(import.meta.dirname, '..', '..', 'news_items', newsItem.directory, 'index.html');
...
router.get('/:news_item_id(\\d+)/*', asyncHandler(async (req, res) => {
    ...
    const newsItem = await queryOptionalRow(sql.select_news_item, { news_item_id: req.params.news_item_id }, NewsItemSchema);
    if (newsItem == null) {
        throw new HttpStatusError(404, 'Not found');
    }
    const newsItemDir = path.join(import.meta.dirname, '..', '..', 'news_items', newsItem.directory);
