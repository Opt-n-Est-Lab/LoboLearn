...
async function setLocals(req, res) {
    ...
    const disablePublicWorkspaces = await features.enabledFromLocals('disable-public-workspaces', res.locals);
    if (res.locals.question.workspace_image && disablePublicWorkspaces) {
        throw new error.HttpStatusError(403, 'Access denied');
    }
    if (!(res.locals.question.share_publicly || res.locals.question.share_source_publicly) ||
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'grade' || req.body.__action === 'save') {
        ...
        res.redirect(`${res.locals.urlPrefix}/question/${res.locals.question.id}/preview/?variant_id=${variant_id}`);
    }
...
//# sourceMappingURL=publicQuestionPreview.js.map