import { ECR, ECRClient, RepositoryAlreadyExistsException, RepositoryNotFoundException, } from '@aws-sdk/client-ecr';
...
export async function gitStatus(locals) {
    ...
    serverJob.executeInBackground(async (job) => {
        ...
        await job.exec('git', ['log', '--all', '--graph', '--date=short', '--format=format:%h %cd%d %cn %s'], {
            cwd: locals.course.path,
        });
    });
...
async function pullAndPushToECR(image, dockerAuth, job) {
    ...
    await new Promise((resolve, reject) => {
        const printedInfos = new Set();
        docker.modem.followProgress(pushStream, (err) => {
            if (err)
                reject(err);
            resolve(null);
        }, (output) => {
