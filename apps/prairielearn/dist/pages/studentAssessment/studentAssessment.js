...
import { GroupOperationError, canUserAssignGroupRoles, createGroup, getGroupConfig, getGroupId, getGroupInfo, joinGroup, leaveGroup, updateGroupRoles, } from '../../lib/groups.js';
...
import studentAssessmentAccess, { checkPasswordOrRedirect, } from '../../middlewares/studentAssessmentAccess.js';
...
router.get('/', asyncHandler(async function (req, res) {
    ...
    if (!checkPasswordOrRedirect(req, res))
        return;
    ...
    res.send(StudentAssessment({ resLocals: res.locals, groupConfig, groupInfo, userCanAssignRoles }));
}));
router.post('/', asyncHandler(async function (req, res) {
    ...
    if (req.body.__action === 'new_instance') {
        ...
        if (res.locals.assessment.group_work) {
            ...
            if (!groupInfo.start) {
                throw new HttpStatusError(403, 'Group has invalid composition or role assignment. Cannot start assessment.');
            }
        }
        const time_limit_min = res.locals.assessment.type === 'Exam' ? res.locals.authz_result.time_limit_min : null;
        const client_fingerprint_id = await getClientFingerprintId(req, res);
        const assessment_instance_id = await makeAssessmentInstance({
            assessment: res.locals.assessment,
            user_id: res.locals.user.user_id,
        ...
        await joinGroup(req.body.join_code, res.locals.assessment.id, res.locals.user.uid, res.locals.authn_user.user_id).catch((err) => {
        ...
        await createGroup(req.body.groupName, res.locals.assessment.id, [res.locals.user.uid], res.locals.authn_user.user_id).catch((err) => {
            if (err instanceof GroupOperationError) {
                flash('error', err.message);
            }
            else {
        ...
        await updateGroupRoles(req.body, res.locals.assessment.id, groupId, res.locals.user.user_id, res.locals.authz_data.has_course_instance_permission_edit, res.locals.authn_user.user_id);
        res.redirect(req.originalUrl);
    }
...
//# sourceMappingURL=studentAssessment.js.map