...
import { RegenerateInstanceAlert, RegenerateInstanceModal, } from '../../components/AssessmentRegenerate.html.js';
...
import { ExamQuestionAvailablePoints, ExamQuestionStatus, InstanceQuestionPoints, QuestionVariantHistory, } from '../../components/QuestionScore.html.js';
...
import { DateFromISOString, IdSchema, InstanceQuestionSchema, } from '../../lib/db-types.js';
...
export function StudentAssessmentInstance({ instance_question_rows, showTimeLimitExpiredModal, groupConfig, groupInfo, userCanAssignRoles, userCanDeleteAssessmentInstance, resLocals, }) {
    let savedAnswers = 0;
    ...
    const zoneTitleColspan = run(() => {
        const trailingColumnsCount = resLocals.assessment.type === 'Exam'
      ...
      <body>
        ...
        : html `
                      <div class="col-md-3 col-sm-6">
                        Total points:
                        ${formatPoints(resLocals.assessment_instance.points)}/${formatPoints(resLocals.assessment_instance.max_points)}
                        ${resLocals.assessment_instance.max_bonus_points
            ? html `
                              <br />
                              (${resLocals.assessment_instance.max_bonus_points} bonus
        ...
        : ''}
            ...
            >
              ...
              <tbody>
                ${instance_question_rows.map((instance_question_row) => html `
        ...
        rowLabelText: resLocals.assessment.type === 'Exam'
        ...
        ? html `
            ...
            ? html `
                ...
                ? ExamQuestionAvailablePoints({
                    open: resLocals.assessment_instance.open &&
                        instance_question_row.open,
                    currentWeight: (instance_question_row.points_list_original?.[instance_question_row.number_attempts] ?? 0) -
                        (instance_question_row.max_manual_points ?? 0),
                    pointsList: instance_question_row.points_list?.map((p) => p - (instance_question_row.max_manual_points ?? 0)),
        ...
        : html `
            ...
            ? html `
                ...
                const currentAutoValue = (instance_question_row.current_value ?? 0) -
                    (instance_question_row.max_manual_points ?? 0);
...
function AssessmentStatus({ assessment_instance, authz_result, }) {
...
function InstanceQuestionTableHeader({ resLocals }) {
    const trailingColumns = resLocals.assessment.type === 'Exam'
        ? html `
          ${resLocals.has_auto_grading_question && resLocals.assessment.allow_real_time_grading
            ? html `
...
function ZoneInfoPopover({ label, content }) {
    return html `
    <button
      type="button"
...
function RowLabel({ instance_question_row, userGroupRoles, rowLabelText, urlPrefix, }) {
...
function ConfirmFinishModal({ instance_question_rows, csrfToken, }) {
...
//# sourceMappingURL=studentAssessmentInstance.html.js.map