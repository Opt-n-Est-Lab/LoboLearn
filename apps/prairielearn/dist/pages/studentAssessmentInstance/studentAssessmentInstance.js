...
import { canUserAssignGroupRoles, getGroupConfig, getGroupInfo, getQuestionGroupPermissions, leaveGroup, updateGroupRoles, } from '../../lib/groups.js';
...
import { InstanceQuestionRowSchema, StudentAssessmentInstance, } from './studentAssessmentInstance.html.js';
...
async function ensureUpToDate(locals) {
    const updated = await assessment.updateAssessmentInstance(locals.assessment_instance.id, locals.authn_user.user_id);
    if (updated) {
        // we updated the assessment_instance, so reload it
        locals.assessment_instance = await queryRow(sql.select_assessment_instance, { assessment_instance_id: locals.assessment_instance.id }, AssessmentInstanceSchema);
    }
}
...
async function processDeleteFile(req, res) {
    ...
    const validFiles = (res.locals.file_list ?? []).filter((file) => idsEqual(file.id, req.body.file_id));
...
router.post('/', asyncHandler(async function (req, res, next) {
    ...
    if (!res.locals.authz_result.authorized_edit &&
        ['attach_file', 'attach_text', 'delete_file', 'timeLimitFinish', 'leave_group'].includes(req.body.__action)) {
    ...
    if (req.body.__action === 'attach_file') {
        ...
        await assessment.gradeAssessmentInstance(res.locals.assessment_instance.id, res.locals.user.user_id, res.locals.authn_user.user_id, requireOpen, closeExam, overrideGradeRate, res.locals.client_fingerprint_id);
        ...
        await leaveGroup(res.locals.assessment.id, res.locals.user.user_id, res.locals.authn_user.user_id);
        ...
        await updateGroupRoles(req.body, res.locals.assessment.id, res.locals.assessment_instance.group_id, res.locals.user.user_id, res.locals.authz_data.has_course_instance_permission_edit, res.locals.authn_user.user_id);
        res.redirect(req.originalUrl);
    }
    else {
...
router.get('/', asyncHandler(async (req, res, _next) => {
    ...
    const instance_question_rows = await queryRows(sql.select_instance_questions, {
        assessment_instance_id: res.locals.assessment_instance.id,
    ...
    for (const instance_question of instance_question_rows) {
        instance_question.previous_variants = allPreviousVariants.filter((variant) => idsEqual(variant.instance_question_id, instance_question.id));
    }
    res.locals.has_manual_grading_question = instance_question_rows?.some((q) => q.max_manual_points || q.manual_points || q.requires_manual_grading);
    res.locals.has_auto_grading_question = instance_question_rows?.some((q) => q.max_auto_points || q.auto_points || !q.max_points);
    const assessment_text_templated = assessment.renderText(res.locals.assessment, res.locals.urlPrefix);
    res.locals.assessment_text_templated = assessment_text_templated;
    const showTimeLimitExpiredModal = req.query.timeLimitExpired === 'true';
    if (!res.locals.assessment.group_work) {
        res.send(StudentAssessmentInstance({
    ...
    const userCanAssignRoles = groupInfo != null &&
    ...
    if (groupConfig.has_roles) {
        ...
        if (!res.locals.authz_data.has_course_instance_permission_view) {
            for (const question of instance_question_rows) {
                question.group_role_permissions = await getQuestionGroupPermissions(question.id, res.locals.assessment_instance.group_id, res.locals.authz_data.user.user_id);
            }
        }
    }
    res.send(StudentAssessmentInstance({
...
//# sourceMappingURL=studentAssessmentInstance.js.map