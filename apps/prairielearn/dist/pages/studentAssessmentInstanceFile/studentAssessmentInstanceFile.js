...
router.get('/:unsafe_file_id(\\d+)/:unsafe_display_filename', asyncHandler(async (req, res, next) => {
    // Assert that the file belongs to this assessment, that the display
    // filename matches, and that the file is not deleted.
    const file = await queryOptionalRow(sql.select_file, {
...
//# sourceMappingURL=studentAssessmentInstanceFile.js.map