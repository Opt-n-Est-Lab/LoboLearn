...
import { getAndRenderVariant, renderPanelsForSubmission, setRendererHeader, } from '../../lib/question-render.js';
...
async function validateAndProcessSubmission(req, res) {
    ...
    if (res.locals.group_config?.has_roles && !res.locals.group_role_permissions.can_submit) {
        throw new HttpStatusError(403, 'Your current group role does not give you permission to submit to this question.');
    }
    ...
    if (req.body.__action === 'grade' || req.body.__action === 'save') {
        if (res.locals.assessment.type === 'Exam') {
            if (res.locals.authz_result.time_limit_expired) {
                throw new HttpStatusError(403, 'Time limit is expired, please go back and finish your assessment');
            }
        ...
        if (res.locals.assessment.type === 'Exam') {
            ...
            res.redirect(`${res.locals.urlPrefix}/instance_question/${res.locals.instance_question.id}/?variant_id=${variant_id}`);
        }
    }
        ...
        await gradeAssessmentInstance(res.locals.assessment_instance.id, res.locals.user.user_id, res.locals.authn_user.user_id, requireOpen, closeExam, overrideGradeRate, res.locals.client_fingerprint_id);
...
router.get('/variant/:unsafe_variant_id(\\d+)/submission/:unsafe_submission_id(\\d+)', asyncHandler(async (req, res) => {
    const variant = await selectAndAuthzVariant({
...
router.get('/', asyncHandler(async (req, res) => {
    let variant_id = res.locals.assessment.type === 'Exam' || typeof req.query.variant_id !== 'string'
        ? null
        : req.query.variant_id;
    const isAssessmentAvailable = res.locals.assessment_instance.open && res.locals.authz_result.active;
    if (variant_id === null && !isAssessmentAvailable) {
        ...
        // cannot be generated.
        const last_variant_id = await queryOptionalRow(sql.select_last_variant_id, { instance_question_id: res.locals.instance_question.id }, IdSchema);
    ...
    if (res.locals.group_config?.has_roles &&
        ...
        if (res.locals.instance_question_info.prev_instance_question.id != null) {
            res.locals.prev_instance_question_role_permissions = await getQuestionGroupPermissions(res.locals.instance_question_info.prev_instance_question.id, res.locals.assessment_instance.group_id, res.locals.authz_data.user.user_id);
        }
        if (res.locals.instance_question_info.next_instance_question.id) {
            res.locals.next_instance_question_role_permissions = await getQuestionGroupPermissions(res.locals.instance_question_info.next_instance_question.id, res.locals.assessment_instance.group_id, res.locals.authz_data.user.user_id);
        }
    }
    ...
    const lastGrader = res.locals.instance_question.last_grader
        ...
        : null;
    res.send(StudentInstanceQuestion({
        resLocals: res.locals,
        userCanDeleteAssessmentInstance: canDeleteAssessmentInstance(res.locals),
        assignedGrader,
        lastGrader,
...
//# sourceMappingURL=studentInstanceQuestion.js.map