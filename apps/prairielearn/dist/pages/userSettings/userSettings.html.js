...
import {} from '../../ee/lib/billing/purchases.js';
...
export function UserSettings({ authn_user, authn_institution, authn_provider_name, accessTokens, newAccessTokens, purchases, isExamMode, showEnhancedNavigationToggle, enhancedNavigationEnabled, resLocals, }) {
    return PageLayout({
        resLocals,
        ...
        ${newAccessTokens.length > 0
            ? html `
              <div class="card-body">
                ...
                ${newAccessTokens.map((token) => html `
...
function TokenList({ accessTokens, isExamMode, resLocals, }) {
    if (isExamMode) {
        return html `
    ...
    return accessTokens.map((token) => html `
      <li class="list-group-item d-flex align-items-center">
...
function TokenGenerateForm({ csrfToken }) {
    return html `
...
function TokenDeleteForm({ token_id, csrfToken }) {
    return html `
...
//# sourceMappingURL=userSettings.html.js.map