...
router.get('/', asyncHandler(async (req, res) => {
    let navTitle, pageTitle, pageNote;
    if (res.locals.assessment == null) {
        // instructor preview
    ...
    res.send(Workspace({
        ...
        navTitle,
        navTitleHref,
        showLogs: res.locals.authz_data.has_course_instance_permission_view ||
        ...
        socketToken: generateSignedToken({ workspace_id: res.locals.workspace_id.toString() }, config.secretKey),
...
router.post('/', asyncHandler(async (req, res, next) => {
    ...
    if (req.body.__action === 'reboot') {
        ...
        await workspaceUtils.updateWorkspaceState(workspace_id, 'uninitialized', 'Resetting container');
