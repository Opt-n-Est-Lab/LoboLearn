...
import { WorkspaceLogRowSchema, WorkspaceLogs, WorkspaceVersionLogs, } from './workspaceLogs.html.js';
const router = Router();
...
async function loadLogsForWorkspaceVersion(workspaceId, version) {
    ...
    if (['launching', 'running'].includes(workspace.state) &&
...
router.get('/', asyncHandler(async (_req, res, _next) => {
    const workspaceLogs = await sqldb.queryRows(sql.select_workspace_logs, {
        workspace_id: res.locals.workspace_id,
        workspace_version: null,
        display_timezone: res.locals.course_instance?.display_timezone ?? res.locals.course.display_timezone,
...
router.get('/version/:version', asyncHandler(async (req, res, _next) => {
    const workspaceLogs = await sqldb.queryRows(sql.select_workspace_logs, {
        workspace_id: res.locals.workspace_id,
    ...
    if (containerLogsEnabled && !containerLogsExpired) {
        containerLogs = await loadLogsForWorkspaceVersion(res.locals.workspace_id, req.params.version);
    }
    res.send(WorkspaceVersionLogs({
        workspaceLogs,
        containerLogs,
...
//# sourceMappingURL=workspaceLogs.js.map