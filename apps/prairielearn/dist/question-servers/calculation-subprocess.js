...
async function callFunction(func, question_course, question, inputData) {
    ...
    const { fullPath: questionServerPath } = await filePaths.questionFilePath('server.js', question.directory, courseHostPath, question);
    ...
    const questionServerRuntimePath = getQuestionRuntimePath(questionServerPath, courseHostPath, courseRuntimePath);
    try {
 ...
 */
export async function render(_renderSelection, _variant, _question, _submission, submissions, _course, _locals) {
    const data = {
        extraHeadersHtml: '',
        questionHtml: '',
...
//# sourceMappingURL=calculation-subprocess.js.map