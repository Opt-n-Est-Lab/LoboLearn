...
function grade(server, coursePath, submission, variant, question) {
    ...
    const questionDir = path.join(coursePath, 'questions', question.directory);
    const grading = server.gradeAnswer(vid, params, trueAnswer, submittedAnswer, options, questionDir);
...
(async () => {
    ...
    questionServerPath, func, coursePath, question, 
    // Depending on which function is being invoked, these may or may not
    // be present.
    variant_seed, variant, submission, } = input;
...
//# sourceMappingURL=calculation-worker.js.map