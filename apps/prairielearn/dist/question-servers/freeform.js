...
import { ElementCoreJsonSchema, ElementCourseJsonSchema, ElementExtensionJsonSchema, } from '../schemas/index.js';
...
async function loadExtensionsForCourse(context) {
    ...
    const extensions = await loadExtensions(path.join(course_dir_host, 'elementExtensions'), path.join(course_dir, 'elementExtensions'));
...
function getElementClientFiles(data, elementName, context) {
    ...
    if ('base_url' in data.options) {
        dataCopy.options.client_files_element_url = assets.courseElementAssetPath(context.course.commit_hash, data.options.base_url, `${elementName}/clientFilesElement`);
        ...
        if (Object.prototype.hasOwnProperty.call(context.course_element_extensions, elementName)) {
            Object.keys(context.course_element_extensions[elementName]).forEach((extension) => {
                const url = assets.courseElementExtensionAssetPath(context.course.commit_hash, data.options.base_url, `${elementName}/${extension}/clientFilesExtension`);
...
async function execPythonServer(codeCaller, phase, data, html, context) {
    ...
    debug(`execPythonServer(): codeCaller.call(pythonFile=${pythonFile}, pythonFunction=${pythonFunction})`);
    try {
        const { result, output } = await codeCaller.call(type, directory, pythonFile, pythonFunction, pythonArgs);
...
function checkData(data, origData, phase) {
    ...
    if (extraProps.length > 0)
        return `"data" has invalid extra keys: ${extraProps.join(', ')}`;
    return null;
}
/**
 * @template T
...
async function experimentalProcess(phase, codeCaller, data, context, html) {
    ...
    let output = null;
    try {
        const res = await codeCaller.call('question', context.question.directory, 'question.html', phase, [pythonContext, data]);
        result = res.result;
        output = res.output;
    }
    catch (err) {
        courseIssues.push(new CourseIssueError(err.message, {
            data: err.data,
            cause: err,
            fatal: true,
        }));
    ...
    if ((output?.length ?? 0) > 0) {
        courseIssues.push(new CourseIssueError(`output logged on console during ${phase}()`, {
...
async function traverseQuestionAndExecuteFunctions(phase, codeCaller, data, context, html) {
    ...
    const visitNode = async (node) => {
        if (node.tagName && questionElements.has(node.tagName)) {
            ...
            try {
                ({ result: ret_val, output: consoleLog } = await elementFunction(codeCaller, phase, elementName, serializedNode, { ...data, extensions }, context));
            }
            catch (e) {
            ...
            if (typeof consoleLog === 'string' && consoleLog.length > 0) {
                courseIssues.push(new CourseIssueError(`${elementFile}: output logged on console during ${phase}()`, {
            ...
            if (phase === 'render') {
                if (typeof ret_val !== 'string') {
                    throw new CourseIssueError(`${elementFile}: Error calling ${phase}(): return value is not a string`, { data: ret_val, fatal: true });
                }
                ...
                if (buf.length > 0) {
                    if (fileData.length > 0) {
                        ...
                        throw new CourseIssueError(`${elementFile}: Error calling ${phase}(): attempting to overwrite non-empty fileData`, { fatal: true });
                    }
                    else {
                ...
                if (checkErr) {
                    throw new CourseIssueError(`${elementFile}: Invalid state after ${phase}(): ${checkErr}`, { fatal: true });
                }
            }
        }
        const newChildren = [];
    ...
    try {
        await async.eachSeries(questionElements, async (elementName) => {
            await async.eachSeries($(elementName).toArray(), async (element) => {
                ...
                let result, output;
                try {
                    ({ result, output } = await elementFunction(codeCaller, phase, elementName, elementHtml, { ...data, extensions }, context));
                }
                catch (err) {
                    const courseIssue = new CourseIssueError(`${elementFile}: Error calling ${phase}(): ${err.toString()}`, { data: err.data, fatal: true });
                ...
                if (phase === 'render') {
                    if (typeof result !== 'string') {
                        const courseIssue = new CourseIssueError(`${elementFile}: Error calling ${phase}(): return value is not a string`, { data: { result }, fatal: true });
                        courseIssues.push(courseIssue);
                    ...
                    if (buf.length > 0) {
                        if (fileData.length > 0) {
                            // If fileData already has non-zero length, throw an error
                            const courseIssue = new CourseIssueError(`${elementFile}: Error calling ${phase}(): attempting to overwrite non-empty fileData`, { fatal: true });
                            courseIssues.push(courseIssue);
                    ...
                    const checkErr = checkData(data, origData, phase);
                    if (checkErr) {
                        const courseIssue = new CourseIssueError(`${elementFile}: Invalid state after ${phase}(): ${checkErr}`, { fatal: true });
                        courseIssues.push(courseIssue);
                        // As above, throw the error to abort the traversal.
                        throw courseIssue;
                    }
...
function getPartialScoreValues(val) {
    const obj = /** @type {Record<string, unknown>} */ (typeof val === 'object' && val != null ? val : {});
    return {
...
async function processQuestionHtml(phase, codeCaller, data, context) {
    ...
    const { courseIssues, data: resultData, html: processedHtml, fileData, renderedElementNames, } = await run(async () => {
    ...
    if (phase === 'grade' || phase === 'test') {
        if (context.question.partial_credit) {
            ...
            let score = 0;
            if (Object.keys(resultData.partial_scores ?? {}).length > 0 &&
                Object.values(resultData.partial_scores ?? {}).every((value) => getPartialScoreValues(value).score >= 1)) {
                score = 1;
            }
...
async function processQuestionServer(phase, codeCaller, data, html, fileData, context) {
    ...
    if (checkErrBefore) {
        courseIssues.push(new CourseIssueError(`Invalid state before calling server ${phase}(): ${checkErrBefore}`, {
            fatal: true,
        }));
    ...
    try {
        ...
        courseIssues.push(new CourseIssueError(`${serverFile}: Error calling ${phase}(): ${err.toString()}`, {
            data: err.data,
            fatal: true,
            cause: err,
        }));
    ...
    if (typeof output === 'string' && output.length > 0) {
        const serverFile = path.join(context.question_dir, 'server.py');
        courseIssues.push(new CourseIssueError(`${serverFile}: output logged on console`, {
            data: { outputBoth: output },
            fatal: false,
        }));
    }
    if (phase === 'render') {
        ...
        if (buf.length > 0) {
            if (fileData.length > 0) {
                // If fileData already has non-zero length, throw an error
                const serverFile = path.join(context.question_dir, 'server.py');
                courseIssues.push(new CourseIssueError(`${serverFile}: Error calling ${phase}(): attempting to overwrite non-empty fileData`, { fatal: true }));
                return { courseIssues, data };
            }
            else {
    ...
    if (checkErrAfter) {
        const serverFile = path.join(context.question_dir, 'server.py');
        courseIssues.push(new CourseIssueError(`${serverFile}: Invalid state after ${phase}(): ${checkErrAfter}`, {
            fatal: true,
        }));
        return { courseIssues, data };
    }
...
async function processQuestion(phase, codeCaller, data, context) {
    ...
    return instrumentedWithMetrics(meter, `freeform.${phase}`, async () => {
        if (phase === 'generate') {
            ...
            const { courseIssues, data: htmlData, html, fileData, renderedElementNames, } = await processQuestionHtml(phase, codeCaller, data, context);
            ...
            const { courseIssues: serverCourseIssues, data: serverData, html: serverHtml, fileData: serverFileData, } = await processQuestionServer(phase, codeCaller, htmlData, html, fileData, context);
...
export async function generate(question, course, variant_seed) {
    return instrumented('freeform.generate', async () => {
        ...
        return await withCodeCaller(course, async (codeCaller) => {
            const { courseIssues, data: resultData } = await processQuestion('generate', codeCaller, data, context);
            return {
                courseIssues,
                data: {
...
export async function prepare(question, course, variant) {
    return instrumented('freeform.prepare', async () => {
        if (variant.broken_at)
            throw new Error('attempted to prepare broken variant');
        const context = await getContext(question, course);
        /** @satisfies {import('./types.js').ExecutionData} */
        const data = {
        ...
        return await withCodeCaller(course, async (codeCaller) => {
            const { courseIssues, data: resultData } = await processQuestion('prepare', codeCaller, data, context);
            return {
                courseIssues,
                data: {
                    params: resultData.params,
...
async function renderPanel(panel, codeCaller, variant, submission, course, locals, context) {
    ...
    const options = {
        ...
        course_element_files_url: assets.courseElementAssetBasePath(course.commit_hash, locals.urlPrefix),
        course_element_extension_files_url: assets.courseElementExtensionAssetBasePath(course.commit_hash, locals.urlPrefix),
    ...
    const data = {
        ...
        manual_grading: run(() => {
            if (locals.questionRenderContext === 'manual_grading')
                return true;
    ...
    const { data: cachedData, cacheHit } = await getCachedDataOrCompute(course, data, context, async () => {
        const { courseIssues, html, renderedElementNames } = await processQuestion('render', codeCaller, data, context);
    ...
    return {
        ...
        html: locals.questionRenderContext === 'ai_grading'
            ? await stripHtmlForAiGrading(cachedData.html)
            : cachedData.html,
        cacheHit,
    };
}
async function renderPanelInstrumented(panel, codeCaller, submission, variant, question, course, locals, context) {
    return instrumented(`freeform.renderPanel:${panel}`, async (span) => {
        ...
        const result = await renderPanel(panel, codeCaller, variant, submission, course, locals, context);
        span.setAttribute('cache.status', result.cacheHit ? 'hit' : 'miss');
        return result;
    });
}
export async function render(renderSelection, variant, question, submission, submissions, course, locals) {
    return instrumented('freeform.render', async () => {
        ...
        return withCodeCaller(course, async (codeCaller) => {
            if (renderSelection.question) {
                const { courseIssues: newCourseIssues, html, renderedElementNames, } = await renderPanelInstrumented('question', codeCaller, submission, variant, question, course, locals, context);
            ...
            if (renderSelection.submissions) {
                htmls.submissionHtmls = await async.mapSeries(submissions, async (submission) => {
                    const { courseIssues: newCourseIssues, html, renderedElementNames, } = await renderPanelInstrumented('submission', codeCaller, submission, variant, question, course, locals, context);
                    courseIssues.push(...newCourseIssues);
                    allRenderedElementNames = _.union(allRenderedElementNames, renderedElementNames);
                    return html;
                });
            ...
            if (renderSelection.answer) {
                const { courseIssues: newCourseIssues, html, renderedElementNames, } = await renderPanelInstrumented('answer', codeCaller, submission, variant, question, course, locals, context);
                courseIssues.push(...newCourseIssues);
            ...
            allRenderedElementNames.forEach((elementName) => {
                ...
                const elementDynamicDependencies = structuredClone(resolvedElement.dynamicDependencies) ?? {};
                ...
                if ('elementScripts' in elementDynamicDependencies) {
                    elementDynamicDependencies.elementScripts = _.mapValues(elementDynamicDependencies.elementScripts, (dep) => `${resolvedElement.name}/${dep}`);
                }
                for (const type in elementDependencies) {
                    ...
                    const resolvedType = run(() => {
                        if (resolvedElement.type === 'course') {
                            if (type === 'elementStyles')
                                return 'courseElementStyles';
                            if (type === 'elementScripts')
                                return 'courseElementScripts';
                        }
                        else {
                            if (type === 'elementStyles')
                                return 'coreElementStyles';
                            if (type === 'elementScripts')
                                return 'coreElementScripts';
                        }
                        return type;
                    });
                    if (!(resolvedType in dependencies))
                        continue;
                ...
                for (const type in elementDynamicDependencies) {
                    ...
                    for (const key in elementDynamicDependencies[type]) {
                        ...
                        else if (dynamicDependencies[resolvedType][key] !== elementDynamicDependencies[type][key]) {
                            courseIssues.push(new CourseIssueError(`Dynamic dependency ${key} assigned to conflicting files`, {
                                data: {
                                    dependencyType: type,
                ...
                if (Object.prototype.hasOwnProperty.call(extensions, elementName)) {
                    for (const extensionName of Object.keys(extensions[elementName])) {
                        if (!('dependencies' in extensions[elementName][extensionName]) &&
                            !('dynamicDependencies' in extensions[elementName][extensionName])) {
                            continue;
                        }
                        const extension = structuredClone(extensions[elementName][extensionName].dependencies) ?? {};
                        const extensionDynamic = structuredClone(extensions[elementName][extensionName].dynamicDependencies) ?? {};
                        ...
                        if ('extensionScripts' in extensionDynamic) {
                            extensionDynamic.extensionScripts = _.mapValues(extensionDynamic.extensionScripts, (dep) => `${elementName}/${extensionName}/${dep}`);
                        }
            ...
            dependencies.coreElementStyles.forEach((file) => styleUrls.push(assets.coreElementAssetPath(file)));
            dependencies.coreElementScripts.forEach((file) => scriptUrls.push(assets.coreElementAssetPath(file)));
            const courseElementUrlPrefix = locals.urlPrefix +
                (!idsEqual(question.course_id, variant.course_id)
                    ? `/sharedElements/course/${course.id}`
                    : '');
            dependencies.courseElementStyles.forEach((file) => styleUrls.push(assets.courseElementAssetPath(course.commit_hash, courseElementUrlPrefix, file)));
            dependencies.courseElementScripts.forEach((file) => scriptUrls.push(assets.courseElementAssetPath(course.commit_hash, courseElementUrlPrefix, file)));
            dependencies.extensionStyles.forEach((file) => styleUrls.push(assets.courseElementExtensionAssetPath(course.commit_hash, locals.urlPrefix, file)));
            dependencies.extensionScripts.forEach((file) => scriptUrls.push(assets.courseElementExtensionAssetPath(course.commit_hash, locals.urlPrefix, file)));
            const importMap = {
                imports: {
                    ..._.mapValues(dynamicDependencies.nodeModulesScripts, (file) => assets.nodeModulesAssetPath(file)),
                    ..._.mapValues(dynamicDependencies.clientFilesCourseScripts, (file) => `${locals.urlPrefix}/clientFilesCourse/${file}`),
                    ..._.mapValues(dynamicDependencies.coreElementScripts, (file) => assets.coreElementAssetPath(file)),
                    ..._.mapValues(dynamicDependencies.courseElementScripts, (file) => assets.courseElementAssetPath(course.commit_hash, courseElementUrlPrefix, file)),
                    ..._.mapValues(dynamicDependencies.extensionScripts, (file) => assets.courseElementExtensionAssetPath(course.commit_hash, locals.urlPrefix, file)),
                },
            };
            ...
            Object.keys(importMap.imports).forEach((key) => {
                const types = Object.entries(dynamicDependencies)
                    ...
                    .map(([type]) => type);
                if (types.length > 1) {
                    courseIssues.push(new CourseIssueError(`Dynamic dependency key ${key} assigned to multiple types of dependencies`, { data: { types }, fatal: true }));
                }
            });
            const headerHtmls = [
...
export async function file(filename, variant, question, course) {
    return instrumented('freeform.file', async (span) => {
        debug('file()');
        if (variant.broken_at)
            throw new Error('attempted to get a file for a broken variant');
        const context = await getContext(question, course);
        /** @satisfies {import('./types.js').ExecutionData} */
        const data = {
            // These should never be null, but that can't be encoded in the schema.
        ...
        const { data: cachedData, cacheHit } = await getCachedDataOrCompute(course, data, context, async () => {
            ...
            return withCodeCaller(course, async (codeCaller) => {
                const { courseIssues, fileData } = await processQuestion('file', codeCaller, data, context);
...
export async function parse(submission, variant, question, course) {
    return instrumented('freeform.parse', async () => {
        debug('parse()');
        if (variant.broken_at)
            throw new Error('attempted to parse broken variant');
        const context = await getContext(question, course);
        /** @satisfies {import('./types.js').ExecutionData} */
        const data = {
            // These should never be null, but that can't be encoded in the schema.
        ...
        return withCodeCaller(course, async (codeCaller) => {
            const { courseIssues, data: resultData } = await processQuestion('parse', codeCaller, data, context);
            if (Object.keys(resultData.format_errors).length > 0)
                resultData.gradable = false;
            return {
                courseIssues,
                data: {
                    params: resultData.params,
...
export async function grade(submission, variant, question, question_course) {
    return instrumented('freeform.grade', async () => {
        debug('grade()');
        if (variant.broken_at)
            throw new Error('attempted to grade broken variant');
        if (submission.broken)
            throw new Error('attempted to grade broken submission');
        ...
        return withCodeCaller(question_course, async (codeCaller) => {
            const { courseIssues, data: resultData } = await processQuestion('grade', codeCaller, data, context);
            if (Object.keys(resultData.format_errors).length > 0)
                resultData.gradable = false;
            return {
                courseIssues,
...
export async function test(variant, question, course, test_type) {
    return instrumented('freeform.test', async () => {
        debug('test()');
        if (variant.broken_at)
            throw new Error('attempted to test broken variant');
        const context = await getContext(question, course);
        ...
        return withCodeCaller(course, async (codeCaller) => {
            const { courseIssues, data: resultData } = await processQuestion('test', codeCaller, data, context);
            if (Object.keys(resultData.format_errors).length > 0)
                resultData.gradable = false;
            return {
                courseIssues,
