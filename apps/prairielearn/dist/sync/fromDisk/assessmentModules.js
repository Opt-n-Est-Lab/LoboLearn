...
export async function sync(courseId, courseData) {
    ...
    const existingAssessmentModules = await queryRows(sql.select_assessment_modules, { course_id: courseId }, AssessmentModuleSchema);
    // Based on the set of desired assessment modules, determine which ones must be
    // added, updated, or deleted.
    const { entitiesToCreate: assessmentModulesToCreate, entitiesToUpdate: assessmentModulesToUpdate, entitiesToDelete: assessmentModulesToDelete, } = determineOperationsForEntities({
        ...
        await runInTransactionAsync(async () => {
            if (assessmentModulesToCreate.length) {
                await queryAsync(sql.insert_assessment_modules, {
                    course_id: courseId,
                    modules: assessmentModulesToCreate.map((am) => JSON.stringify([am.name, am.heading, am.number, am.implicit])),
                });
            }
            if (assessmentModulesToUpdate.length) {
                await queryAsync(sql.update_assessment_modules, {
                    course_id: courseId,
                    modules: assessmentModulesToUpdate.map((am) => JSON.stringify([am.name, am.heading, am.number, am.implicit])),
                });
            }
...
//# sourceMappingURL=assessmentModules.js.map