...
export async function sync(courseId, courseData) {
    ...
    const existingAssessmentSets = await queryRows(sql.select_assessment_sets, { course_id: courseId }, AssessmentSetSchema);
    // Based on the set of desired assessment sets, determine which ones must be
    // added, updated, or deleted.
    const { entitiesToCreate: assessmentSetsToCreate, entitiesToUpdate: assessmentSetsToUpdate, entitiesToDelete: assessmentSetsToDelete, } = determineOperationsForEntities({
        ...
        await runInTransactionAsync(async () => {
            if (assessmentSetsToCreate.length) {
                await queryAsync(sql.insert_assessment_sets, {
                    course_id: courseId,
                    sets: assessmentSetsToCreate.map((as) => JSON.stringify([
                        as.name,
                        as.abbreviation,
                        as.heading,
                        as.color,
            ...
            if (assessmentSetsToUpdate.length) {
                await queryAsync(sql.update_assessment_sets, {
                    course_id: courseId,
                    sets: assessmentSetsToUpdate.map((as) => JSON.stringify([
                        as.name,
                        as.abbreviation,
                        as.heading,
                        as.color,
...
//# sourceMappingURL=assessmentSets.js.map