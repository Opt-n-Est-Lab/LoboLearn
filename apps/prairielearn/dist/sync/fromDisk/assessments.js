...
function getParamsForAssessment(assessmentInfoFile, questionIds) {
    if (infofile.hasErrors(assessmentInfoFile))
        return null;
    const assessment = assessmentInfoFile.data;
    if (!assessment)
        throw new Error(`Missing assessment data for ${assessmentInfoFile.uuid}`);
    ...
    const alternativeGroups = (assessment.zones ?? []).map((zone) => {
        ...
        return zone.questions.map((question) => {
            ...
            const normalizedAlternatives = alternatives.map((alternative) => {
                const hasSplitPoints = alternative.autoPoints !== null ||
                ...
                if (assessment.type === 'Exam') {
                    ...
                    const initPoints = (Array.isArray(autoPoints) ? autoPoints[0] : autoPoints) + manualPoints;
            ...
            const questions = normalizedAlternatives.map((alternative, alternativeIndex) => {
                ...
                return {
                    ...
                    effective_advance_score_perc: alternative.advanceScorePerc ??
    ...
    return {
        ...
        shuffle_questions: (assessment.type === 'Exam' && assessment.shuffleQuestions === undefined) ||
...
function isCourseInstanceAccessible(courseInstanceData) {
    ...
    if (!courseInstance.allowAccess?.length)
        return false;
...
export async function sync(courseId, courseInstanceId, courseInstanceData, questionIds) {
    ...
    if (isCourseInstanceAccessible(courseInstanceData) && config.checkAccessRulesExamUuid) {
        ...
        const uuidsRes = await sqldb.queryRows(sql.check_access_rules_exam_uuid, { exam_uuids: JSON.stringify([...examUuids]) }, z.object({ uuid: z.string(), uuid_exists: z.boolean() }));
        uuidsRes.forEach(({ uuid, uuid_exists }) => {
            if (!uuid_exists) {
                uuidAssessmentMap.get(uuid)?.forEach((tid) => {
                    infofile.addWarning(assessments[tid], `examUuid "${uuid}" not found. Ensure you copied the correct UUID from PrairieTest.`);
                });
            }
        });
    }
    ...
    if (importedQids.size > 0) {
        const institutionId = await sqldb.queryRow(sql.get_institution_id, { course_id: courseId }, z.string());
        ...
        if (!(questionSharingEnabled || consumePublicQuestionsEnabled) && config.checkSharingOnSync) {
            for (const [tid, qids] of assessmentImportedQids.entries()) {
                if (qids.length > 0) {
                    infofile.addError(assessments[tid], "You have attempted to import a question with '@', but question sharing is not enabled for your course.");
                }
            }
        }
    }
    const importedQuestions = await sqldb.queryRows(sql.get_imported_questions, {
        course_id: courseId,
    ...
    if (config.checkSharingOnSync) {
        for (const [tid, qids] of assessmentImportedQids.entries()) {
            ...
            if (assessmentMissingQids.length > 0) {
                infofile.addError(assessments[tid], `For each of the following, either the course you are referencing does not exist, or the question does not exist within that course: ${[
