...
export async function sync(courseId, courseData, questionIds) {
    ...
    const courseSharingSets = await sqldb.queryRows(sql.select_course_sharing_sets, { course_id: courseId }, z.object({
        id: IdSchema,
        name: z.string(),
    }));
    ...
    const questionSharingSets = [];
    Object.entries(courseData.questions).forEach(([qid, question]) => {
        if (infofile.hasErrors(question))
            return;
        const dedupedQuestionSharingSetNames = new Set(question.data?.sharingSets ?? []);
        const questionSharingSetIds = [...dedupedQuestionSharingSetNames].map((sharingSet) => sharingSetIdsByName[sharingSet]);
