import { callAsync, loadSqlEquiv, queryAsync, queryRows, runInTransactionAsync, } from '@prairielearn/postgres';
...
export async function sync(courseId, courseData, questionIds) {
    ...
    const { entitiesToCreate: tagsToCreate, entitiesToUpdate: tagsToUpdate, entitiesToDelete: tagsToDelete, } = determineOperationsForEntities({
    ...
    const newTags = await run(async () => {
        if (!tagsToCreate.length && !tagsToUpdate.length && !tagsToDelete.length)
            return [];
        return await runInTransactionAsync(async () => {
            const insertedTags = await run(async () => {
                if (tagsToCreate.length === 0)
                    return [];
                return queryRows(sql.insert_tags, {
                    course_id: courseId,
                    tags: tagsToCreate.map((t) => JSON.stringify([t.name, t.description, t.color, t.number, t.implicit])),
                }, TagSchema);
            });
            if (tagsToUpdate.length > 0) {
                await queryAsync(sql.update_tags, {
                    course_id: courseId,
                    tags: tagsToUpdate.map((t) => JSON.stringify([t.name, t.description, t.color, t.number, t.implicit])),
                });
            }
