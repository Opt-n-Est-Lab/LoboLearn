...
export async function sync(courseId, courseData) {
    ...
    const { entitiesToCreate: topicsToCreate, entitiesToUpdate: topicsToUpdate, entitiesToDelete: topicsToDelete, } = determineOperationsForEntities({
        ...
        await runInTransactionAsync(async () => {
            if (topicsToCreate.length > 0) {
                await queryAsync(sql.insert_topics, {
                    course_id: courseId,
                    topics: topicsToCreate.map((t) => JSON.stringify([t.name, t.description, t.color, t.number, t.implicit])),
                });
            }
            if (topicsToUpdate.length > 0) {
                await queryAsync(sql.update_topics, {
                    course_id: courseId,
                    topics: topicsToUpdate.map((t) => JSON.stringify([t.name, t.description, t.color, t.number, t.implicit])),
                });
            }
