...
import {} from '../lib/server-jobs.js';
import {} from './course-db.js';
import { isDraftQid } from './question.js';
const sql = sqldb.loadSqlEquiv(import.meta.url);
export async function selectSharedQuestions(courseId) {
    return await sqldb.queryRows(sql.select_shared_questions, { course_id: courseId }, z.object({
        id: IdSchema,
        qid: z.string(),
        share_publicly: z.boolean(),
    }));
}
export function getInvalidRenames(sharedQuestions, courseData, logger) {
    ...
    if (existInvalidRenames) {
        logger.error(`✖ Course sync completely failed. The following questions are shared and cannot be renamed or deleted: ${invalidRenames.join(', ')}`);
    }
    return existInvalidRenames;
}
export function checkInvalidPublicSharingRemovals(sharedQuestions, courseData, logger) {
    const invalidUnshares = [];
    sharedQuestions.forEach((question) => {
    ...
    if (existInvalidUnshares) {
        logger.error(`✖ Course sync completely failed. The following questions are are publicly shared and cannot be unshared: ${invalidUnshares.join(', ')}`);
    }
    return existInvalidUnshares;
}
export async function checkInvalidSharingSetDeletions(courseId, courseData, logger) {
    const sharingSets = await sqldb.queryRows(sql.select_sharing_sets, { course_id: courseId }, z.string());
    const invalidSharingSetDeletions = [];
    ...
    if (existInvalidSharingSetDeletions) {
        logger.error(`✖ Course sync completely failed. The following sharing sets cannot be removed from 'infoCourse.json': ${invalidSharingSetDeletions.join(', ')}`);
    }
    return existInvalidSharingSetDeletions;
}
export function checkInvalidSharingSetAdditions(courseData, logger) {
    const invalidSharingSetAdditions = {};
    const sharingSetNames = (courseData.course.data?.sharingSets || []).map((ss) => ss.name);
    ...
    if (existInvalidSharingSetAdditions) {
        logger.error(`✖ Course sync completely failed. The following questions are being added to sharing sets which do not exist: ${Object.keys(invalidSharingSetAdditions)
            .map((key) => `${key}: ${JSON.stringify(invalidSharingSetAdditions[key])}`)
            .join(', ')}`);
    }
    return existInvalidSharingSetAdditions;
}
export async function checkInvalidSharingSetRemovals(courseId, courseData, logger) {
    const sharedQuestions = await sqldb.queryRows(sql.select_question_sharing_sets, { course_id: courseId }, z.object({
        id: IdSchema,
        qid: z.string(),
        sharing_sets: z.string().array(),
    }));
    const invalidSharingSetRemovals = {};
    sharedQuestions.forEach((question) => {
        if (!courseData.questions[question.qid]) {
    ...
    if (existInvalidSharingSetRemovals) {
        logger.error(`✖ Course sync completely failed. The following questions are not allowed to be removed from the listed sharing sets: ${Object.keys(invalidSharingSetRemovals)
            .map((key) => `${key}: ${JSON.stringify(invalidSharingSetRemovals[key])}`)
            .join(', ')}`);
    }
    return existInvalidSharingSetRemovals;
}
export function checkInvalidSharedAssessments(courseData, logger) {
    const invalidSharedAssessments = new Set();
    ...
    if (existInvalidSharedAssessment) {
        logger.error(`✖ Course sync completely failed. The following assessments have their source publicly shared, but contain questions which are not publicly shared: ${Array.from(invalidSharedAssessments).join(', ')}`);
    }
    return existInvalidSharedAssessment;
}
export function checkInvalidDraftQuestionSharing(courseData, logger) {
    const draftQuestionsWithSharingSets = [];
    const draftQuestionsWithPublicSharing = [];
    for (const qid in courseData.questions) {
        const question = courseData.questions[qid];
    ...
    if (draftQuestionsWithSharingSets.length > 0) {
        logger.error(`✖ Course sync completely failed. The following draft questions cannot be added to sharing sets: ${draftQuestionsWithSharingSets.join(', ')}`);
    }
    if (draftQuestionsWithPublicSharing.length > 0) {
        logger.error(`✖ Course sync completely failed. The following draft questions cannot be publicly shared: ${draftQuestionsWithPublicSharing.join(', ')}`);
    }
