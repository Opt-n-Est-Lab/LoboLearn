...
import { checkInvalidDraftQuestionSharing, checkInvalidPublicSharingRemovals, checkInvalidSharedAssessments, checkInvalidSharingSetAdditions, checkInvalidSharingSetDeletions, checkInvalidSharingSetRemovals, getInvalidRenames, selectSharedQuestions, } from './sharing.js';
export async function checkSharingConfigurationValid(courseId, courseData, logger) {
    ...
    const existInvalidPublicSharingRemovals = checkInvalidPublicSharingRemovals(sharedQuestions, courseData, logger);
    const existInvalidSharingSetDeletions = await checkInvalidSharingSetDeletions(courseId, courseData, logger);
    const existInvalidSharingSetAdditions = checkInvalidSharingSetAdditions(courseData, logger);
    const existInvalidSharingSetRemovals = await checkInvalidSharingSetRemovals(courseId, courseData, logger);
    ...
    const sharingConfigurationValid = !existInvalidRenames &&
...
export async function syncDiskToSqlWithLock(courseId, courseDir, logger, courseData) {
    ...
    if (!courseData) {
        courseData = await timed('Loaded course data from disk', () => courseDB.loadFullCourse(courseId, courseDir));
    }
    const sharingConfigurationValid = await timed('Validated sharing configuration', () => checkSharingConfigurationValid(courseId, courseData, logger));
    ...
    await timed('Synced all course data', async () => {
        ...
        const courseInstanceIds = await timed('Synced course instances', () => syncCourseInstances.sync(courseId, courseData));
        await timed('Synced topics', () => syncTopics.sync(courseId, courseData));
        const questionIds = await timed('Synced questions', () => syncQuestions.sync(courseId, courseData));
        ...
        await timed('Synced assessment modules', () => syncAssessmentModules.sync(courseId, courseData));
        await timed('Synced all assessments', async () => {
            ...
            await async.eachLimit(Object.entries(courseData.courseInstances), 3, async ([ciid, courseInstanceData]) => {
                const courseInstanceId = courseInstanceIds[ciid];
                await timed(`Synced assessments for ${ciid}`, () => syncAssessments.sync(courseId, courseInstanceId, courseInstanceData, questionIds));
            });
        });
    });
    if (config.devMode) {
...
export async function syncDiskToSql(course_id, courseDir, logger) {
    const lockName = getLockNameForCoursePath(courseDir);
    logger.verbose(`Trying lock ${lockName}`);
    const result = await namedLocks.doWithLock(lockName, {
        timeout: 0,
        onNotAcquired: () => {
