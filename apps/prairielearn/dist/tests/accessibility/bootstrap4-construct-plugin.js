...
const BOOTSTRAP_LEGACY_CLASSES = [
    ...
    'form-row',
    'form-inline',
    ...['ml', 'mr', 'pl', 'pr'].flatMap((prefix) => ['', ...BOOTSTRAP_BREAKPOINTS].flatMap((breakpoint) => ['0', '1', '2', '3', '4', '5', 'auto'].map((suffix) => `${prefix}-${breakpoint}${suffix}`))),
    'badge-primary',
    'badge-secondary',
    'badge-success',
    'badge-danger',
    ...
    'rounded-left',
    'rounded-right',
    ...BOOTSTRAP_BREAKPOINTS.flatMap((breakpoint) => ['left', 'right'].map((suffix) => `text-${breakpoint}${suffix}`)),
    'text-monospace',
    'font-weight-light',
    'font-weight-normal',
    'font-weight-bold',
        ...
        this.on('dom:ready', (event) => {
            ...
            BOOTSTRAP_LEGACY_ATTRIBUTES.forEach((attr) => {
                document.querySelectorAll(`[${attr}]`).forEach((node) => {
                    ...
                    if (attr === 'data-content' && node.tagName === 'OPTION')
                        return;
                    this.report({
                        node,
            ...
            BOOTSTRAP_LEGACY_CLASSES.forEach((className) => {
                document.querySelectorAll(`.${className}`).forEach((node) => {
                    ...
                    if (['dropdown-menu-left', 'dropdown-menu-right', 'float-left', 'float-right'].includes(className) &&
            ...
            document.querySelectorAll('.card-header.bg-info.text-white').forEach((node) => {
                this.report({
                    node,
                    message: 'Bootstrap 5 no longer provides sufficient contrast for white text on "info"-colored card headers.',
                });
            });
...
//# sourceMappingURL=bootstrap4-construct-plugin.js.map