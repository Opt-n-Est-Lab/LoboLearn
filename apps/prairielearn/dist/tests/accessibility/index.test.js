...
async function checkPage(url) {
    ...
    for (const result of validationResults.results) {
        result.messages = result.messages.filter((m) => {
            ...
            if (m.ruleId === 'prefer-native-element' &&
                m.selector &&
                [
    ...
    if (axeResults.violations.length > 0) {
        const err = new A11yError(axeResults.violations, A11yResults.convert(axeResults.violations).sort());
        messages += err.format({});
    }
...
describe('accessibility', () => {
    ...
    before('set up testing server', async function () {
        ...
        const firstNewsItemResult = await sqldb.queryOneRowAsync('SELECT id FROM news_items ORDER BY id ASC LIMIT 1', {});
        const assessmentResult = await sqldb.queryOneRowAsync('SELECT id FROM assessments WHERE tid = $tid', { tid: 'hw1-automaticTestSuite' });
        const questionResult = await sqldb.queryOneRowAsync('SELECT id FROM questions WHERE qid = $qid', { qid: 'downloadFile' });
        ...
        await sqldb.queryOneRowAsync('UPDATE questions SET share_publicly = true WHERE id = $question_id', { question_id: routeParams.question_id });
        await sqldb.queryOneRowAsync('UPDATE assessments SET share_source_publicly = true WHERE id = $assessment_id', { assessment_id: routeParams.assessment_id });
        const courseId = await sqldb.queryOneRowAsync('SELECT course_id FROM course_instances WHERE id = $course_instance_id', { course_instance_id: routeParams.course_instance_id });
        await sqldb.queryOneRowAsync('UPDATE pl_courses SET sharing_name = $sharing_name WHERE id = $course_id', { sharing_name: 'test', course_id: courseId.rows[0].course_id });
    });
