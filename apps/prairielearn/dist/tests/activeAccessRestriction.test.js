...
describe('Exam and homework assessment with active access restriction', function () {
    ...
    step('check that an assessment instance was not created', async () => {
        const results = await sqldb.queryRows(sql.select_assessment_instances, AssessmentInstanceSchema);
    ...
    step('start the exam and access questions', async () => {
        ...
        const workspaceQuestionResponse = await helperClient.fetchCheerio(context.examQuestionWithWorkspaceUrl, { headers });
    ...
    step('count number of variants generated', async () => {
        context.numberOfVariants = await sqldb.queryRow(sql.count_variants, {
    ...
    step('ensure that no new variants have been created', async () => {
        const countVariantsResult = await sqldb.queryRow(sql.count_variants, {
            assessment_instance_id: helperClient.parseAssessmentInstanceId(context.examInstanceUrl),
        }, z.number());
    ...
    step('check that no credit is received for an answer submitted when active is false', async () => {
        const points = await sqldb.queryRow(sql.read_assessment_instance_points, {
    ...
    step('check that no files or text were attached', async () => {
        const numberOfFiles = await sqldb.queryRow(sql.get_attached_files, {
            assessment_id: context.hwId,
        }, z.number());
...
//# sourceMappingURL=activeAccessRestriction.test.js.map