...
describe('Course element extensions', function () {
    ...
    describe('Extensions can be loaded', function () {
        ...
        it("shouldn't fail when there are no extensions to load", async () => {
            const extensions = await freeform.loadExtensions(path.join(TEST_COURSE_PATH, 'elementExtensions'), path.join(TEST_COURSE_PATH, 'elementExtensions'));
            assert.isEmpty(extensions, 'non-zero number of extensions were loaded from a course without extensions');
        });
    });
    describe('Extensions can insert client-side assets into the page', function () {
        ...
        step('check the question page for extension css and js files', async () => {
            ...
            const page$ = response.$;
            assert.lengthOf(page$(`script[src$="${incJs}"]`), 1, 'page did not load extension javascript');
            assert.lengthOf(page$(`link[rel="stylesheet"][href$="${incCss}"]`), 1, 'page did not load extension css');
            const importMap = page$('script[type="importmap"]').html();
            const importMapData = JSON.parse(importMap ?? '');
            assert.property(importMapData.imports, incDynamicJsKey, 'importmap did not include dynamic extension js');
            assert.equal(importMapData.imports[incDynamicJsKey].slice(-incDynamicJs.length), incDynamicJs);
        });
...
//# sourceMappingURL=courseElementExtension.test.js.map