...
describe('database', function () {
    ...
    it('should not contain "ON DELETE CASCADE" foreign keys from soft-delete to hard-delete tables', async function () {
        ...
        const [softDeleteTables, hardDeleteTables] = _.partition(Object.keys(data.tables), (table) => data.tables[table].columns.some((column) => column.name === 'deleted_at'));
        for (const table of softDeleteTables) {
            for (const constraint of data.tables[table].foreignKeyConstraints) {
                ...
                if (deleteAction === 'CASCADE' && hardDeleteTables.includes(otherTable)) {
                    throw new Error(`Soft-delete table "${table}" has ON DELETE CASCADE foreign key "${keyName}" to hard-delete table "${otherTable}"`);
                }
            }
        }
    });
