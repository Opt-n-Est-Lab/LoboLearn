...
import { parseInstanceQuestionId, saveOrGrade, setUser } from './helperClient.js';
...
async function waitForExternalGrader($questionsPage) {
    ...
    return new Promise((resolve, reject) => {
        ...
        socket.emit('init', { variant_id: variantId.toString(), variant_token: variantToken.toString() }, (msg) => {
            if (!msg)
...
const loadHomeworkPage = async (user) => {
    ...
    const hm9InternalExternalManualUrl = siteUrl +
        $courseInstancePage('a:contains("Homework for Internal, External, Manual grading methods")').attr('href');
        ...
        describe('"Manual" with auto points only (treat as "Internal")', () => {
            describe('"grade" action', () => {
                it('should load page as student', async () => {
                    ...
                    iqUrl =
                        siteUrl +
                            $hm1Body('a:contains("Manual Grading: Adding two numbers (with auto points)")').attr('href');
                    // open page to produce variant because we want to get the correct answer
                    questionsPage = await (await fetch(iqUrl)).text();
                    $questionsPage = cheerio.load(questionsPage);
                    assert.lengthOf($questionsPage('button[value="grade"]'), 1);
        ...
        describe('"Internal" with manual points only (treat as "Manual")', () => {
            describe('"grade" action', () => {
                it('should load page as student', async () => {
                    ...
                    iqUrl =
                        siteUrl +
                            $hm1Body('a:contains("Internal Grading: Adding two numbers (with manual points)")').attr('href');
                    // open page to produce variant because we want to get the correct answer
                    questionsPage = await (await fetch(iqUrl)).text();
                    $questionsPage = cheerio.load(questionsPage);
                    assert.lengthOf($questionsPage('button[value="grade"]'), 0);
...
//# sourceMappingURL=gradingMethods.test.js.map