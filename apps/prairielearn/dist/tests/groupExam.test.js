...
describe('Group based exam assessments', function () {
    ...
    describe('instructor access for exam assessment', function () {
        step("should load the group tab for the first assessment's instructor URL", async function () {
            ...
            const instructorAssessmentsUrlGroupTab = courseInstanceUrl + '/instructor/assessment/' + assessmentId + '/groups';
    ...
    describe('exam group creation, joining, and starting', function () {
        it('allows group creation, joining, and starting', async function () {
            ...
            assert.equal($('#group-name').text(), GROUP_NAME, 'The group info should contain the correct group name');
            let joinCode = $('#join-code').text();
            assert.lengthOf(joinCode, $('#group-name').text().length + 1 + 4, 'Page must contain the 4-character join code');
            assert.isTrue($('#start-assessment').is(':disabled'), 'Start button must be disabled');
            assert.lengthOf($('.text-center:contains(1 more)'), 1, 'Page must show the group to be missing 1 more member');
            ...
            assert.lengthOf(joinCode, $('#group-name').text().length + 1 + 4, 'Page must contain the 4-character join code');
            assert.isTrue($('#start-assessment').is(':disabled'), 'Start button must be disabled');
            assert.lengthOf($('.text-center:contains(1 more)'), 0, 'Page should not show a warning for more members');
            ...
            const { $: $secondUser } = await switchUserAndLoadAssessment(studentUsers[1], assessmentUrl, 'confirm-form');
            $ = $secondUser;
            ...
            const assessmentInstanceURL = courseInstanceUrl + '/assessment_instance/' + assessmentInstanceId;
...
describe('cross group exam access', function () {
    ...
    it("prevents unauthorized users from accessing other groups' assessment instances", async function () {
        ...
        const { csrfToken: secondUserInstanceCsrfToken } = await switchUserAndLoadAssessment(studentUsers[1], assessmentUrl, // redirects to instance URL
        null, '#leaveGroupModal');
...
describe('cross exam assessment access', function () {
    ...
    it("prevents unauthorized users from accessing other groups' assessment instances", async function () {
        ...
        const studentUsers = await generateThreeStudentUsers();
        // Load exam assessment page as first student
        const { csrfToken: firstUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[0], firstAssessmentUrl, 'create-form');
        // As first user, create group, load the page, and check group information
        let $ = await createGroup(GROUP_NAME, firstUserCsrfToken, firstAssessmentUrl);
        const firstAssessmentJoinCode = $('#join-code').text();
        // Join group as second user
        const { csrfToken: secondUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[1], firstAssessmentUrl, 'joingroup-form');
        ...
        const { csrfToken: thirdUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[2], secondAssessmentUrl, 'joingroup-form');
...
//# sourceMappingURL=groupExam.test.js.map