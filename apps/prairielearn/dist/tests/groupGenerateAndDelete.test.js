...
describe('test random groups and delete groups', function () {
    ...
    step('randomly assign groups', async () => {
        ...
        const job_sequence_id = await groupUpdate.randomGroups(locals.assessment_id, user_id, authn_user_id, max_group_size, min_group_size);
        await helperServer.waitForJobSequenceSuccess(job_sequence_id);
    });
    step('check groups and users', async () => {
        const groupUserCounts = await sqldb.queryAsync('SELECT count(group_id) FROM group_users GROUP BY group_id', []);
    ...
    step('delete groups', async () => {
        ...
        const groups = await sqldb.queryAsync('SELECT deleted_at FROM groups WHERE deleted_at IS NULL', []);
...
//# sourceMappingURL=groupGenerateAndDelete.test.js.map