 ...
 */
async function leaveGroup(assessmentUrl) {
    const res = await fetch(assessmentUrl, {
        method: 'POST',
        body: new URLSearchParams({
 ...
 */
function verifyRoleAssignmentsInRoleTable(roleAssignments) {
 ...
 */
async function updateGroupRoles(roleUpdates, groupRoles, studentUsers, assessmentUrl) {
...
describe('Test group based assessments with custom group roles from student side', function () {
    ...
    step('first user should be able to remove role assignments from second user', async function () {
        ...
        await updateGroupRoles(locals.roleUpdates, locals.groupRoles, locals.studentUsers, locals.assessmentUrl);
    });
    ...
    step('second user can load assessment as manager and see group role select table', async function () {
    ...
    step('database contains correct role configuration after second user leaves and rejoins', async function () {
        const expectedRoleUpdates = [
            { roleId: locals.manager.id, groupUserId: locals.studentUsers[0].user_id },
            { roleId: locals.recorder.id, groupUserId: locals.studentUsers[1].user_id },
        ];
    ...
    step('first user can edit role table to correct role configuration and remove manager role', async function () {
    ...
    step('should have correct role configuration in the database after assigning two recorders', async function () {
        await verifyRoleAssignmentsInDatabase(locals.roleUpdates, locals.assessment_id);
    });
    ...
    step('should remove non-required roles when group size does not exceed minimum required role assignments', async function () {
    ...
    step('should assign fifth user as non-required role when group size exceeds minimum required role assignments', async function () {
    ...
    step('first user can swap recorder and contributor roles between second and fifth user', async function () {
        locals.roleUpdates = [
    ...
    step('should replace non-required role with required role of leaving user when group is big enough', async function () {
        ...
        assert.lengthOf(result.rows, 4);
        const secondUserRoles = result.rows.filter((row) => row.user_id === locals.studentUsers[1].user_id);
        assert.isTrue(secondUserRoles.length === 1);
        const secondUserRole = secondUserRoles[0];
        assert.isTrue(secondUserRole.group_role_id === locals.recorder.id ||
    ...
    step('correct roles are checked in group role table after required roles are transferred', async function () {
        // Switch to assigner
        await switchUserAndLoadAssessment(locals.studentUsers[0], locals.assessmentUrl, '00000001', 3);
        verifyRoleAssignmentsInRoleTable(locals.roleUpdates);
    });
    step('first user can swap reflector and contributor roles between third and fourth user', async function () {
        locals.roleUpdates = [
    ...
    step('should replace non-required role with required role of leaving user when group meets minimum required role assignments', async function () {
    ...
    step('required roles of leaving user are transferred when group size falls below minimum required role assignments', async function () {
        ...
        const firstUserRoleUpdates = result.rows.filter((row) => row.user_id === locals.studentUsers[0].user_id);
    ...
    step('group role table should have correct roles checked after roles transfer upon leave', async function () {
        await switchUserAndLoadAssessment(locals.studentUsers[0], locals.assessmentUrl, '00000001', 3);
        verifyRoleAssignmentsInRoleTable(locals.roleUpdates);
    });
    ...
    step('first user can load assessment without roles', async function () {
        await switchUserAndLoadAssessment(locals.studentUsers[0], locals.assessmentUrlWithoutRoles, '00000001', 2);
    });
...
const changeGroupRolesConfig = async (courseDir, groupRoles) => {
    const infoAssessmentPath = path.join(courseDir, 'courseInstances', 'Sp15', 'assessments', 'hw5-templateGroupWork', 'infoAssessment.json');
...
describe('Test group role reassignments with role of minimum > 1', function () {
    ...
    describe('test correct role config where group size matches minimum required role count', function () {
        step('first user can update roles with correct role configuration without manager', async function () {
            ...
            await updateGroupRoles(locals.roleUpdates, locals.groupRoles, locals.studentUsers, assessmentUrl);
        });
        step('contains correct role configuration with manager role after reassigning', async function () {
            // We expect the db to have all role updates, including the assigner role
            locals.roleUpdates = [...locals.roleUpdates, locals.assignerRoleUpdate];
            await verifyRoleAssignmentsInDatabase(locals.roleUpdates, assessmentId);
    ...
    describe('test correct role config where group size exceeds minimum required role count', function () {
        ...
        step('should have no errors displayed', function () {
            assertAlert(locals.$, 'to be assigned', 0);
            assertAlert(locals.$, 'At least one user does not have a role. All users must have a role.', 0);
        });
    ...
    describe('test incorrect role config where user has no roles', function () {
        ...
        step('should have correct errors displayed', function () {
            assertAlert(locals.$, '1 more student needs to be assigned to the role "Recorder" (1 assigned, 2 expected)');
            assertAlert(locals.$, 'At least one user does not have a role. All users must have a role.');
        });
        step('should not be able to start assessment', function () {
            elemList = locals.$('#start-assessment');
    ...
    describe('test correct role config where group size falls below minimum required role count', function () {
        ...
        step('should have correct role configuration in the database after fourth user leaves', async function () {
            locals.roleUpdates = [
                { roleId: locals.manager.id, groupUserId: locals.studentUsers[0].user_id },
                { roleId: locals.recorder.id, groupUserId: locals.studentUsers[0].user_id },
                { roleId: locals.recorder.id, groupUserId: locals.studentUsers[1].user_id },
        ...
        step('should have correct role configuration in the database after first user leaves', async function () {
            const params = {
                assessment_id: assessmentId,
            };
            const result = await sqldb.queryAsync(sql.select_group_user_roles, params);
            // Ensure that there are two recorder assignments, one manager, and one reflector, and no contributors
            assert.lengthOf(result.rows.filter(({ group_role_id }) => group_role_id === locals.manager.id), 1);
            assert.lengthOf(result.rows.filter(({ group_role_id }) => group_role_id === locals.recorder.id), 2);
            assert.lengthOf(result.rows.filter(({ group_role_id }) => group_role_id === locals.reflector.id), 1);
            assert.lengthOf(result.rows.filter(({ group_role_id }) => group_role_id === locals.contributor.id), 0);
        });
    });
});
describe('Test group role reassignment logic when user leaves', function () {
    ...
    step('should transfer required roles to another user when there are no non-required roles to replace', function () {
        ...
        const result = getGroupRoleReassignmentsAfterLeave(locals.groupInfo, locals.studentUsers[1].user_id);
    ...
    step("should replace another user's non-required role with leaving user's required role", function () {
        // Setup group of 2 users with one user as manager and the other user as recorder
        locals.groupInfo.groupMembers = locals.groupMembers.slice(0, 2);
        locals.groupInfo.groupSize = 2;
        const roleAssignments = [
        ...
        const result = getGroupRoleReassignmentsAfterLeave(locals.groupInfo, locals.studentUsers[0].user_id);
    ...
    step("should replace other users' non-required roles with leaving user's required roles", function () {
        ...
        const secondUserRoleAssignment = result.find(({ user_id }) => user_id === locals.studentUsers[1].user_id);
        assert.isDefined(secondUserRoleAssignment);
        const expected = secondUserRoleAssignment?.group_role_id === locals.manager.id ? expected1 : expected2;
        assert.sameDeepMembers(result, expected);
    });
    step("should replace other users' non-required roles with leaving user's required roles, then transfer after non-required roles run out", function () {
        ...
        assert.lengthOf(result.filter(({ group_role_id }) => group_role_id === locals.manager.id), 1);
        assert.lengthOf(result.filter(({ group_role_id }) => group_role_id === locals.recorder.id), 1);
        assert.lengthOf(result.filter(({ group_role_id }) => group_role_id === locals.reflector.id), 1);
        // Ensure that there are no contributors
        assert.lengthOf(result.filter(({ group_role_id }) => group_role_id === locals.contributor.id), 0);
    });
...
//# sourceMappingURL=groupRole.test.js.map