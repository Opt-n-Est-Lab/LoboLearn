...
import { loadSqlEquiv, queryAsync, queryOneRowAsync, queryRow, queryRows, } from '@prairielearn/postgres';
import { config } from '../lib/config.js';
import { GroupRoleSchema, IdSchema } from '../lib/db-types.js';
 ...
 */
async function joinGroup(assessmentUrl, joinCode, csrfToken) {
    const res = await fetch(assessmentUrl, {
        method: 'POST',
        body: new URLSearchParams({
            __action: 'join_group',
 ...
 */
async function updateGroupRoles(roleUpdates, groupRoles, studentUsers, csrfToken, assessmentUrl, $) {
    ...
    assert.lengthOf(checkedBoxes, roleUpdates.length, 'all checkboxes in role select form must be checked');
...
async function getQuestionUrl(courseInstanceUrl, assessmentInstanceId, questionId) {
    const result = await queryRow(sql.select_instance_questions, {
...
async function prepareGroup() {
    ...
    // Get group roles
    const groupRoles = await queryRows(sql.select_assessment_group_roles, { assessment_id: assessmentId }, GroupRoleSchema.pick({
        id: true,
        role_name: true,
        minimum: true,
        maximum: true,
    ...
    const { csrfToken: firstUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[0], assessmentUrl, 'create-form');
    let $ = await createGroup(GROUP_NAME, firstUserCsrfToken, assessmentUrl);
    const joinCode = $('#join-code').text();
    // Join group as second user
    const { csrfToken: secondUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[1], assessmentUrl, 'joingroup-form');
    await joinGroup(assessmentUrl, joinCode, secondUserCsrfToken);
    // Join group as third user
    const { csrfToken: thirdUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[2], assessmentUrl, 'joingroup-form');
    await joinGroup(assessmentUrl, joinCode, thirdUserCsrfToken);
    // Switch to first user and assign group roles
    const { $: $preJoinFirstUserPage } = await switchUserAndLoadAssessment(studentUsers[0], assessmentUrl, null, '#leaveGroupModal');
    ...
    $ = await updateGroupRoles(validRoleConfig, groupRoles, studentUsers, firstUserCsrfToken, assessmentUrl, $preJoinFirstUserPage);
...
describe('Assessment instance with group roles & permissions - Homework', function () {
    describe('valid group role configuration tests', function () {
        ...
        it('enforces correct permissions during valid group role configuration', async function () {
            const { assessmentInstanceUrl, questionOneUrl, questionTwoUrl, questionThreeUrl, studentUsers, } = await prepareGroup();
            const { $: $assessmentInstanceFirstUserPage } = await switchUserAndLoadAssessment(studentUsers[0], assessmentInstanceUrl, null, '#leaveGroupModal');
            ...
            lockedRows.each((_, element) => {
                ...
                assert.strictEqual(popoverText, 'Your current group role (Manager) restricts access to this question.');
            });
            ...
            const { $: $questionOneThirdUserPage } = await switchUserAndLoadAssessment(studentUsers[2], questionOneUrl, 'attach-file-form');
            ...
            const { $: $questionOneSecondUserPage } = await switchUserAndLoadAssessment(studentUsers[1], questionOneUrl, 'attach-file-form');
            ...
            const { $: $questionOneFirstUserPage, csrfToken: questionOneFirstUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[0], questionOneUrl, 'attach-file-form');
            ...
            assert.strictEqual(popoverContent, 'Your group role (Manager) is not allowed to submit this question.');
            ...
            assert.equal(questionSubmissionWithNoPermissionResponse.status, 403, 'status should be forbidden');
            // Submitting with valid permissions does not yield any errors
            const { csrfToken: questionOneSecondtUserCsrfToken } = await switchUserAndLoadAssessment(studentUsers[1], questionOneUrl, 'attach-file-form');
    ...
    describe('invalid role configuration tests', function () {
        ...
        it('shows correct errors during invalid group role configuration', async function () {
            const { assessmentInstanceUrl, questionOneUrl, validRoleConfig, studentUsers, manager, recorder, reflector, groupRoles, } = await prepareGroup();
            // Assign an invalid configuration
            const { $: $assessmentInstanceFirstUserPage, csrfToken } = await switchUserAndLoadAssessment(studentUsers[0], assessmentInstanceUrl, null, '#leaveGroupModal');
            ...
            let $ = await updateGroupRoles(invalidRoleConfig, groupRoles, studentUsers, csrfToken, assessmentInstanceUrl, $assessmentInstanceFirstUserPage);
            ...
            const { $: assessmentInstanceSecondUserPage } = await switchUserAndLoadAssessment(studentUsers[1], assessmentInstanceUrl, null, '#leaveGroupModal');
            ...
            const { $: $assessmentInstanceFirstUserPage2, csrfToken: firstUserCsrfToken2 } = await switchUserAndLoadAssessment(studentUsers[0], assessmentInstanceUrl, null, '#leaveGroupModal');
            $ = await updateGroupRoles(validRoleConfig, groupRoles, studentUsers, firstUserCsrfToken2, assessmentInstanceUrl, $assessmentInstanceFirstUserPage2);
...
//# sourceMappingURL=groupRolePermissions.test.js.map