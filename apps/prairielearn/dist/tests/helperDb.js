...
import {} from 'mocha';
import pg from 'pg';
import { SCHEMA_MIGRATIONS_PATH, initBatchedMigrations, init as initMigrations, stopBatchedMigrations, } from '@prairielearn/migrations';
...
const postgresTestUtils = sqldb.makePostgresTestUtils({
    ...
    prepareAfterReset: async (client) => {
        ...
        // add it back here.
        await client.query("INSERT INTO institutions (id, long_name, short_name) VALUES (1, 'Default', 'Default') ON CONFLICT DO NOTHING;");
    },
});
async function runMigrationsAndSprocs(dbName, runMigrations) {
    ...
    if (runMigrations) {
        await initMigrations([path.resolve(import.meta.dirname, '..', 'migrations'), SCHEMA_MIGRATIONS_PATH], 'prairielearn');
    }
...
async function databaseExists(dbName) {
    ...
    const result = await client.query(`SELECT exists(SELECT * FROM pg_catalog.pg_database WHERE datname = '${dbName}');`);
 ...
 * desired database.
 */
export async function beforeOnlyCreate() {
    // long timeout because DROP DATABASE might take a long time to error
    // if other processes have an open connection to that database
    this.timeout?.(20000);
    await setupDatabases();
...
export function getDatabaseNameForCurrentWorker() {
 ...
 * use this carefully.
 */
export async function runInTransactionAndRollback(fn) {
    await sqldb
...
//# sourceMappingURL=helperDb.js.map