...
describe('Editing assessment settings', () => {
    ...
    step('change assessment info', async () => {
        const settingsPageResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/assessment/1/settings`);
        assert.equal(settingsPageResponse.status, 200);
        const response = await fetch(`${siteUrl}/pl/course_instance/1/instructor/assessment/1/settings`, {
            method: 'POST',
    ...
    step('verify assessment info change in db', async () => {
        const assessment = await queryRow(sql.select_assessment_by_id, { id: 1 }, AssessmentSchema.extend({
    ...
    step('should not be able to submit if provided assessment id falls outside the correct root directory', async () => {
        const settingsPageResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/assessment/1/settings`);
...
//# sourceMappingURL=instructorAssessmentSettings.test.js.map