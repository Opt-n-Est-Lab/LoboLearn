...
describe('Creating an assessment', () => {
    ...
    step('create a new assessment without module', async () => {
        ...
        const assessmentsPageResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/instance_admin/assessments`);
        assert.equal(assessmentsPageResponse.status, 200);
        // Create the new assessment without a module
        const assessmentCreationResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/instance_admin/assessments`, {
            method: 'POST',
            body: new URLSearchParams({
                __action: 'add_assessment',
                __csrf_token: assessmentsPageResponse.$('input[name=__csrf_token]').val(),
                orig_hash: assessmentsPageResponse.$('input[name=orig_hash]').val(),
                title: 'Test Title',
                aid: 'HW2',
                type: 'Homework',
                set: 'Practice Quiz',
        ...
        assert.equal(assessmentCreationResponse.url, `${siteUrl}/pl/course_instance/1/instructor/assessment/2/questions`);
    });
    step('verify the assessment has the correct info', async () => {
        const assessmentLiveInfoPath = path.join(assessmentLiveDir, 'HW2', // Verify that the aid was used as the assessment folder's name
        'infoAssessment.json');
    ...
    step('create new assessment with module', async () => {
        ...
        assert.equal(assessmentCreationResponse.status, 200);
        assert.equal(assessmentCreationResponse.url, `${siteUrl}/pl/course_instance/1/instructor/assessment/3/questions`);
    });
    step('verify the assessment has the correct info, including the module', async () => {
        const assessmentLiveInfoPath = path.join(assessmentLiveDir, 'HW3', // Verify that the aid was used as the assessment folder's name
        'infoAssessment.json');
        const assessmentInfo = JSON.parse(await fs.readFile(assessmentLiveInfoPath, 'utf8'));
    ...
    step('create new assessment with duplicate aid, title', async () => {
        ...
        assert.equal(assessmentCreationResponse.status, 200);
        assert.equal(assessmentCreationResponse.url, `${siteUrl}/pl/course_instance/1/instructor/assessment/4/questions`);
    });
    step('verify that the title and aid had 2 appended to them', async () => {
        const assessmentLiveInfoPath = path.join(assessmentLiveDir, 'HW2_2', // Verify that the aid was used as the assessment folder's name
        'infoAssessment.json');
        const assessmentInfo = JSON.parse(await fs.readFile(assessmentLiveInfoPath, 'utf8'));
    ...
    step('should not be able to create an assessment without fields', async () => {
        ...
        assert.equal(assessmentCreationResponse.url, `${siteUrl}/pl/course_instance/1/instructor/instance_admin/assessments`);
    });
    step('should not be able to create an assessment with aid not contained in the root directory', async () => {
        // Fetch the assessments page for the course instance
        const assessmentsPageResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/instance_admin/assessments`);
        assert.equal(assessmentsPageResponse.status, 200);
...
//# sourceMappingURL=instructorAssessments.test.js.map