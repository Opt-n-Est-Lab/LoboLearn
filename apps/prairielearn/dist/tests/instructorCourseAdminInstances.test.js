...
describe('Creating a course instance', () => {
    ...
    step('create a new course instance', async () => {
        ...
        const courseInstancePageResponse = await fetchCheerio(`${siteUrl}/pl/course/1/course_admin/instances`);
        assert.equal(courseInstancePageResponse.status, 200);
        // Create the new course instance
        const courseInstanceCreationResponse = await fetchCheerio(`${siteUrl}/pl/course/1/course_admin/instances`, {
            method: 'POST',
        ...
        assert.equal(courseInstanceCreationResponse.url, `${siteUrl}/pl/course_instance/2/instructor/instance_admin/assessments`);
    });
    step('verify course instance has the correct info', async () => {
        const courseInstanceInfoPath = path.join(courseInstancesCourseLiveDir, 'Fa19', // Verify that the short_name has been used as the course instance folder's name
    ...
    step('add course instance with the same long_name and short_name', async () => {
        ...
        assert.equal(courseInstanceCreationResponse.status, 200);
        // Verify that the user is redirected to the assessments page for the new course instance
        assert.equal(courseInstanceCreationResponse.url, `${siteUrl}/pl/course_instance/3/instructor/instance_admin/assessments`);
    });
    step('verify that the new course instance names had 2 appended to them', async () => {
        const courseInstanceInfoPath = path.join(courseInstancesCourseLiveDir, 'Fa19_2', 'infoCourseInstance.json');
        const courseInstanceInfo = JSON.parse(await fs.readFile(courseInstanceInfoPath, 'utf8'));
    ...
    step('add course instance without start_access_date and end_access_date', async () => {
        ...
        assert.equal(courseInstanceCreationResponse.status, 200);
        // Verify that the user is redirected to the assessments page for the new course instance
        assert.equal(courseInstanceCreationResponse.url, `${siteUrl}/pl/course_instance/4/instructor/instance_admin/assessments`);
    });
    step('verify course instance is created with an empty allowAccess array', async () => {
        const courseInstanceInfoPath = path.join(courseInstancesCourseLiveDir, 'Fa20', 'infoCourseInstance.json');
        const courseInstanceInfo = JSON.parse(await fs.readFile(courseInstanceInfoPath, 'utf8'));
    ...
    step('add course instance with access_dates_enabled unchecked', async () => {
        ...
        assert.equal(courseInstanceCreationResponse.status, 200);
        // Verify that the user is redirected to the assessments page for the new course instance
        assert.equal(courseInstanceCreationResponse.url, `${siteUrl}/pl/course_instance/5/instructor/instance_admin/assessments`);
    });
    step('verify course instance is created with an empty allowAccess array', async () => {
        const courseInstanceInfoPath = path.join(courseInstancesCourseLiveDir, 'Sp21', 'infoCourseInstance.json');
        const courseInstanceInfo = JSON.parse(await fs.readFile(courseInstanceInfoPath, 'utf8'));
    ...
    step('should not be able to create course instance with no short_name', async () => {
        ...
        assert.equal(courseInstanceCreationResponse.url, `${siteUrl}/pl/course/1/course_admin/instances`);
    });
    ...
    step('should not be able to create course instance with short_name that falls outside correct root directory', async () => {
        const courseInstancePageResponse = await fetchCheerio(`${siteUrl}/pl/course/1/course_admin/instances`);
        assert.equal(courseInstancePageResponse.status, 200);
...
//# sourceMappingURL=instructorCourseAdminInstances.test.js.map