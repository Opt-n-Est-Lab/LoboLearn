...
describe('Editing question settings', () => {
    ...
    step('change question info', async () => {
        const settingsPageResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/question/1/settings`);
    ...
    step('verify question id changed', async () => {
        questionLiveInfoPath = path.join(questionLiveDir, 'question2', // The new question id
        'info.json');
    ...
    step('should not be able to submit if changed question id is not in the root directory', async () => {
        const settingsPageResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/question/1/settings`);
        assert.equal(settingsPageResponse.status, 200);
...
//# sourceMappingURL=instructorQuestionSettings.test.js.map