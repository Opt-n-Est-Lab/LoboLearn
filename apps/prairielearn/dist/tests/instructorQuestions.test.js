...
import { testElementClientFiles, testFileDownloads, testQuestionPreviews, } from './helperQuestionPreview.js';
...
describe('Instructor questions', function () {
    ...
    describe('QID redirect routes', () => {
        it('redirects to the correct question from course route', async () => {
            ...
            assert.equal(res.status, 200);
            assert.equal(res.url, `${baseUrl}/course/1/question/${addNumbers.id}/preview?variant_seed=1234`);
        });
        it('redirects to the correct question from instance route', async () => {
            ...
            assert.equal(res.status, 200);
            assert.equal(res.url, `${baseUrl}/course_instance/1/instructor/question/${addNumbers.id}/preview?variant_seed=1234`);
        });
    });
});
//# sourceMappingURL=instructorQuestions.test.js.map