...
describe('Creating a question', () => {
    ...
    step('create a new empty question', async () => {
        ...
        const questionsResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/course_admin/questions`);
        assert.equal(questionsResponse.status, 200);
        // Create the new empty question
        const createQuestionResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/course_admin/questions`, {
            method: 'POST',
            body: new URLSearchParams({
                __action: 'add_question',
                __csrf_token: questionsResponse.$('input[name=__csrf_token]').val(),
                orig_hash: questionsResponse.$('input[name=orig_hash]').val(),
                title: 'Test Question',
                qid: 'test-question',
        ...
        assert.equal(createQuestionResponse.url, `${siteUrl}/pl/course_instance/1/instructor/question/2/file_edit/questions/test-question/question.html`);
    });
    step('verify that the new empty question has the correct info', async () => {
        const questionLiveInfoPath = path.join(questionsLiveDir, 'test-question', // Verify that the qid was used as the question folder's name
        'info.json');
    ...
    step('create a new template question', async () => {
        ...
        assert.equal(createQuestionResponse.status, 200);
        assert.equal(createQuestionResponse.url, `${siteUrl}/pl/course_instance/1/instructor/question/3/preview`);
    });
    step('verify that the new template question has the correct info', async () => {
        ...
        const originalQuestionServerFilePath = path.join(EXAMPLE_COURSE_PATH, 'questions', 'template', 'matrix-component-input', 'random-graph', 'server.py');
        const newQuestionServerFileContent = await fs.readFile(newQuestionServerFilePath, 'utf8');
        const originalQuestionServerFileContent = await fs.readFile(originalQuestionServerFilePath, 'utf8');
        ...
        const originalQuestionHtmlFilePath = path.join(EXAMPLE_COURSE_PATH, 'questions', 'template', 'matrix-component-input', 'random-graph', 'question.html');
    ...
    step('create new question with duplicate qid, title', async () => {
        ...
        assert.equal(createQuestionResponse.status, 200);
        assert.equal(createQuestionResponse.url, `${siteUrl}/pl/course_instance/1/instructor/question/4/file_edit/questions/test-question_2/question.html`);
    });
    step('verify that the title and qid had 2 appended to them', async () => {
        const questionLiveInfoPath = path.join(questionsLiveDir, 'test-question_2', // Verify that the qid with 2 appended to it was used as the name of the question folder
        'info.json');
        const questionInfo = JSON.parse(await fs.readFile(questionLiveInfoPath, 'utf8'));
    ...
    step('should not be able to create a question with qid not contained in the root directory', async () => {
        // Fetch the questions page for the course instance
        const questionsResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/course_admin/questions`);
        assert.equal(questionsResponse.status, 200);
    ...
    step('should not be able to create a question from a non-existent template question', async () => {
        // Fetch the questions page for the course instance
        const questionsResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/course_admin/questions`);
        assert.equal(questionsResponse.status, 200);
        ...
        assert.equal(createQuestionResponse.status, 200);
        assert.match(createQuestionResponse.url, /\/pl\/course_instance\/1\/instructor\/edit_error\/\d+$/);
    });
    step('should not be able to create a question with template_qid not contained in the root directory', async () => {
        // Fetch the questions page for the course instance
        const questionsResponse = await fetchCheerio(`${siteUrl}/pl/course_instance/1/instructor/course_admin/questions`);
        assert.equal(questionsResponse.status, 200);
...
//# sourceMappingURL=instructorQuestionsCreation.test.js.map