...
describe('LTI 1.3', () => {
    ...
    step('create an LTI instance', async () => {
        ...
        const ltiInstancesResponse = await fetchCheerio(`${siteUrl}/pl/administrator/institution/1/lti13`);
        ...
        const createInstanceResponse = await fetchCheerio(`${siteUrl}/pl/administrator/institution/1/lti13`, {
            method: 'POST',
            body: new URLSearchParams({
                __csrf_token: newInstanceForm.find('input[name=__csrf_token]').val(),
                __action: newInstanceButton.attr('value'),
            }),
        });
    ...
    step('configure an LTI instance', async () => {
        const ltiInstanceResponse = await fetchCheerio(`${siteUrl}/pl/administrator/institution/1/lti13/1`);
        ...
        const updatePlatformOptionsResponse = await fetchCheerio(`${siteUrl}/pl/administrator/institution/1/lti13/1`, {
            method: 'POST',
            body: new URLSearchParams({
                __csrf_token: platformOptionsForm.find('input[name=__csrf_token]').val(),
                __action: platformOptionsForm.find('input[name=__action]').val(),
                platform: 'Unknown',
        ...
        // Create a key
        const createKeyResponse = await fetchCheerio(`${siteUrl}/pl/administrator/institution/1/lti13/1`, {
            method: 'POST',
            body: new URLSearchParams({
                __csrf_token: keystoreForm.find('input[name=__csrf_token]').val(),
                __action: addKeyButton.attr('value'),
            }),
        });
    ...
    step('enable LTI 1.3 as an authentication provider', async () => {
        ...
        const enableLtiResponse = await fetchCheerio(`${siteUrl}/pl/administrator/institution/1/sso`, {
            ...
            body: new URLSearchParams({
                ...
                __action: saveButton.attr('value'),
    ...
    step('perform login', async () => {
        ...
        const redirectUrl = new URL(startLoginResponse.headers.get('location'));
        ...
        assert.equal(redirectUrl.searchParams.get('redirect_uri'), `${siteUrl}/pl/lti13_instance/1/auth/callback`);
        ...
        const fakeIdToken = await new jose.SignJWT({
            ...
            'https://purl.imsglobal.org/spec/lti/claim/deployment_id': '7fdce954-4c33-47c9-97b4-e435dbbed9bb',
    ...
    step('validate login', async () => {
        ...
        const ltiUser = await queryOptionalRow('SELECT * FROM lti13_users WHERE user_id = $user_id', {
    ...
    step('malformed requests fail', async () => {
        const fetchWithCookies = fetchCookie(fetchCheerio);
        // Malformed login
        const startBadLoginResponse = await fetchWithCookies(`${siteUrl}/pl/lti13_instance/1/auth/login`, {
            method: 'POST',
            body: new URLSearchParams({
                iss: siteUrl,
    ...
    step('request access token', async () => {
        ...
        app.post('/token', async (req, res) => {
            ...
            assert.equal(req.body.client_assertion_type, 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer');
            // Decodes but does not validate
            const jwt = JSON.parse(Buffer.from(req.body.client_assertion.split('.')[1], 'base64').toString());
...
describe('fetchRetry()', async () => {
    ...
    step('should return the full list by iterating with intermittant 403s', async () => {
        apiCount = 0;
        await withServer(app, apiProviderPort, async () => {
            const resultArray = await fetchRetryPaginated(baseUrl + '403oddAttempt', {}, { sleepMs: 100 });
            assert.equal(resultArray.length, 3);
            const products = z.string().array().array().parse(resultArray);
            const fullList = products.flat();
            assert.equal(fullList.length, 26);
...
//# sourceMappingURL=lti13.test.js.map