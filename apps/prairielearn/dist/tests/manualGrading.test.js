...
import { assertAlert, parseInstanceQuestionId, saveOrGrade, setUser, } from './helperClient.js';
 ...
 */
async function loadHomeworkPage(user) {
    setUser(user);
    ...
    const hm9InternalExternalManualUrl = siteUrl + $courseInstancePage(`a:contains("${assessmentTitle}")`).attr('href');
 ...
 */
async function loadHomeworkQuestionUrl(user) {
...
async function submitGradeForm(method = 'rubric') {
    ...
    if (adjust_points)
        params.append('score_manual_adjust_points', adjust_points.toString());
    ...
    if (method === 'percentage')
        params.append('use_score_perc', 'on');
...
function checkGradingResults(assigned_grader, grader) {
    step('manual grading page for instance question lists updated values', async () => {
        ...
        if (rubric_items) {
            ...
            assert.equal(form.find('input[name=score_manual_adjust_points]').val(), (adjust_points ?? '').toString());
        }
        else {
    ...
    step('manual grading page for assessment question lists updated values', async () => {
        setUser(defaultUser);
        const manualGradingAQData = await (await fetch(manualGradingAssessmentQuestionUrl + '/instances.json')).text();
    ...
    step('next ungraded button should point to general page after grading', async () => {
        ...
        assert.equal(nextUngraded.headers.get('location'), new URL(manualGradingAssessmentQuestionUrl).pathname);
        setUser(mockStaff[1]);
    ...
    step('student view should have the new score/feedback/rubric', async () => {
        ...
        assert.equal(getLatestSubmissionStatus($questionsPage), `manual grading: ${Math.floor(score_percent)}%`);
        assert.equal($questionsPage('#question-score-panel tr:contains("Total points") [data-testid="awarded-points"]')
            .first()
            .text()
            .trim(), `${score_points}`);
        assert.equal(feedbackBlock.find('[data-testid="feedback-body"]').first().text().trim(), feedback_note);
        if (!rubric_items) {
            ...
            rubric_items.forEach((item, index) => {
                ...
                if (item.always_show_to_students || selected_rubric_items.includes(index)) {
                    ...
                    assert.equal(container.find('input[type="checkbox"]').is(':checked'), selected_rubric_items.includes(index));
                    assert.equal(container.find('[data-testid="rubric-item-points"]').text().trim(), `[${item.points >= 0 ? '+' : ''}${item.points}]`);
                    assert.equal(container.find('[data-testid="rubric-item-description"]').html()?.trim(), item.description_render ?? item.description);
                    if (item.explanation) {
                        assert.equal(container.find('[data-testid="rubric-item-explanation"]').attr('data-bs-content'), item.explanation_render ?? `<p>${item.explanation}</p>`);
                    }
                    else {
        ...
        if (adjust_points) {
            assert.equal(feedbackBlock.find('[data-testid="rubric-adjust-points"]').text().trim(), `[${adjust_points >= 0 ? '+' : ''}${adjust_points}]`);
        }
        else {
...
function checkSettingsResults(starting_points, min_points, max_extra_points) {
    step('rubric settings modal should update with new values', async () => {
        const manualGradingIQPage = await (await fetch(manualGradingIQUrl)).text();
        const $manualGradingIQPage = cheerio.load(manualGradingIQPage);
        const form = $manualGradingIQPage('form[name=rubric-settings]');
        assert.equal(form.find(`input[name="starting_points"][value="${starting_points}"]`).is(':checked'), true);
        ...
        rubric_items.forEach((item, index) => {
            ...
            const explanation = form.find(`button[data-input-name="rubric_item[cur${item.id}][explanation]"]`);
            assert.equal(explanation.attr('data-current-value') ?? '', item.explanation ?? '');
            const graderNote = form.find(`button[data-input-name="rubric_item[cur${item.id}][grader_note]"]`);
            assert.equal(graderNote.attr('data-current-value') ?? '', item.grader_note ?? '');
            const always_show_to_students = form.find(`[name="rubric_item[cur${item.id}][always_show_to_students]"]:checked`);
    ...
    step('grading panel should have proper values for rubric', async () => {
        ...
        rubric_items.forEach((item) => {
            ...
            assert.equal(container.find('[data-testid="rubric-item-points"]').text().trim(), `[${item.points >= 0 ? '+' : ''}${item.points}]`);
            assert.equal(container.find('[data-testid="rubric-item-description"]').html()?.trim(), item.description_render ?? item.description);
            assert.equal(container.find('[data-testid="rubric-item-explanation"]').html()?.trim(), item.explanation_render ?? (item.explanation ? `<p>${item.explanation}</p>` : ''));
            assert.equal(container.find('[data-testid="rubric-item-grader-note"]').html()?.trim(), item.grader_note_render ?? (item.grader_note ? `<p>${item.grader_note}</p>` : ''));
        });
    });
}
function buildRubricItemFields(items) {
    return Object.fromEntries(Object.entries(_.mapKeys(items, (item, index) => (item.id ? `cur${item.id}` : `new${index}`))).flatMap(([key, item], order) => Object.entries({ order, ...item }).map(([field, value]) => [
...
describe('Manual Grading', function () {
    ...
    before('add staff users', async () => {
        await Promise.all(mockStaff.map(async (staff) => {
    ...
    describe('Submit and grade a manually graded question', () => {
        describe('Student submission tags question for grading', () => {
            ...
            step('submit an answer to the question', async () => {
                ...
                assert.equal(getLatestSubmissionStatus($questionsPage), 'manual grading: waiting for grading');
            });
        ...
        describe('Manual grading behavior while instance is open', () => {
            ...
            step('manual grading page for assessment question should warn about an open instance', async () => {
                setUser(defaultUser);
            ...
            step('manual grading page for instance question should warn about an open instance', async () => {
                setUser(defaultUser);
                const manualGradingIQPage = await (await fetch(manualGradingIQUrl)).text();
                const $manualGradingIQPage = cheerio.load(manualGradingIQPage);
        ...
        describe('Manual grading behaviour when instance is closed', () => {
            step('close assessment', async () => {
                ...
                const token = $instancesBody('#grade-all-form').find('input[name=__csrf_token]').attr('value') || '';
            ...
            step('manual grading page for assessment question should NOT warn about an open instance', async () => {
                setUser(defaultUser);
                const manualGradingAQPage = await (await fetch(manualGradingAssessmentQuestionUrl)).text();
                const $manualGradingAQPage = cheerio.load(manualGradingAQPage);
                assertAlert($manualGradingAQPage, 'has one open instance', 0);
            });
            step('manual grading page for instance question should NOT warn about an open instance', async () => {
                setUser(defaultUser);
                const manualGradingIQPage = await (await fetch(manualGradingIQUrl)).text();
                const $manualGradingIQPage = cheerio.load(manualGradingIQPage);
        ...
        describe('Assigning grading to staff members', () => {
            ...
            step('manual grading page should NOT show next ungraded button for non-assigned grader', async () => {
                setUser(mockStaff[1]);
                const manualGradingPage = await (await fetch(manualGradingAssessmentUrl)).text();
                $manualGradingPage = cheerio.load(manualGradingPage);
                const row = $manualGradingPage(`tr:contains("${manualGradingQuestionTitle}")`);
            ...
            describe('Negative grading', () => {
                step('set rubric settings to negative grading should succeed', async () => {
                    ...
                    rubric_items = [
                        ...
                        {
                            points: 1.6,
                            description: 'Positive rubric item in negative grading (positive points, floating point)',
                            always_show_to_students: false,
                        },
                        {
                            points: 6,
        ...
        describe('New submission after manual grading', () => {
            ...
            step('student view should keep the old feedback/rubric', async () => {
                ...
                assert.equal(submissions.eq(1).find('[data-testid="feedback-body"]').first().text().trim(), feedback_note);
            });
            step('manual grading page should warn about an open instance', async () => {
                setUser(defaultUser);
                const manualGradingPage = await (await fetch(manualGradingAssessmentUrl)).text();
...
//# sourceMappingURL=manualGrading.test.js.map