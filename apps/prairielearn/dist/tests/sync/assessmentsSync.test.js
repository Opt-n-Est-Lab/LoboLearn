...
import { AlternativeGroupSchema, AssessmentQuestionRolePermissionsSchema, AssessmentQuestionSchema, AssessmentSchema, GroupRoleSchema, QuestionSchema, ZoneSchema, } from '../../lib/db-types.js';
...
function getPermission(permissions, groupRole, assessmentQuestion) {
    return permissions.find((permission) => parseInt(permission.assessment_question_id) === parseInt(assessmentQuestion.id) &&
        parseInt(permission.group_role_id) === parseInt(groupRole.id));
}
async function getSyncedAssessmentData(tid) {
    return await sqldb.queryRow(sql.get_data_for_assessment, { tid }, z.object({
...
describe('Assessment syncing', () => {
    ...
    it('syncs alternatives in an Exam zone', async () => {
        ...
        const thirdAssessmentQuestion = syncedData.assessment_questions.find((aq) => aq.question.qid === util.MANUAL_GRADING_QUESTION_ID);
    ...
    it('reuses assessment questions when questions are removed and added again', async () => {
        ...
        if (!removedQuestion)
            throw new Error('removedQuestion is null');
        await util.overwriteAndSyncCourseData(courseData, courseDir);
    ...
    it('removes an access rule from an exam', async () => {
        ...
        const rulesForAssessment = syncedAssessmentAccessRules.filter((aar) => idsEqual(aar.assessment_id, originalSyncedAssessment?.id));
    ...
    it('sets mode to Exam if an access rule specifies an examUuid but not mode=Exam', async () => {
        ...
        const syncedAssessmentAccessRules = await util.dumpTable('assessment_access_rules');
        const rulesForAssessment = syncedAssessmentAccessRules.filter((aar) => idsEqual(aar.assessment_id, syncedAssessment?.id));
        assert.lengthOf(rulesForAssessment, 1);
    ...
    it('syncs empty arrays correctly', async () => {
        ...
        const assessmentAccessRule = assessmentAccessRules.find((aar) => idsEqual(aar.assessment_id, syncedAssessment?.id));
    ...
    it('syncs group roles and valid question-level permissions correctly', async () => {
        ...
        const firstQuestionRecorderPermission = getPermission(syncedPermissions, recorder, firstAssessmentQuestion);
        assert.isDefined(firstQuestionRecorderPermission);
        assert.isTrue(firstQuestionRecorderPermission?.can_view && firstQuestionRecorderPermission?.can_submit, 'recorder should have permission to view and submit first question');
        const firstQuestionContributorPermission = getPermission(syncedPermissions, contributor, firstAssessmentQuestion);
        assert.isDefined(firstQuestionContributorPermission);
        assert.isTrue(firstQuestionContributorPermission?.can_view &&
            !firstQuestionContributorPermission?.can_submit, 'contributor should only have permission to view first question');
        const secondQuestionRecorderPermission = getPermission(syncedPermissions, recorder, secondAssessmentQuestion);
        assert.isDefined(secondQuestionRecorderPermission);
        assert.isTrue(secondQuestionRecorderPermission?.can_view && secondQuestionRecorderPermission?.can_submit, 'recorder should have permission to view and submit second question');
        const secondQuestionContributorPermission = getPermission(syncedPermissions, contributor, secondAssessmentQuestion);
        assert.isOk(secondQuestionContributorPermission);
        assert.isTrue(!secondQuestionContributorPermission?.can_view &&
            !secondQuestionContributorPermission?.can_submit, 'contributor should not be able to view or submit second question');
    });
    ...
    it('removes group roles and role permissions correctly upon re-sync', async () => {
        ...
        // Check permissions
        const syncedPermissions = await util.dumpTable('assessment_question_role_permissions');
        assert.equal(syncedPermissions.filter((p) => parseInt(p.group_role_id) === parseInt(foundRecorder?.id))
            .length, 2);
        assert.equal(syncedPermissions.filter((p) => parseInt(p.group_role_id) === parseInt(foundContributor?.id))
            .length, 2);
        ...
        if (!lastZone)
            throw new Error('could not find last zone');
        lastZone.questions = [
            {
                id: util.QUESTION_ID,
                points: 5,
        ...
        assert.notEqual(newSyncedRoles.find((role) => role.role_name === 'Recorder'), undefined);
        ...
        assert.equal(newSyncedPermissions.filter((p) => parseInt(p.group_role_id) === parseInt(foundRecorder?.id))
            .length, 2);
        assert.equal(newSyncedPermissions.filter((p) => parseInt(p.group_role_id) === parseInt(foundContributor?.id)).length, 0);
    });
    it('records an error if a question has permissions for non-existent group roles', async () => {
        ...
        assert.match(syncedAssessment?.sync_errors, /The zone question's "canView" permission contains the non-existent group role name "Invalid"./);
    });
    it('records an error if a zone has permissions for non-existent group roles', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('groupAssessmentFail');
        assert.match(syncedAssessment?.sync_errors, /The zone's "canView" permission contains the non-existent group role name "Invalid"./);
    });
    it('records an error if an assessment has permissions for non-existent group roles', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('groupAssessmentFail');
        assert.match(syncedAssessment?.sync_errors, /The assessment's "canView" permission contains the non-existent group role name "Invalid"./);
    });
    it('records an error if there is no group role with minimum > 0 that can reassign roles', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('groupAssessmentFail');
        assert.match(syncedAssessment?.sync_errors, /Could not find a role with minimum >= 1 and "canAssignRoles" set to "true"./);
    });
    it('records an error if group role max/min are greater than the group maximum', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('groupAssessmentFail');
        assert.match(syncedAssessment?.sync_errors, /Group role "Manager" contains an invalid minimum. \(Expected at most 4, found 10\)./);
        assert.match(syncedAssessment?.sync_errors, /Group role "Reflector" contains an invalid maximum. \(Expected at most 4, found 10\)./);
    });
    it('removes deleted question-level permissions correctly', async () => {
        ...
        const firstQuestionContributorPermission = newSyncedPermissions.find((p) => parseInt(p.assessment_question_id) === parseInt(firstAssessmentQuestion.id) &&
        ...
        const secondQuestionContributorPermission = newSyncedPermissions.find((p) => parseInt(p.assessment_question_id) === parseInt(secondAssessmentQuestion.id) &&
            parseInt(p.group_role_id) === parseInt(foundContributor?.id));
        assert.isTrue(secondQuestionContributorPermission?.can_view);
    ...
    it('isolates roles/permissions to the assessment they are defined in', async () => {
        ...
        const syncedPermissions = await util.dumpTableWithSchema('assessment_question_role_permissions', AssessmentQuestionRolePermissionsSchema);
        ...
        const firstQuestionPermission = syncedPermissions.find((p) => {
            return (p.assessment_question_id === firstAssessmentQuestion.id &&
        ...
        const secondQuestionPermission = syncedPermissions.find((p) => {
            return (p.assessment_question_id === secondAssessmentQuestion.id &&
    ...
    it('handles assessment sets that are not present in infoCourse.json', async () => {
        ...
        assert.isTrue(syncedAssessmentSet?.heading && syncedAssessmentSet.heading.length > 0, 'assessment set should not have empty heading');
    ...
    it('handles assessment modules that are not present in infoCourse.json', async () => {
        ...
        let syncedAssessmentModule = syncedAssessmentModules.find((amod) => amod.name === missingAssessmentModuleName);
        assert.isOk(syncedAssessmentModule);
        assert.isTrue(syncedAssessmentModule.implicit);
        assert.isTrue(syncedAssessmentModule?.heading && syncedAssessmentModule.heading.length > 0, 'assessment module should not have empty heading');
        ...
        syncedAssessmentModule = syncedAssessmentModules.find((amod) => amod.name === missingAssessmentModuleName);
    ...
    it('records an error if an access rule specifies an examUuid and mode=Public', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_warnings, /Invalid allowAccess rule: examUuid cannot be used with "mode": "Public"/);
    });
    ...
    it('records an error if a question specifies maxPoints on an Exam-type assessment', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_errors, /Cannot specify "maxPoints" or "maxAutoPoints" for a question in an "Exam" assessment/);
    });
    it('records an error if a question does not specify points on an Exam-type assessment', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_errors, /Must specify "points", "autoPoints" or "manualPoints" for a question/);
    });
    ...
    it('records an error if a question specifies points and autoPoints', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_errors, /Cannot specify "points" for a question if "autoPoints", "manualPoints" or "maxAutoPoints" are specified/);
    });
    ...
    it('records an error if a question specifies maxPoints and autoPoints', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_errors, /Cannot specify "maxPoints" for a question if "autoPoints", "manualPoints" or "maxAutoPoints" are specified/);
    });
    it('records an error if a question specifies points as an array on a Homework-type assessment', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_errors, /Cannot specify "points" or "autoPoints" as a list for a question in a "Homework" assessment/);
    });
    ...
    it('records a warning if a question has zero autoPoints and non-zero maxAutoPoints on a Homework-type assessment', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.match(syncedAssessment?.sync_errors, /Cannot specify "autoPoints": 0 when "maxAutoPoints" > 0/);
    });
    ...
    describe('Test validating shared quesitons on sync', () => {
        ...
        it('records an error if a zone references a QID from another course that does not exist or we do not have permissions for', async () => {
            ...
            await util.writeAndSyncCourseData(courseData);
            const syncedAssessment = await findSyncedAssessment('fail');
            assert.match(syncedAssessment?.sync_errors, /For each of the following, either the course you are referencing does not exist, or the question does not exist within that course: @example-course\/i do not exist/);
        });
    });
    ...
    it('records a warning if the same UUID is used multiple times in one course instance', async () => {
        ...
        assert.match(syncedAssessment1?.sync_warnings, /UUID ".*" is used in other assessments in this course instance: fail2/);
        const syncedAssessment2 = await findSyncedAssessment('fail2');
        assert.match(syncedAssessment2?.sync_warnings, /UUID ".*" is used in other assessments in this course instance: fail1/);
    });
    ...
    it('records an error if multipleInstance is true for Homework-type assessments', async () => {
        ...
        await util.writeAndSyncCourseData(courseData);
        const syncedAssessment = await findSyncedAssessment('fail');
        assert.equal(syncedAssessment?.sync_errors, '"multipleInstance" cannot be used for Homework-type assessments');
    });
    ...
    it('forbids draft questions on assessments', async () => {
        ...
        assert.match(syncedData.assessment?.sync_errors, /The following questions are marked as draft and therefore cannot be used in assessments: "__drafts__\/draft_1"/);
    });
    describe('exam UUID validation', () => {
        let originalCheckAccessRulesExamUuid;
        before(() => {
        ...
        it('validates exam UUIDs for assessments in an accessible course instances', async () => {
            ...
            if (!courseInstance)
                throw new Error('missing courseInstance');
            ...
            await util.writeAndSyncCourseData(courseData);
            const syncedAssessment = await findSyncedAssessment('fail');
            assert.match(syncedAssessment?.sync_warnings, /examUuid "00000000-0000-0000-0000-000000000000" not found./);
...
//# sourceMappingURL=assessmentsSync.test.js.map