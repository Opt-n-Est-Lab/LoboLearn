...
describe('Course instance syncing', () => {
    ...
    it('syncs access rules', async () => {
        ...
        assert.equal(remainingRule.start_date.getTime(), new Date('2024-02-01T06:00:00.000Z').getTime());
    ...
    it('syncs empty arrays correctly', async () => {
        ...
        const courseData = util.getCourseData();
        const courseInstanceAllowAccessRule = courseData.courseInstances[util.COURSE_INSTANCE_ID].courseInstance.allowAccess?.[0];
        ...
        const syncedAccessRules = (await util.dumpTable('course_instance_access_rules')).filter((ar) => idsEqual(ar.course_instance_id, syncedCourseInstance?.id));
    ...
    it('handles unique sequence of renames and duplicate UUIDs', async () => {
        ...
        const newCourseInstanceRow1 = courseInstances.find((ci) => ci.short_name === 'b' && ci.deleted_at === null);
        ...
        const newCourseInstanceRow2 = courseInstances.find((q) => q.short_name === 'c' && q.deleted_at === null);
...
//# sourceMappingURL=courseInstancesSync.test.js.map