...
describe('Question syncing', () => {
    ...
    it('syncs empty arrays correctly', async () => {
        ...
        assert.isArray(syncedQuestion?.external_grading_files, 'external_grading_files should be an array');
        assert.isEmpty(syncedQuestion?.external_grading_files, 'external_grading_files should be empty');
    });
    ...
    it('preserves question tag even if question tag is deleted', async () => {
        ...
        const syncedQuestionTag = syncedQuestionTags.find((qt) => idsEqual(qt.question_id, newSyncedQuestion?.id) && idsEqual(qt.tag_id, syncedTag?.id));
        assert.isTrue(syncedTag?.implicit);
...
//# sourceMappingURL=questionsSync.test.js.map