...
export async function enrollUser(courseInstanceId, user) {
...
export async function unenrollUser(courseInstanceId, user) {
    return await withUser(user, async () => {
        const url = baseUrl + '/enroll';
        return await fetch(url, {
            method: 'POST',
...
export async function enrollRandomUsers(courseInstanceId, count) {
    // Get current number of enrolled students.
    const currentCount = await queryRow('SELECT COUNT(*)::integer FROM enrollments WHERE course_instance_id = $id', { id: courseInstanceId }, z.number());
