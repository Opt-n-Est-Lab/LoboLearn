...
async function checkFileContents(targetPath, filename, expectedContents) {
...
describe('Workspace dynamic files', function () {
    it('succeeds with valid dynamic files', async () => {
        ...
        // Static files.
        await checkFileContents(targetPath.path, 'starter_code.c', '// starter_code.c\n\n#include <stdio.h>\n#include "starter_code.h"\n\nint main()\n{\n    // enter your code\n}\n');
        ...
        await checkFileContents(targetPath.path, 'binary.bin', '\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F');
        // Template files.
        ...
        await checkFileContents(targetPath.path, 'path/with/multiple/components.py', 'b = "WORD"\nprint(b)\n');
        // Dynamic files.
        await checkFileContents(targetPath.path, 'first_dynamic_file.py', 'a, b = b, a\nprint(a, b)\n');
        await checkFileContents(targetPath.path, 'second_dynamic_file.bin', '\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F');
        await checkFileContents(targetPath.path, 'path/with/multiple/dynamic/components.bin', '\x00\x11\x22\x33\x44\x55\x66\x77\x88\x99\xAA\xBB\xCC\xDD\xEE\xFF');
        await checkFileContents(targetPath.path, 'blank_file.txt', '');
        await checkFileContents(targetPath.path, 'reference_file.txt', 'This is included in the workspace.\n');
        ...
        await checkFileContents(targetPath.path, 'not_normalized.txt', 'File identified by path that is not normalized\n');
        // Ensure that template files override static files.
        await checkFileContents(targetPath.path, 'template_and_static.txt', 'The correct answer is 35.\n');
    ...
    it('fails with invalid dynamic files', async () => {
        ...
        for (const expectedError of expectedErrors) {
            ...
            if (expectedError.contents != null) {
                ...
                await fs.access(join(targetPath.path, expectedError.file)).then(() => assert.fail(`File ${expectedError.file} should not exist`), (err) => assert.equal(err.code, 'ENOENT'));
            }
        }
        assert.lengthOf(fileGenerationErrors, 8);
    });
});
//# sourceMappingURL=workspaceDynamicFiles.test.js.map