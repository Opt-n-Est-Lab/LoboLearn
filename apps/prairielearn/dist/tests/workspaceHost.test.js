...
async function insertWorkspaceHost(id, state = 'launching') {
    return sqldb.queryRow('INSERT INTO workspace_hosts (id, instance_id, state) VALUES ($id, $instance_id, $state) RETURNING *;', {
        id,
        instance_id: uuidv4(),
        state,
    }, WorkspaceHostSchema);
...
async function insertWorkspace(id, hostId = null) {
    return sqldb.queryRow('INSERT INTO workspaces (id, state, workspace_host_id) VALUES ($id, $state, $workspace_host_id) RETURNING *;', {
        id,
        state: 'launching',
...
async function selectWorkspaceHost(id) {
    return sqldb.queryRow('SELECT * FROM workspace_hosts WHERE id = $id;', { id }, WorkspaceHostSchema);
}
...
async function getWorkspaceHostLogs(id) {
    return sqldb.queryRows('SELECT * FROM workspace_host_logs WHERE workspace_host_id = $id;', { id }, WorkspaceHostLogsSchema);
}
async function getWorkspaceLogs(id) {
    return sqldb.queryRows('SELECT * FROM workspace_logs WHERE workspace_id = $id;', { id }, WorkspaceLogsSchema);
}
describe('workspaceHost utilities', function () {
    ...
    describe('findTerminableWorkspaceHosts', () => {
        ...
        it('marks unhealthy host that exceeded timeout as terminating', async () => {
            ...
            await sqldb.queryAsync("UPDATE workspace_hosts SET unhealthy_at = NOW() - INTERVAL '1 hour', load_count = 5 WHERE id = $id;", { id: host1.id });
            const host2 = await insertWorkspaceHost(2, 'unhealthy');
            await sqldb.queryAsync("UPDATE workspace_hosts SET unhealthy_at = NOW() - INTERVAL '10 seconds', load_count = 5 WHERE id = $id;", { id: host2.id });
        ...
        it('marks launching host that exceeded timeout as terminating', async () => {
            ...
            await sqldb.queryAsync("UPDATE workspace_hosts SET launched_at = NOW() - INTERVAL '1 hour', load_count = 5 WHERE id = $id;", { id: host1.id });
            const host2 = await insertWorkspaceHost(2, 'launching');
            await sqldb.queryAsync("UPDATE workspace_hosts SET launched_at = NOW() - INTERVAL '10 seconds', load_count = 5 WHERE id = $id;", { id: host2.id });
            // Only the first host should be terminated; the second one hasn't
    ...
    describe('terminateWorkspaceHostsIfNotLaunching', () => {
        it('terminates hosts that are not launching', async () => {
            ...
            assert.isDefined(workspaces.find((w) => w.workspace_id === workspace2.id && w.state === 'stopped'));
            assert.isDefined(workspaces.find((w) => w.workspace_id === workspace3.id && w.state === 'stopped'));
            const hostLogs1 = await getWorkspaceHostLogs(1);
...
//# sourceMappingURL=workspaceHost.test.js.map