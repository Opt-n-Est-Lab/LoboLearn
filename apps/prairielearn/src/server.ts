...
if ('h' in argv || 'help' in argv) {
  const msg = `LoboLearn command line options:
...
/**
 * Creates the express application and sets up all LoboLearn routes.
...
export async function initExpress(): Promise<Express> {
  ...
  // Serve element statics. As with core LoboLearn assets and files served
...
export async function stopServer() {
  ...
  //
  // In production environments, LoboLearn should always be deployed behind
  // a load balancer that will drain and close any open connections before
  // LoboLearn is stopped.
...
if (esMain(import.meta) && config.startServer) {
  try {
    logger.verbose('LoboLearn server start');
    ...
    logger.error('Error initializing LoboLearn server:', err);
    throw err;
  }
  logger.info('LoboLearn server ready, press Control-C to quit');
  if (config.devMode) {
