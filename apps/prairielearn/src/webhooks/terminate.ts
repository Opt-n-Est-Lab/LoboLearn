 ...
 * gracefully.
 *
 * This app requires a signed `LoboLearn-Signature` JWT header to be present,
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!jwt) {
      res.status(403).send('Missing LoboLearn-Signature header');
      return;
    }
    if (Array.isArray(jwt)) {
      res.status(403).send('Multiple LoboLearn-Signature headers');
      return;
    }
    ...
    try {
      ...
      logger.error('Error decoding LoboLearn-Signature header', err);
      res.status(403).send(`Invalid LoboLearn-Signature header: ${err.message}`);
      return;
    }
