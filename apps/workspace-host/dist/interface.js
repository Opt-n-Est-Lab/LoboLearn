...
let server;
const workspace_server_settings = {};
...
app.post('/', asyncHandler(async (req, res) => {
    ...
    if (workspace_id == null) {
        ...
        const useInitialZip = req.body.options?.useInitialZip ?? false;
...
async
    ...
    async () => {
        ...
        for (const container of allContainers) {
            ...
            if (result.rows.some((ws) => ws.id === containerWorkspaceId))
                continue;
            logger.info(`Killing dangling container ${container.Id} for workspace ${containerWorkspaceId}`);
...
async function pruneRunawayContainers() {
    ...
    await async.each(running_workspaces, async (container_info) => {
        if (container_info.Names.length !== 1)
            return;
...
async function killAndRemoveWorkspace(workspace_id, container) {
    ...
    try {
        await workspaceUtils.updateWorkspaceDiskUsage(workspace_id.toString(), config.workspaceHostHomeDirRoot);
    }
    catch (err) {
 ...
 */
async function _allocateContainerPort(workspace_id) {
    ...
    return _allocateContainerPortMutex.runExclusive(async () => {
        ...
        const max_attempts = config.workspaceHostMaxPortAllocationAttempts > 0
        ...
        for (let i = 0; !done && i < max_attempts; i++) {
            ...
            port =
                config.workspaceHostMinPortRange +
                    Math.floor(Math.random() * (config.workspaceHostMaxPortRange - config.workspaceHostMinPortRange));
            if (!(await check_port_db(port)))
                continue;
            if (!(await check_port_server(port)))
                continue;
            done = true;
        }
        if (!done || !port) {
        ...
        function checkWorkspace() {
            ...
            fetch(`http://${hostname}:${port}/`, {
                ...
                if (endTime - startTime > startTimeout) {
                    ...
                    reject(new Error(`Max startup time exceeded for workspace ${id} (version ${version}, launch uuid ${launch_uuid})`));
                }
                else {
...
const ProgressDetailsSchema = z.record(z.object({
    current: z.number(),
    total: z.number(),
}));
async function _getCachedProgressDetails(workspace_image) {
...
async function _pullImage(workspace) {
    ...
    await new Promise((resolve, reject) => {
        docker.modem.followProgress(stream, (err) => {
            ...
            cache.set(`workspaceProgressInit:${workspace_image}`, progressDetailsInit, 1000 * 60 * 60 * 24 * 30);
            workspaceUtils
...
async function _createContainer(workspace) {
    ...
    await fs.chown(workspaceJobPath, config.workspaceJobsDirectoryOwnerUid, config.workspaceJobsDirectoryOwnerGid);
...
async function _startContainer(workspace_id, container) {
...
async function initSequence(workspace_id, useInitialZip, res) {
    ...
    const { version, course_id, institution_id } = (await sqldb.queryOneRowAsync(sql.select_workspace, { workspace_id })).rows[0];
    ...
    try {
        ...
        const workspace = await run(async () => {
            try {
                ...
                safeUpdateWorkspaceState(workspace_id, 'stopped', 'Error configuring workspace. Click "Reboot" to try again.');
            }
        });
        if (!workspace)
        ...
        try {
            ...
            safeUpdateWorkspaceState(workspace.id, 'stopped', 'Error pulling image. Click "Reboot" to try again.');
            return; // don't set host to unhealthy
        }
        let container;
        try {
            ...
            safeUpdateWorkspaceState(workspace.id, 'stopped', 'Error creating container. Click "Reboot" to try again.');
            return; // don't set host to unhealthy
        }
        try {
            ...
            const hostname = await sqldb.runInTransactionAsync(async () => {
                ...
                if (launch_uuid !== workspace.launch_uuid) {
                    logger.info(`Abandoning container for workspace ${workspace.id}: relaunched with launch_uuid ${launch_uuid}`);
                    return null;
                }
            ...
            safeUpdateWorkspaceState(workspace.id, 'stopped', 'Error starting container. Click "Reboot" to try again.');
...
async function sendGradedFilesArchive(workspace_id, res) {
    ...
    let gradedFiles;
    try {
        gradedFiles = await workspaceUtils.getWorkspaceGradedFiles(workspaceDir, workspaceSettings.workspace_graded_files, {
