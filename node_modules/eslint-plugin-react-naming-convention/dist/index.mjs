...
function create(context) {
  ...
  const { rule } = options;
  const collector = ER2.useComponentCollector(context);
  const collectorLegacy = ER2.useComponentCollectorLegacy();
  return {
    ...collector.listeners,
...
function isValidName(name3, options) {
  ...
  switch (options.rule) {
    case "CONSTANT_CASE":
      return RegExp.CONSTANT_CASE.test(normalized);
    case "PascalCase":
...
function create2(context) {
  ...
  return {
    CallExpression(node) {
      if (!ER2.isCreateContextCall(context, node)) return;
      const id = ER2.getInstanceId(node);
      if (id == null) return;
      const name3 = match(id).with({ type: AST_NODE_TYPES.Identifier, name: P.select() }, identity).with({ type: AST_NODE_TYPES.MemberExpression, property: { name: P.select(P.string) } }, identity).otherwise(() => null);
      if (name3 != null && ER2.isComponentName(name3) && name3.endsWith("Context")) return;
      context.report({
        messageId: "invalid",
        node: id
      });
...
function create3(context) {
  ...
  function validate(name3, casing = rule, ignores = excepts) {
    ...
    return match(casing).with("PascalCase", () => RegExp.PASCAL_CASE.test(name3)).with("camelCase", () => RegExp.CAMEL_CASE.test(name3)).with("kebab-case", () => RegExp.KEBAB_CASE.test(name3)).with("snake_case", () => RegExp.SNAKE_CASE.test(name3)).exhaustive();
  }
...
function create5(context) {
  ...
  const isUseStateCall = ER2.isReactHookCallWithNameAlias(context, "useState", alias);
  return {
    CallExpression(node) {
      ...
      const setterName = match(setter).with({ type: AST_NODE_TYPES.Identifier }, (id2) => id2.name).otherwise(() => null);
