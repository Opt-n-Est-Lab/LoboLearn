...
function create29(context) {
  ...
  function getReportDescriptor(node) {
    ...
    if (AST8__namespace.is(types.AST_NODE_TYPES.JSXExpressionContainer)(node)) return getReportDescriptor(node.expression);
    if (AST8__namespace.isJSX(node)) return eff._;
    if (AST8__namespace.isTypeExpression(node)) return getReportDescriptor(node.expression);
    return tsPattern.match(node).with({ type: types.AST_NODE_TYPES.LogicalExpression, operator: "&&" }, ({ left, right }) => {
      ...
      const isLeftNan = left.type === types.AST_NODE_TYPES.Identifier && left.name === "NaN" || VAR2__namespace.toStaticValue({ kind: "lazy", node: left, initialScope }).value === "NaN";
      if (isLeftNan) {
        return {
      ...
      const variable = VAR2__namespace.findVariable(n.name, context.sourceCode.getScope(n));
      const variableDefNode = variable?.defs.at(0)?.node;
      return tsPattern.match(variableDefNode).with({ init: tsPattern.P.select({ type: tsPattern.P.not(types.AST_NODE_TYPES.VariableDeclaration) }) }, getReportDescriptor).otherwise(() => eff._);
    }).otherwise(() => eff._);
  }
  return {
    JSXExpressionContainer: eff.flow(getReportDescriptor, kit.Reporter.make(context).send)
  };
}
...
function create30(context) {
  ...
  const {
    ctx,
    listeners
  } = ER25__namespace.useComponentCollector(
    context,
    {
      collectDisplayName: true,
      collectHookCalls: false,
      hint: ER25__namespace.DEFAULT_COMPONENT_DETECTION_HINT
    }
  );
  return {
    ...listeners,
    "Program:exit"(program) {
      ...
      for (const { node, displayName, flag } of components.values()) {
        const isMemoOrForwardRef = (flag & (ER25__namespace.ComponentFlag.ForwardRef | ER25__namespace.ComponentFlag.Memo)) > 0n;
        if (AST8__namespace.getFunctionId(node) != null) {
          continue;
        }
...
function create31(context) {
  ...
  return {
    CallExpression(node) {
      if (!ER25__namespace.isCreateContextCall(context, node)) return;
      createCalls.push(node);
    },
    "Program:exit"() {
      for (const call of createCalls) {
        const id = ER25__namespace.getInstanceId(call);
        if (id == null) {
          context.report({
        ...
        const hasDisplayNameAssignment = displayNameAssignments.some((node) => {
          ...
          return ER25__namespace.isInstanceIdEqual(context, id, object);
        });
...
function create32(context) {
  ...
  function checkIteratorElement(node) {
    switch (node.type) {
      case types.AST_NODE_TYPES.JSXElement: {
        const initialScope = context.sourceCode.getScope(node);
        if (!ER25__namespace.hasAttribute(context, "key", node.openingElement.attributes, initialScope)) {
          return {
  ...
  function checkBlockStatement(node) {
    const descriptors = [];
    for (const statement of AST8__namespace.getNestedReturnStatements(node)) {
  ...
  return {
    ArrayExpression(node) {
      ...
      const elements = node.elements.filter(AST8__namespace.is(types.AST_NODE_TYPES.JSXElement));
      if (elements.length === 0) {
        return;
      }
      const initialScope = context.sourceCode.getScope(node);
      for (const element of elements) {
        if (!ER25__namespace.hasAttribute(context, "key", element.openingElement.attributes, initialScope)) {
          report.send({
            messageId: "missingKey",
            node: element
          });
    ...
    CallExpression(node) {
      state.isWithinChildrenToArray ||= ER25__namespace.isChildrenToArrayCall(context, node);
      if (state.isWithinChildrenToArray) return;
      const callback = tsPattern.match(node).when(AST8__namespace.isArrayMapCall, (n) => n.arguments[0]).when(AST8__namespace.isArrayFromCall, (n) => n.arguments[1]).otherwise(() => null);
      if (!AST8__namespace.isFunction(callback)) return;
    ...
    "CallExpression:exit"(node) {
      if (!ER25__namespace.isChildrenToArrayCall(context, node)) {
        return;
      }
...
function create33(context) {
  ...
  return {
    CallExpression(node) {
      if (!ER25__namespace.isCaptureOwnerStackCall(context, node)) return;
      if (AST8__namespace.findParentNode(node, isDevelopmentOnlyCheck) == null) {
        context.report({
...
function isDevelopmentOnlyCheck(node) {
  if (node.type !== types.AST_NODE_TYPES.IfStatement) return false;
  if (AST8__namespace.isProcessEnvNodeEnvCompare(node.test, "!==", "production")) return true;
  return false;
}
...
function create34(context) {
  const hint = ER25__namespace.ComponentDetectionHint.SkipArrayMapArgument | ER25__namespace.ComponentDetectionHint.SkipNullLiteral | ER25__namespace.ComponentDetectionHint.SkipUndefined | ER25__namespace.ComponentDetectionHint.SkipBooleanLiteral | ER25__namespace.ComponentDetectionHint.SkipStringLiteral | ER25__namespace.ComponentDetectionHint.SkipNumberLiteral | ER25__namespace.ComponentDetectionHint.StrictLogical | ER25__namespace.ComponentDetectionHint.StrictConditional;
  const collector = ER25__namespace.useComponentCollector(context, { hint });
  const collectorLegacy = ER25__namespace.useComponentCollectorLegacy();
  return {
    ...collector.listeners,
    ...
    "Program:exit"(program) {
      ...
      const isFunctionComponent = (node) => {
        return AST8__namespace.isFunction(node) && functionComponents.some((component) => component.node === node);
      };
      const isClassComponent12 = (node) => {
        return AST8__namespace.isClass(node) && classComponents.some((component) => component.node === node);
      };
      for (const { name: name5, node: component } of functionComponents) {
        if (name5 == null) continue;
        if (ER25__namespace.isDirectValueOfRenderPropertyLoose(component)) continue;
        if (isInsideJSXAttributeValue(component)) {
          if (!ER25__namespace.isDeclaredInRenderPropLoose(component)) {
            context.report({
        ...
        const parentComponent = AST8__namespace.findParentNode(component, isFunctionComponent);
        if (parentComponent != null && !ER25__namespace.isDirectValueOfRenderPropertyLoose(parentComponent)) {
          context.report({
            messageId: "noNestedComponentDefinitions",
            node: component,
            data: {
              name: name5,
              suggestion: component.parent.type === types.AST_NODE_TYPES.Property ? "Move it to the top level or pass it as a prop." : "Move it to the top level."
            }
          });
          continue;
        }
        if (ER25__namespace.isInsideRenderMethod(component)) {
          context.report({
            messageId: "noNestedComponentDefinitions",
            node: component,
            data: {
      ...
      for (const { name: name5 = "unknown", node: component } of classComponents) {
        if (AST8__namespace.findParentNode(component, (n) => isClassComponent12(n) || isFunctionComponent(n)) == null) {
          continue;
        }
        context.report({
          messageId: "noNestedComponentDefinitions",
...
function isInsideJSXAttributeValue(node) {
  return node.parent.type === types.AST_NODE_TYPES.JSXAttribute || ER25__namespace.findParentAttribute(node, (n) => n.value?.type === types.AST_NODE_TYPES.JSXExpressionContainer) != null;
}
function isInsideCreateElementProps(context, node) {
  const call = AST8__namespace.findParentNode(node, ER25__namespace.isCreateElementCall(context));
...
function create35(context) {
  const hint = ER25__namespace.ComponentDetectionHint.None;
  const collector = ER25__namespace.useComponentCollector(context, { hint });
  const collectorLegacy = ER25__namespace.useComponentCollectorLegacy();
  ...
  return {
    ...
    ImportExpression(node) {
      const lazyCall = AST8__namespace.findParentNode(node, (n) => ER25__namespace.isLazyCall(context, n));
      if (lazyCall != null) {
    ...
    "Program:exit"(program) {
      ...
      for (const lazy of lazyComponentDeclarations) {
        const significantParent = AST8__namespace.findParentNode(lazy, (n) => {
          if (AST8__namespace.isJSX(n)) return true;
          if (n.type === types.AST_NODE_TYPES.CallExpression) {
            return ER25__namespace.isReactHookCall(n) || ER25__namespace.isCreateElementCall(context, n) || ER25__namespace.isCreateContextCall(context, n);
          }
          if (AST8__namespace.isFunction(n)) {
            return functionComponents.some((c) => c.node === n);
          }
          if (AST8__namespace.isClass(n)) {
...
function create36(context) {
  ...
  return {
    AssignmentExpression(node) {
      ...
      const variable = VAR2__namespace.findVariable(object.name, context.sourceCode.getScope(node));
      const variableNode = VAR2__namespace.getVariableInitNode(variable, 0);
      if (variableNode != null && (AST8__namespace.isFunction(variableNode) || ER25__namespace.isClassComponent(variableNode))) {
...
function isShouldComponentUpdate(node) {
  return AST8__namespace.isMethodOrProperty(node) && node.key.type === types.AST_NODE_TYPES.Identifier && node.key.name === "shouldComponentUpdate";
}
  ...
  return {
    ...
    "Program:exit"(program) {
      ...
      for (const { name: name5 = "PureComponent", node: component, flag } of components.values()) {
        if ((flag & ER25__namespace.ComponentFlag.PureComponent) === 0n) {
          continue;
        }
        const { body } = component.body;
        for (const member of body) {
...
function create38(context) {
  ...
  return {
    CallExpression(node) {
      ...
      const clazz = AST8__namespace.findParentNode(node, ER25__namespace.isClassComponent);
      const method = AST8__namespace.findParentNode(node, (n) => n === clazz || ER25__namespace.isComponentDidMount(n));
...
function create39(context) {
  ...
  return {
    CallExpression(node) {
      ...
      const clazz = AST8__namespace.findParentNode(node, ER25__namespace.isClassComponent);
      const method = AST8__namespace.findParentNode(node, (n) => n === clazz || ER25__namespace.isComponentDidUpdate(n));
      if (clazz == null || method == null || method === clazz) return;
      const methodScope = context.sourceCode.getScope(method);
      const upperScope = context.sourceCode.getScope(node).upper;
      if (method.parent === clazz.body && upperScope === methodScope) {
...
function create40(context) {
  ...
  return {
    CallExpression(node) {
      ...
      const clazz = AST8__namespace.findParentNode(node, ER25__namespace.isClassComponent);
      const method = AST8__namespace.findParentNode(node, (n) => n === clazz || ER25__namespace.isComponentWillUpdate(n));
      if (clazz == null || method == null || method === clazz) return;
      const methodScope = context.sourceCode.getScope(method);
      const upperScope = context.sourceCode.getScope(node).upper;
      if (method.parent === clazz.body && upperScope === methodScope) {
...
function create41(context) {
  ...
  function onClassBodyEnter(node) {
    if (ER25__namespace.isClassComponent(node.parent)) {
...
function getAttributeValueText(context, node) {
  ...
  switch (true) {
    case (node.type === types.AST_NODE_TYPES.Literal && typeof node.value === "string"):
      return context.sourceCode.getText(node);
    case (node.type === types.AST_NODE_TYPES.JSXExpressionContainer && node.expression.type === types.AST_NODE_TYPES.Literal && typeof node.expression.value === "string"):
      return context.sourceCode.getText(node.expression);
    case (node.type === types.AST_NODE_TYPES.JSXExpressionContainer && node.expression.type === types.AST_NODE_TYPES.TemplateLiteral):
      return context.sourceCode.getText(node.expression);
    default:
      return null;
  }
...
function isUnsafeComponentWillMount(node) {
  return AST8__namespace.isMethodOrProperty(node) && node.key.type === types.AST_NODE_TYPES.Identifier && node.key.name === "UNSAFE_componentWillMount";
}
...
function isUnsafeComponentWillReceiveProps(node) {
  return AST8__namespace.isMethodOrProperty(node) && node.key.type === types.AST_NODE_TYPES.Identifier && node.key.name === "UNSAFE_componentWillReceiveProps";
}
...
function isUnsafeComponentWillUpdate(node) {
  return AST8__namespace.isMethodOrProperty(node) && node.key.type === types.AST_NODE_TYPES.Identifier && node.key.name === "UNSAFE_componentWillUpdate";
}
...
function create45(context) {
  ...
  const { ctx, listeners } = ER25__namespace.useComponentCollector(context);
  const constructions = /* @__PURE__ */ new WeakMap();
  return {
    ...listeners,
    JSXOpeningElement(node) {
      const fullName = ER25__namespace.getElementType(context, node.parent);
      ...
      const construction = VAR2__namespace.getConstruction(valueExpression, initialScope);
      if (construction == null) return;
      if (ER25__namespace.isReactHookCall(construction.node)) {
        return;
      }
      eff.getOrElseUpdate(constructions, functionEntry.node, () => []).push(construction);
    },
    "Program:exit"(program) {
      ...
      for (const { node: component } of components) {
        for (const construction of constructions.get(component) ?? []) {
          ...
          context.report({
            ...
            data: {
              type: AST8__namespace.toDelimiterCaseType(constructionNode),
              suggestion
            }
          });
        }
...
function create46(context) {
  ...
  return {
    ...
    "Program:exit"(program) {
      ...
      for (const { node: component } of components.values()) {
        ...
        for (const prop of properties) {
          ...
          const construction = VAR2__namespace.getConstruction(
            value,
            initialScope,
            VAR2__namespace.ConstructionDetectionHint.StrictCallExpression
          );
          ...
          const forbiddenType = AST8__namespace.toDelimiterCaseType(right);
          context.report({
    ...
    [kit.Selector.OBJECT_DESTRUCTURING_VARIABLE_DECLARATOR](node) {
      const functionEntry = ctx.getCurrentEntry();
      if (functionEntry == null) return;
      eff.getOrElseUpdate(
        declarators,
...
function create47(context) {
  ...
  function classEnter(node) {
    ...
    if (!ER25__namespace.isClassComponent(node)) {
      return;
    }
  ...
  function classExit() {
    ...
    if (currentClass == null || !ER25__namespace.isClassComponent(currentClass)) {
      return;
    }
    const className = AST8__namespace.getClassId(currentClass)?.name;
    ...
    for (const def of defs) {
      const methodName = AST8__namespace.getPropertyName(def);
      if (methodName == null) {
        continue;
      }
  ...
  return {
    ...
    MemberExpression(node) {
      ...
      if (!ER25__namespace.isClassComponent(currentClass) || currentMethod.static) {
        return;
      }
      if (!AST8__namespace.isThisExpression(node.object) || !isKeyLiteral2(node, node.property)) {
        return;
      }
      ...
      const propertyName = AST8__namespace.getPropertyName(node.property);
      ...
      if (node.init != null && AST8__namespace.isThisExpression(node.init) && node.id.type === types.AST_NODE_TYPES.ObjectPattern) {
        for (const prop of node.id.properties) {
          if (prop.type === types.AST_NODE_TYPES.Property && isKeyLiteral2(prop, prop.key)) {
            const keyName = AST8__namespace.getPropertyName(prop.key);
            if (keyName != null) {
...
function create48(context) {
  ...
  function methodEnter(node) {
    ...
    if (node.static) {
      if (ER25__namespace.isGetDerivedStateFromProps(node) && tsPattern.isMatching({ params: [tsPattern.P.nonNullable, ...tsPattern.P.array()] })(node.value)) {
        const defNode = stateDefs.get(currentClass)?.node;
        stateDefs.set(currentClass, { node: defNode, isUsed: true });
      }
      return;
    }
    if (AST8__namespace.getPropertyName(node.key) === "state") {
  ...
  return {
    AssignmentExpression(node) {
      if (!ER25__namespace.isAssignmentToThisState(node)) {
        return;
      }
      const currentClass = classEntries.at(-1);
      if (currentClass == null || !ER25__namespace.isClassComponent(currentClass)) {
    ...
    VariableDeclarator(node) {
      ...
      const hasState = node.id.properties.some((prop) => {
        if (prop.type === types.AST_NODE_TYPES.Property && isKeyLiteral3(prop, prop.key)) {
          return AST8__namespace.getPropertyName(prop.key) === "state";
        }
        return false;
      });
      if (!hasState) {
...
function create49(context) {
  ...
  return {
    CallExpression(node) {
      if (!ER25__namespace.isReactHookCall(node)) {
        return;
      }
      hookCalls.add(node);
    },
    ...
    "Program:exit"() {
      const isUseContextCall = ER25__namespace.isReactHookCallWithNameAlias(context, "useContext", [...useContextNames]);
...
function create50(context) {
  return {
    CallExpression(node) {
      ...
      if (component == null || !AST8__namespace.isFunction(component)) {
        return;
      }
...
function create51(context, [option]) {
  ...
  return {
    JSXElement(node) {
      if (!ER25__namespace.isFragmentElement(context, node)) return;
...
function isPaddingSpaces(node) {
  return ER25__namespace.isJsxText(node) && isWhiteSpace(node) && node.raw.includes("\n");
}
...
function checkNode(context, node, allowExpressions) {
  const initialScope = context.sourceCode.getScope(node);
  if (ER25__namespace.isKeyedElement(context, node, initialScope)) {
    return;
  }
  if (ER25__namespace.isHostElement(context, node.parent)) {
    context.report({
  ...
  const isChildElement = AST8__namespace.isOneOf([types.AST_NODE_TYPES.JSXElement, types.AST_NODE_TYPES.JSXFragment])(node.parent);
  switch (true) {
    // <Foo content={<>ee eeee eeee ...</>} />
    case (allowExpressions && !isChildElement && node.children.length === 1 && ER25__namespace.isJsxText(node.children.at(0))): {
      return;
    }
    // <Foo><>hello, world</></Foo>
...
function canFix(context, node) {
  if (node.parent.type === types.AST_NODE_TYPES.JSXElement || node.parent.type === types.AST_NODE_TYPES.JSXFragment) {
    return ER25__namespace.isHostElement(context, node.parent);
  }
  if (node.children.length === 0) {
    return false;
  }
  if (node.children.some((child) => ER25__namespace.isJsxText(child) && !isWhiteSpace(child) || AST8__namespace.is(types.AST_NODE_TYPES.JSXExpressionContainer)(child))) {
    return false;
  }
  return true;
}
  ...
  return {
    ...
    "Program:exit"(program) {
      ...
      function isFunctionComponent(block) {
        if (!AST8__namespace.isFunction(block)) {
          return false;
        }
        const id = AST8__namespace.getFunctionId(block);
        return id != null && ER25__namespace.isComponentNameLoose(id.name) && components.some((component) => component.node === block);
      }
      for (const [initialScope, memberExpression] of memberExpressionWithNames) {
        ...
        const isMatch = (node) => node != null && node.type === types.AST_NODE_TYPES.Identifier && node.name === memberExpression.object.name;
        if (isMatch(props)) {
          context.report({
...
function isTypeReadonlyLoose(services, type) {
  if (typeUtils.isTypeReadonly(services.program, type)) return true;
  try {
...
function create55(context) {
  return {
    JSXAttribute(node) {
      const { value } = node;
      const propName = ER25__namespace.getAttributeName(context, node);
