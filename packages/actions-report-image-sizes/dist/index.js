...
const DockerApiTokenSchema = zod_1.z.object({
    token: zod_1.z.string(),
});
const DockerApiManifestListSchema = zod_1.z.object({
...
const DockerApiManifestSchema = zod_1.z.object({
    mediaType: zod_1.z.union([
        zod_1.z.literal('application/vnd.docker.distribution.manifest.v2+json'),
        zod_1.z.literal('application/vnd.oci.image.manifest.v1+json'),
    ]),
...
const DockerApiImageManifestSchema = zod_1.z.union([
...
async function getDockerHubToken(image) {
    const tokenResponse = await fetch(`https://auth.docker.io/token?service=registry.docker.io&scope=repository:${image}:pull`);
    const { token } = DockerApiTokenSchema.parse(await tokenResponse.json());
    return token;
}
async function getImageManifest(token, image, reference) {
...
async function getAllImagesFromRegistry(image, sha) {
    ...
    // being "unknown".
    const imageManifests = manifest.manifests.filter((m) => m.platform.os !== 'unknown' && m.platform.architecture !== 'unknown');
    const sizes = [];
    for (const imageManifest of imageManifests) {
        ...
        if (platformManifest.mediaType !== 'application/vnd.docker.distribution.manifest.v2+json' &&
...
async function commentSizeReport(title, changedImages) {
    ...
    const existingComment = comments.find((comment) => comment.body?.startsWith(`## ${title}`) && comment.user?.login === 'github-actions[bot]');
    ...
    for (const image of sortedImages) {
        ...
        const change = image.oldSize
            ...
            : 'N/A';
        lines.push(`| [${image.name}:${image.newTag}](${imageLink}) | ${image.platform} | ${oldSize} | ${newSize} | ${change} |`);
    }
...
function logSizeReport(changedImages) {
    changedImages.forEach((image) => {
        const delta = image.oldSize != null ? ((image.newSize / image.oldSize - 1) * 100).toFixed(2) : null;
