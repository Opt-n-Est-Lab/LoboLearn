...
export function emitMessageForWorkspace(workspaceId, event, ...args) {
    if (!socketIoServer)
 ...
 */
export async function updateWorkspaceMessage(workspace_id, message, toDatabase = true) {
    if (toDatabase)
        await queryAsync(sql.update_workspace_message, { workspace_id, message });
 ...
 * @param message The workspace's new message.
 */
export async function updateWorkspaceState(workspace_id, state, message = '') {
    // TODO: add locking
    const duration_milliseconds = await queryRow(sql.update_workspace_state, { workspace_id, state, message }, IntervalSchema);
...
export async function getWorkspaceGradedFiles(workspaceDir, gradedFiles, limits) {
    const files = (await fg(gradedFiles, {
        cwd: workspaceDir,
        stats: true,
    ...
    if (totalSize > limits.maxSize) {
        throw new Error(`Workspace files exceed limit of ${filesize(limits.maxSize, {
            base: 2,
        })}.`);
    }
    return files;
 ...
 */
export async function updateWorkspaceDiskUsage(workspace_id, workspacesRoot) {
    ...
    for (let i = 1; i <= version; i++) {
        const workspaceVersionPath = path.join(workspacesRoot, `workspace-${workspace.id}-${workspace.version}`);
...
async function getDirectoryDiskUsage(dir) {
    let size = 0;
...
async function getWorkspaceFiles(dir) {
    try {
 ...
 */
export async function updateCourseInstanceUsagesForWorkspace({ workspace_id, duration_milliseconds, }) {
